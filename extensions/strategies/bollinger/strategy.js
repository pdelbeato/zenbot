...
module.exports = {
	...
	getCommands: function (s, opts = {}) {
		...
		this.command('bollinger', 'w', {desc: 'toggle Dump Watchdog'.grey, action: function() {
		...
		this.command('bollinger', 'W', {desc: 'toggle Pump Watchdog'.grey, action: function() {
		...
		this.command('bollinger', 'u', {desc: ('Avoid selling at a profit below this pct (for long positions)'.grey + ' INCREASE'.green), action: function() {
		...
		this.command('bollinger', 'j', {desc: ('Avoid selling at a profit below this pct (for long positions)'.grey + ' DECREASE'.red), action: function() {
			strat_opts.sell_min_pct = Number((strat_opts.sell_min_pct - 0.05).toFixed(2))
			console.log('\n' + 'Bollinger - Sell min pct' + ' DECREASE'.red + ' -> ' + strat_opts.sell_min_pct)
		}})
		this.command('bollinger', 'i', {desc: ('Avoid buying at a profit below this pct (for short positions)'.grey + ' INCREASE'.green), action: function() {
		...
		this.command('bollinger', 'k', {desc: ('Avoid buying at a profit below this pct (for short positions)'.grey + ' DECREASE'.red), action: function() {
			strat_opts.buy_min_pct = Number((strat_opts.buy_min_pct - 0.05).toFixed(2))
