...
module.exports = {
  ...
  getOptions: function (strategy_name) {
    ...
    this.option(strategy_name, 'over_and_back_stoch', 'Emit signal when price comes back inside the Stochastic band', Boolean, false)
    this.option(strategy_name, 'stoch_periods', 'Time period for building the Fast-K line', Number, 14)
    this.option(strategy_name, 'stoch_k', 'Smoothing for making the Slow-K line. Usually set to 3', Number, 3)
    this.option(strategy_name, 'stoch_k_ma_type', 'Type of Moving Average for Slow-K : SMA,EMA,WMA,DEMA,TEMA,TRIMA,KAMA,MAMA,T3', String, 'SMA'),
    this.option(strategy_name, 'stoch_k_sell_threshold', 'K must be above this before selling', Number, 70)
...
//					console.log('bollinger strategy - nuovi limiti. upperBound ' + upperBound + ' ; lowerBound= ' + lowerBound)
  ...
  onReport: function (s, opts = {}, callback = function () { }) {
    ...
    function _onReport(cb) {
      if (strat.data.bollinger.upperBound && strat.data.stoch.k) {
        let upperBound = strat.data.bollinger.upperBound
        let lowerBound = strat.data.bollinger.lowerBound
        let midBound = strat.data.bollinger.midBound
        let upperBandwidth = (strat.data.bollinger.upperBound - strat.data.bollinger.midBound)
        ...
        if (strat.data.stoch.k > strat.opts.stoch_k_sell_threshold || strat.data.stoch.k < strat.opts.stoch_k_buy_threshold) {
          color_stoch = 'red'
        }
        ...
        cols.push('(' + s.tools.zeroFill(2, n(strat.data.stoch.k).format('0'), ' ')[color_stoch] + ')')
      }
      else {
