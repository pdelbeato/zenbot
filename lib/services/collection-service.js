...
module.exports = function (conf, cb = function () { }) {
	...
	if (conf.is_sim) {
		...
		db_names.forEach(function (db_name, index) {
			...
			if (db_name === 'trades'  || db_name === 'resume_markers' || db_name === 'sim_results') {
				db[db_name].createIndex({ selector: 1, time: 1 })
			}
			else {
				db[db_name].deleteMany()
			}
		})
		_.set(conf, 'db', db)
		cb()		
		...
		checkDirectory(conf.db.dir, function (err) {
			...
			db_names.forEach(function (db_name, index) {
				...
				//Method aliases
				db[db_name].updateOne = db[db_name].update
				db[db_name].deleteOne = db[db_name].remove
				db[db_name].deleteMany = db[db_name].remove
			})
