...
//		}
		...
		if (s.options.strategy[strategy_name].calc_lookback[i].close.length >= length) {
			marketData[source_key] = s.options.strategy[strategy_name].calc_lookback[i].close.slice()
...
//			if (marketData[source_key].length >= length) {
			talib.execute({
				...
				result = result / s.period.close * 1000
				if (err) {
					console.log(err)
					reject(err, result)
					return
