...
module.exports = function ta_bollinger(s, key = 'close', strategy_name, rsi_periods = 14, DevUp = 2, DevDn = 2, d_ma_type = 'SMA') {
  return new Promise(function (resolve, reject) {
    let tmpMarket = JSON.parse(JSON.stringify(s.options.strategy[strategy_name].calc_lookback))
    
    if (tmpMarket.length >= rsi_periods) {
      tmpMarket = tmpMarket.map(x => x[key])
      tmpMarket.reverse()

      //Non devo inserire periodi aggiuntivi. Calc_lookback ha tutto fino all'ultimo periodo necessario
