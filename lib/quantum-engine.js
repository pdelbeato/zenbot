...
module.exports = function (s, conf) {
	...
	function switchOnListener() {
		eventBus.once('standard', (signal, sig_id, fixed_size, fixed_price, is_reorder, is_taker) => {
			debug.msg('Listener -> standard ' + signal)
			executeSignal (signal, 'standard', sig_id, fixed_size, fixed_price, is_reorder, is_taker)
	...
	function emitSignal (sig_type, signal, sig_id, fixed_size, fixed_price, is_reorder, is_taker) {
		eventBus.emit(sig_type, signal, sig_id, fixed_size, fixed_price, is_reorder, is_taker)
	}
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		if (!s.orders[orderSignal])
			s.orders[orderSignal] = []
		
		if (!s.orders[orderSignal][orderType])
			s.orders[orderSignal][orderType] = []
		
		if (s.positions.find(x => x.id === orderId)) {
			debug.msg('initOrder - Esiste una posizione ' + orderId + '. Associo s.orders.' + orderSignal + '.' + orderType + '.' + orderId)
			s.orders[orderSignal][orderType][orderId] = s.positions.find(x => x.id === orderId)
			s.orders[orderSignal][orderType][orderId].status = 'linked'
			s.orders[orderSignal][orderType][orderId].signal = orderSignal	
			s.orders[orderSignal][orderType][orderId].time = null
			s.orders[orderSignal][orderType][orderId].orig_time = null //Tempo di chiusura dell'ultimo trade della posizione
			s.orders[orderSignal][orderType][orderId].local_time = null //Tempo locale di chiusura dell'ultimo trade della posizione
			s.orders[orderSignal][orderType][orderId].initial_price = null //Prezzo iniziale dell'ultimo trade della posizione
			s.orders[orderSignal][orderType][orderId].price = null //Prezzo finale dell'ultimo trade della posizione
			s.orders[orderSignal][orderType][orderId].fixed_price = null
			s.orders[orderSignal][orderType][orderId].fee = 0
			s.orders[orderSignal][orderType][orderId].size = null //Size attuale del trade (variabile di servizio)
			s.orders[orderSignal][orderType][orderId].remaining_size = null //Rimanenza del trade (serve per i riordini)
			s.orders[orderSignal][orderType][orderId].filled_size = 0 //Size commerciato del trade (variabile di servizio)
			s.orders[orderSignal][orderType][orderId].executed_value = 0 //Value commerciato del trade (variabile di servizio)
			s.orders[orderSignal][orderType][orderId].order_type = (is_taker ? 'taker' : so.order_type)
			s.orders[orderSignal][orderType][orderId].order_id = null
			s.orders[orderSignal][orderType][orderId].order_status = null
		}
		else {
			debug.msg('initOrder - Non esiste una posizione ' + orderId + '. Creo s.orders.' + orderSignal + '.' + orderType + '.' + orderId)
			
			s.orders[orderSignal][orderType][orderId] = {
				id: orderId,
				selector: so.selector.normalized,
				status: 'created',
				position_side: orderSignal, //Side della posizione
				position_price: null, //Prezzo medio della posizione
				position_size: 0, //Valore asset attuale della posizione
				position_value: 0, //Valore currency attuale della posizione
				position_time: 0, //Time apertura della posizione
				signal: orderSignal,	
				time: null,
		...
		debug.printPosition(s.orders[orderSignal][orderType][orderId])
	}
	...
	function syncBalance (cb) {
		s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
			...
			getQuote(function (err) {
				...
				s.positions.forEach(function (position, index) {
					if (position.position_side == 'buy')
						s.available_balance.asset -= position.position_size
					else
						s.available_balance.currency -= position.position_value	
				})
...
//				debug.msg('syncBalance - balance= ' + JSON.stringify(s.balance) + ' ; available_balance= ' + JSON.stringify(s.available_balance))
	...
	function executeSignal (signal, sig_type = 'standard', sig_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		let expected_fee = 0
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		if (is_reorder) {
			...
			if (orderExist(signal, sig_type, sig_id)) {
				debug.msg('executeSignal - Riordine ' + signal + ' ' + sig_type + ' ' + sig_id)
			}
			//...altrimenti esco, perchè non può essere un riordine senza s.orders associato
			else {
				debug.msg('executeSignal - Riordine ma non esiste s.orders.' + signal + '.' + sig_type + '.' + sig_id + ' -> esco.')
				_cb && _cb(null, null)
				return
			}
		}
		//Se non è un riordine...
			...
			if (orderExist(signal, sig_type, sig_id)) {
				debug.msg('executeSignal - Annullo perchè esiste già s.orders.' + signal + '.' + sig_type + '.' + sig_id)
				_cb && _cb(null, null)
				return
			}
			//...altrimenti continuo con i controlli
			else {			
				//Se esiste sig_id, allora è un ordine specifico
				if (sig_id) {
					//Se contemporaneamente c'è un ordine standard aperto su questa posizione, annullo questo executeSignal
					if (orderExist(signal, 'standard', sig_id)) {
						debug.msg('executeSignal - con sig_id. Annullo, perchè esiste ordine standard aperto su ' + sig_id)
						_cb && _cb(null, null)
						return
					}
					
					//Se esiste una posizione sig_id, allora creo l'ordine partendo da quella posizione
					position = s.positions.find(x => x.id === sig_id)
					if (position) {
						//Controllo coerenza signal con eventuale posizione sig_id
						if (position.position_side === signal) {
							debug.msg('executeSignal - con sig_id. Annullo, perchè signal ' + signal + ' non coerente con la posizione (side = ' + position.position_side + ')')
							_cb && _cb(null, null)
							return
						}
						//Creo l'ordine a partire dai valori della posizione sig_id
						else {
							initOrder(signal, sig_type, sig_id, is_taker)
						}
					}
					//Non esiste una posizione sig_id, quindi esco.
					else {
						debug.msg('executeSignal - con sig_id. Annullo, perchè non esiste una posizione aperta ' + sig_id)
						_cb && _cb(null, null)
						return
					}
				}
				//Se sig_id non esiste, allora è un ordine non specifico
				else {
					...
					if (sig_type == 'standard') {			
						...
						if (s.orders[signal] && s.orders[signal]['standard'] && s.orders[signal]['standard'].length != 0) {
							debug.msg('executeSignal - Annullo perchè esiste già s.orders.' + signal + '.standard')
							_cb && _cb(null, null)
							return
						}
						//Creo un sig_id da associare all'ordine
						sig_id = crypto.randomBytes(4).toString('hex')
				
						//Creo l'ordine associato
						debug.msg('executeSignal - senza sig_id - Creo nuovo ordine ' + signal + ' standard ' + sig_id)
						initOrder(signal, 'standard', sig_id, is_taker)
						...
						if (s.max_profit_position.trend[signal_opposite]) {
							...
							getQuote(function (err) {
								if (err) {
									debug.msg('executeSignal - getQuote - error - Annullo')
									_cb && _cb(null, null)
									delete s.orders[signal][sig_type][sig_id]
									return
								}
								...
								if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct + so['max_' + signal + '_loss_pct'] < 0)) {
									debug.msg('executeSignal - standard senza sig_id - Non ci sono posizioni in profitto (max ' + n(position.profit_pct).format('0.00%') + ')')
								...
								// associo un ordine standard a quella posizione
									...
									debug.msg('executeSignal - standard senza sig_id - Prendo max_profit_position.trend.' + signal_opposite + '.id ' + position.id + ' con profitto (' + n(position.profit_pct).format('0.00%') + ')')
									delete s.orders[signal][sig_type][sig_id]
						...
						//E' un ordine standard senza sig_id, quindi dettato dalla strategia. Cancello TUTTI gli ordini di senso opposto ancora in essere
						if (s.orders[signal_opposite])
							delete s.orders[signal_opposite]['standard']
					}
					//Altrimenti creo un ordine sig_type nuovo
						...
						//Creo l'ordine associato
						debug.msg('executeSignal - senza sig_id - Creo nuovo ordine ' + signal + ' ' + sig_type + ' ' + sig_id)
						initOrder(signal, sig_type, sig_id, is_taker)
					}
				}
			}
		...
		let cb = function (err, order) {
			if (!order) {
				debug.msg('executeSignal - cb - cancello s.orders.' + signal + '.' + sig_type + '.' + sig_id)
				if (s.positions[sig_id]) {
					s.positions[sig_id].profit_stop = null
					s.positions[sig_id].profit_stop_limit = null
				}
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			s.orders[signal][sig_type][sig_id].price = fixed_price || nextPriceForQuote(signal)
			if (!s.orders[signal][sig_type][sig_id].initial_price)
				s.orders[signal][sig_type][sig_id].initial_price = s.orders[signal][sig_type][sig_id].price
				
			//Controlli sul prezzo (da sostituire con funzione?)
			//Controllo Limit Price Protection
			if (signal === 'buy' && so.buy_price_limit != null && s.orders[signal][sig_type][sig_id].price > so.buy_price_limit) {
				let err = new Error('\nPrice limit protection')
				err.desc = 'refusing to buy at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', buy price limit -> ' + formatCurrency(so.buy_price_limit, s.currency) + '\n'
				return cb(err)
			}
			if (signal === 'sell' && so.sell_price_limit != null && s.orders[signal][sig_type][sig_id].price < so.sell_price_limit) {
				let err = new Error('\nPrice limit protection')
				err.desc = 'refusing to sell at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', sell price limit -> ' + formatCurrency(so.sell_price_limit, s.currency) + '\n'
				return cb(err)
			}
			
			//Controllo slippage solo sugli ordini non fixed price
			if (!s.orders[signal][sig_type][sig_id].fixed_price && so.max_slippage_pct != null) {
				slippage = Math.abs(n(s.orders[signal][sig_type][sig_id].initial_price).subtract(s.orders[signal][sig_type][sig_id].price).divide(s.orders[signal][sig_type][sig_id].initial_price).multiply(100).value())
				if (slippage > so.max_slippage_pct) {
					let err = new Error('\nslippage protection')
					err.desc = 'refusing to buy at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', slippage of ' + formatPercent(slippage / 100)
			...
			s.orders[signal][sig_type][sig_id].size = n(fixed_size || s.orders[signal][sig_type][sig_id].remaining_size || s.orders[signal][sig_type][sig_id].position_size || n(so.quantum_size).divide(s.orders[signal][sig_type][sig_id].price)).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')

			if (s.orders[signal][sig_type][sig_id].remaining_size == null)
				s.orders[signal][sig_type][sig_id].remaining_size = s.orders[signal][sig_type][sig_id].size
			
//			if (s.orders[signal][sig_type][sig_id].position_size == null)
//				s.orders[signal][sig_type][sig_id].position_size = s.orders[signal][sig_type][sig_id].size
				
			debug.msg('executeSignal - Size: ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset))
			...
			trade_balance = (s.orders[signal][sig_type][sig_id].size * s.orders[signal][sig_type][sig_id].price)
			...
			if (signal === 'sell' && s.orders[signal][sig_type][sig_id].size >= s.available_balance.asset) {
				let err = new Error('\nInsufficient funds')
				err.desc = 'refusing to sell ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + '. Insufficient funds (' + formatAsset(s.available_balance.asset, s.asset) + ')'
				return cb(err)
			}
			
			if (s.product.min_size && ((s.orders[signal][sig_type][sig_id].size) < Number(s.product.min_size) || ('min_total' in s.product && s.product.min_total && n(s.orders[signal][sig_type][sig_id].size).multiply(s.orders[signal][sig_type][sig_id].price).value() < Number(s.product.min_total)))) {
				let err = new Error('\nMinimum size')
				err.desc = 'refusing to ' + signal + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + '. Minimum size not reached'
				return cb(err)
			}
			
			if (s.product.max_size && s.orders[signal][sig_type][sig_id].size > Number(s.product.max_size)) {
				debug.msg('executeSignal - size = s.product.max_size')
				s.orders[signal][sig_type][sig_id].size = s.product.max_size
			}

//			//Controllo profitto della posizione, solo se non sto eseguendo uno stop loss
//			if (s.orders[signal][sig_type][sig_id].profit_pct && !s.acted_on_stop && so['max_' + signal + '_loss_pct'] != null && (s.orders[signal][sig_type][sig_id].profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
//				let err = new Error('\nPosition ' + sig_id + ' ProfitLoss protection')
//				err.desc = 'refusing to ' + signal + ' at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', ' + (so['max_' + signal + '_loss_pct'] > 0 ? 'LOSS of ' : 'PROFIT of ') + formatPercent(s.orders[signal][sig_type][sig_id].profit_pct) + ' (limit ' + formatPercent(-so['max_' + signal + '_loss_pct'] / 100) + ')\n'
//
//				delete s.orders[signal][sig_type][sig_id]
//				
...
//			}
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_type + ' ' + sig_id + ' order over ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance with a expected fee of ' + formatCurrency(expected_fee, s.currency) + ' (' + fee + '%)')
...
//				}
			...
			switch (signal) {
				case 'buy': {
					if (n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(s.orders[signal][sig_type][sig_id].price).multiply(s.orders[signal][sig_type][sig_id].size).value() && s.balance.currency_hold > 0) {
						...
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_type, sig_id)
							}
						}, conf.wait_for_settlement)
					}
					else {
						s.hold_signal = false
						pushMessage('Buying ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + ' on ' + s.exchange.name.toUpperCase(), 'placing ' + signal + ' order at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', ' + formatCurrency(n(s.quote.bid - s.orders[signal][sig_type][sig_id].price).format('0.00'), s.currency) + ' under best bid\n', 9)
...
//						}
				...
				case 'sell': {
					...
					if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(s.orders[signal][sig_type][sig_id].size).value()) {
						...
						debug.msg('executeSignal - s.balance.asset ' + s.balance.asset + ' s.balance.asset_hold ' + s.balance.asset_hold + ' size ' + s.orders[signal][sig_type][sig_id].size)
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_type, sig_id)
						...
						s.hold_signal = false
						pushMessage('Selling ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + ' on ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency) + ', ' + formatCurrency(n(s.orders[signal][sig_type][sig_id].price - s.quote.ask).format('0.00'), s.currency) + ' over best ask\n', 9)
...
//						}
		...
		function doOrder () {
			...
			placeOrder(function (err, order) {
				if (err) {
					err.desc = 'executeSignal - doOrder - Could not execute ' + signal + ' ' + sig_type + ' ' + sig_id + ': error placing order'
					return cb(err)
				}
				...
				if (!order) {
					if (order === false) {
						...
						debug.msg('executeSignal - doOrder - not enough balance, or signal switched, cancel ' + signal + ' ' + sig_type + ' ' + sig_id)
						return cb(null, null)
					}
					if (s.last_signal !== signal) {
						// order timed out but a new signal is taking its place
						debug.msg('executeSignal - doOrder - signal switched, cancel ' + signal + ' ' + sig_type + ' ' + sig_id)
						return cb(null, null)
					}
					// order timed out and needs adjusting
					debug.msg('executeSignal - doOrder - ' + signal + ' ' + sig_type + ' ' + sig_id + ' order timed out, adjusting price')
//					let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
					if (s.orders[signal][sig_type][sig_id].remaining_size != s.orders[signal][sig_type][sig_id].size) {
						debug.msg('executeSignal - doOrder - remaining size: ' + s.orders[signal][sig_type][sig_id].remaining_size + ' of ' + s.orders[signal][sig_type][sig_id].size)
					}
					return executeSignal(signal, sig_type, sig_id, null, null, true, is_taker, _cb)
				}
				cb(null, order)
			})
		}
		...
		function placeOrder (cb) {
			let order_copy = JSON.parse(JSON.stringify(s.orders[signal][sig_type][sig_id]))
			...
			s.exchange[signal](order_copy, function (err, api_order) {
				...
				if (api_order.status === 'rejected') {
					...
					if (api_order.reject_reason === 'post only') {
						...
						debug.msg('executeSignal - placeOrder - post-only ' + signal + ' ' + sig_type + ' ' + sig_id + ' failed, re-ordering')
						return cb(null, null)
					}
					else if (api_order.reject_reason === 'balance') {
						// treat as a no-op.
						debug.msg('executeSignal - placeOrder - not enough balance for ' + signal + ' ' + sig_type + ' ' + sig_id + ', aborting')
						return cb(null, false)
					}
					else if (api_order.reject_reason === 'price') {
						// treat as a no-op.
						debug.msg('executeSignal - placeOrder - invalid price for ' + signal + ' ' + sig_type + ' ' + sig_id + ', aborting')
						return cb(null, false)
					}
					err = new Error('\norder rejected')
					err.order = api_order
				...
				debug.msg('placeOrder - ' + signal + ' ' + sig_type + ' ' + sig_id + ' order placed at ' + formatCurrency(s.orders[signal][sig_type][sig_id].price, s.currency))
				s.orders[signal][sig_type][sig_id].order_id = api_order.id

				//Con ordine piazzato, lo marca temporalmente
				if (!s.orders[signal][sig_type][sig_id].orig_time) {
					s.orders[signal][sig_type][sig_id].orig_time = new Date(api_order.created_at).getTime()
				}
				s.orders[signal][sig_type][sig_id].time = new Date(api_order.created_at).getTime()
				s.orders[signal][sig_type][sig_id].local_time = now()
				s.orders[signal][sig_type][sig_id].order_status = api_order.status
				
				setTimeout(function() { checkOrder(signal, sig_type, sig_id, cb) }, so.order_poll_time)
			})
		}
	}

	function checkOrder (signal, sig_type, sig_id, cb) {
		//debug.msg('checkOrder')
		
		if (!s.orders[signal][sig_type][sig_id]) {
			// signal switched, stop checking order
			debug.msg('checkOrder - signal switched during ' + signal + ' ' + sig_type + ' ' + sig_id + ', aborting')
			pushMessage('Signal switched during ' + signal + ' ' + sig_type + ' ' + sig_id, ' aborting', 9)
			return cancelOrder(signal, sig_type, sig_id, false, cb)
		}
		s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
			if (err)
				return cb(err)
			...
			if (api_order) {
				...
				if (api_order.reject_reason)
					s.orders[signal][sig_type][sig_id].reject_reason = api_order.reject_reason

				//Ordine eseguito!!
				if (api_order.status === 'done') {
					s.orders[signal][sig_type][sig_id].time = new Date(api_order.done_at).getTime()
					s.orders[signal][sig_type][sig_id].price = api_order.price || s.orders[signal][sig_type][sig_id].price // Use actual price if possible. In market order the actual price (api_order.price) could be very different from trade price
					s.orders[signal][sig_type][sig_id].filled_size = api_order.filled_size
					s.orders[signal][sig_type][sig_id].executed_value += api_order.executed_value
					s.orders[signal][sig_type][sig_id].remaining_size = n(s.orders[signal][sig_type][sig_id].size).subtract(api_order.filled_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
					if (s.orders[signal][sig_type][sig_id].position_side == signal) {
						debug.msg('checkOrder - getOrder - side == signal')
						s.orders[signal][sig_type][sig_id].position_value += api_order.executed_value
						s.orders[signal][sig_type][sig_id].position_size += api_order.filled_size
					}
					else {
						debug.msg('checkOrder - getOrder - side != signal')
						s.orders[signal][sig_type][sig_id].position_value -= api_order.executed_value
					...
					debug.msg('checkOrder - getOrder - done - api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.value= ' + s.orders[signal][sig_type][sig_id].position_value)
					executeOrder(signal, sig_type, sig_id)
				...
				//Ordine rifiutato
				if (s.orders[signal][sig_type][sig_id].status === 'rejected' && (s.orders[signal][sig_type][sig_id].reject_reason === 'post only' || api_order.reject_reason === 'post only')) {
					debug.msg('checkOrder - post-only ' + signal + ' ' + sig_type + ' ' + sig_id + ' failed, re-ordering')
					return cb(null, null)
				}
				if (s.orders[signal][sig_type][sig_id].status === 'rejected' && s.orders[signal][sig_type][sig_id].reject_reason === 'balance') {
					debug.msg('checkOrder - not enough balance for ' + signal + ' ' + sig_type + ' ' + sig_id + ', aborting')
					return cb(null, null)
				}
			}				
				
			//Controllo se è trascorso so.order_adjust_time senza che l'ordine sia stato eseguito.
			if (!s.orders[signal][sig_type][sig_id].fixed_price && (now() - s.orders[signal][sig_type][sig_id].local_time >= so.order_adjust_time)) {
				getQuote(function (err) {
					if (err) {
					...
					if (signal === 'buy') {
						if (s.orders[signal][sig_type][sig_id].price < s.quote.bid) {
							debug.msg('checkOrder - ' + s.quote.bid + ' > our ' + s.orders[signal][sig_type][sig_id].price)
							cancelOrder(signal, sig_type, sig_id, true, cb)
						}
						else {
							s.orders[signal][sig_type][sig_id].local_time = now()
							setTimeout(function() { checkOrder(signal, sig_type, sig_id, cb) }, so.order_poll_time)
					...
					if (signal === 'sell') {
						if (s.orders[signal][sig_type][sig_id].price > s.quote.ask) {
							debug.msg('checkOrder - ' + s.quote.ask + ' < our ' + s.orders[signal][sig_type][sig_id].price)
							cancelOrder(signal, sig_type, sig_id, true, cb)
						}
						else {
							s.orders[signal][sig_type][sig_id].local_time = now()
	...
	function cancelOrder (signal, sig_type, sig_id, do_reorder, cb) {
		s.exchange.cancelOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function () {
				...
				s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.value= ' + s.orders[signal][sig_type][sig_id].position_value, false)
							
							if (!do_reorder || !((s.product.min_size && s.orders[signal][sig_type][sig_id].remaining_size >= Number(s.product.min_size)) || (s.product.min_total && n(s.orders[signal][sig_type][sig_id].remaining_size).multiply(s.orders[signal][sig_type][sig_id].price).value() >= Number(s.product.min_total)))) {
...
//							s.orders[signal][sig_type][sig_id].filled_size = api_order.filled_size
//							s.orders[signal][sig_type][sig_id].remaining_size = n(s.orders[signal][sig_type][sig_id].size).subtract(api_order.filled_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
//							s.orders[signal][sig_type][sig_id].cost += n(api_order.filled_size).multiply(api_order.price)
//							debug.msg('cancelOrder - getOrder - cancel failed - order partially done - api_order.filled_size= ' + api_order.filled_size + ' ; s.orders[signal][sig_type][sig_id].remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.cost= ' + s.orders[signal][sig_type][sig_id].cost)
//							if (!((s.product.min_size && s.orders[signal][sig_type][sig_id].remaining_size >= Number(s.product.min_size)) || (s.product.min_total && n(s.orders[signal][sig_type][sig_id].remaining_size).multiply(s.orders[signal][sig_type][sig_id].price).value() >= Number(s.product.min_total)))) {
//								debug.msg('cancelOrder - s.orders[signal][sig_type][sig_id].remaining_size < minimo ordine possibile (o errore equivalente)')
////								s.orders[signal][sig_type][sig_id].time = new Date(api_order.done_at).getTime()
////								order.size = order.orig_size - order.remaining_size
////								order.size = order.filled_size
//								executeOrder(signal, sig_type, sig_id)
//								return syncBalance(function () {
//									cb(null, s.orders[signal][sig_type][sig_id])
//								})
//							}
...
//************** Da controllare questo punto.						
						if (signal === 'buy')
							on_hold = n(s.available_balance.currency).subtract(s.balance.currency_hold || 0).value() < n(s.orders[signal][sig_type][sig_id].price).multiply(s.orders[signal][sig_type][sig_id].remaining_size).value()
						else
							on_hold = n(s.available_balance.asset).subtract(s.balance.asset_hold || 0).value() < n(s.orders[signal][sig_type][sig_id].remaining_size).value()
	...
	function executeOrder (signal, sig_type, sig_id) {
...
//			s.orders[signal_id] = trade
		...
		if (so.order_type === 'maker') {
			if (s.exchange.makerFee)
				fee = n(s.orders[signal][sig_type][sig_id].size).multiply(s.exchange.makerFee / 100).value()
		}
		if (so.order_type === 'taker') {
			if (s.exchange.takerFee)
				fee = n(s.orders[signal][sig_type][sig_id].size).multiply(s.exchange.takerFee / 100).value()
		}
		...
		let my_trade = {
			id: sig_id,
			_id: sig_id,
			order_id: s.orders[signal][sig_type][sig_id].order_id,
			time: s.orders[signal][sig_type][sig_id].time,
			execution_time: s.orders[signal][sig_type][sig_id].time - s.orders[signal][sig_type][sig_id].orig_time,
			slippage: (signal == 'buy' ? 1 : -1) * n(s.orders[signal][sig_type][sig_id].price).subtract(s.orders[signal][sig_type][sig_id].initial_price).divide(s.orders[signal][sig_type][sig_id].initial_price).value(),
			side: signal,
			size: s.orders[signal][sig_type][sig_id].filled_size,
			fee: fee,
			price: n(s.orders[signal][sig_type][sig_id].executed_value).divide(s.orders[signal][sig_type][sig_id].filled_size).format('0.00'),
			//Al cost manca il fee... da aggiungere una volta capito come calcolarlo
			value: s.orders[signal][sig_type][sig_id].position_value,
			order_type: so.order_type || 'taker',
			// profit: s.last_sell_price && (s.last_sell_price - price) / s.last_sell_price, //Da togliere/modificare
			profit: (s.positions[sig_id] != null ? s.positions[sig_id].profit_pct : null),
			position_span: (s.positions[sig_id] != null ? (moment.duration(s.orders[signal][sig_type][sig_id].time - s.positions[sig_id].position_time).humanize()) : null),
		...
		if (so.stats) {
			...
			order_complete += n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + formatCurrency(s.orders[signal][sig_type][sig_id].initial_price, s.currency) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
			order_complete += 'Positions: ' + (s.positions.length+1)
			if (s.positions[sig_id] != null) {
				order_complete += '\nOriginal price: ' + formatCurrency(s.positions[sig_id].position_price, s.currency)
...
//			order_complete += (s[signal_id + '_order'].status != null ? s[signal_id + '_order'].status : '')
		...
		if (s.positions[sig_id] != null) {
			//Da controllare. E' capitato che su un ordine parziale poi chiuso è risultato s.orders.remaining_size=0.0000000 in checkOrder, ma s.positions.remaining_size=null,
			// e quindi il controllo successivo è fallito, trascinandosi NaN e non chiudendo la posizione
			if (s.positions[sig_id].remaining_size != 0) {
//				s.positions[sig_id].remaining_size -= my_trade.size
				debug.msg('executeOrder - posizione ' + sig_id + ' non chiusa completamente. Rimangono ' + formatAsset(s.positions[sig_id].remaining_size, s.asset))
				debug.printPosition(s.positions[sig_id])
				
				s.update_position_id = sig_id
				s.delete_position_id = null
			} 
			else {
				//Elimino la posizione da s.positions
				debug.msg('executeOrder - delete position ' + sig_id + ' (lenght attuale ' + s.positions.length +')')
				
				s.update_position_id = null
				s.delete_position_id = sig_id
				s.positions.forEach(function (position, index) {
					if (position.id == sig_id) {
						debug.msg('executeOrder - delete s.position ' + sig_id + '(index ' + index + ')')
						s.positions.splice(index,1)
					}
				})
				updatePositions(s.orders[signal][sig_type][sig_id])

				//Cancella anche tutti gli ordini connessi con la posizione
				s.orders.forEach(function (order, index) {
					if (order.id == sig_id) {
						debug.msg('executeOrder - delete s.orders.' + signal + '.' + sig_type + '.' + sig_id)
						delete order
					}
				})
...
//			id: orderId,
//			selector: so.selector.normalized,
			s.orders[signal][sig_type][sig_id].status = 'opened'
//			position_side: //Side della posizione
			s.orders[signal][sig_type][sig_id].position_price = (s.orders[signal][sig_type][sig_id].position_price || n(s.orders[signal][sig_type][sig_id].position_value).divide(s.orders[signal][sig_type][sig_id].position_size).format('0.00'))
//			position_size: (s.positions[orderId] ? s.positions[orderId].position_size : 0), //Valore asset attuale della posizione
//			position_value: (s.positions[orderId] ? s.positions[orderId].position_value : 0), //Valore currency attuale della posizione
			s.orders[signal][sig_type][sig_id].position_time = (s.orders[signal][sig_type][sig_id].position_time || s.orders[signal][sig_type][sig_id].time) //Time apertura della posizione
//			signal: orderSignal,	
//			time: null,
//			orig_time: null, //Tempo di chiusura dell'ultimo trade della posizione
//			local_time: null, //Tempo locale di chiusura dell'ultimo trade della posizione
			s.orders[signal][sig_type][sig_id].initial_price = null //Prezzo iniziale dell'ultimo trade della posizione
//Trovare il modo per passare il prezzo finale alla funzione updatePositions
//			s.orders[signal][sig_type][sig_id].price = null //Prezzo finale dell'ultimo trade della posizione
			s.orders[signal][sig_type][sig_id].fixed_price = null
			s.orders[signal][sig_type][sig_id].fee = 0
			s.orders[signal][sig_type][sig_id].size = null //Size attuale del trade (variabile di servizio)
			s.orders[signal][sig_type][sig_id].orig_size = null //Size inizio trade (serve per i riordini)
			s.orders[signal][sig_type][sig_id].remaining_size = null //Rimanenza del trade (serve per i riordini)
			s.orders[signal][sig_type][sig_id].filled_size = 0 //Size commerciato del trade (variabile di servizio)
			s.orders[signal][sig_type][sig_id].executed_value = 0 //Value commerciato del trade (variabile di servizio)
			s.orders[signal][sig_type][sig_id].buy_stop = (s.orders[signal][sig_type][sig_id].position_side == 'buy' ? so.buy_stop_pct && n(s.orders[signal][sig_type][sig_id].position_price).multiply(1 + so.buy_stop_pct/100).value() : null)
			s.orders[signal][sig_type][sig_id].sell_stop = (s.orders[signal][sig_type][sig_id].position_side == 'sell' ? so.sell_stop_pct && n(s.orders[signal][sig_type][sig_id].position_price).multiply(1 - so.sell_stop_pct/100).value() : null)
//			profit_pct: (s.positions[orderId] ? s.positions[orderId].profit_pct : null),
//			profit_stop_limit: (s.positions[orderId] ? s.positions[orderId].profit_stop_limit : null),
//			profit_stop: (s.positions[orderId] ? s.positions[orderId].profit_stop : null),
//			order_type: is_taker ? 'taker' : so.order_type,
//			order_id: null,
//			order_status: null,
//			product_id: s.product_id,
...
//			cancel_after: s.cancel_after // s.cancel_after || 'day'
			...
			if (!s.positions[sig_id]) {
				s.positions[sig_id] = s.orders[signal][sig_type][sig_id]
				s.positions.push(s.positions[sig_id])
			}
...
//		executeSignal('sell', undefined, undefined, undefined, undefined, my_position.id)
	...
	function writeReport (is_progress, blink_off) {
		...
		if (s.orders['buy'] && s.orders['buy'].length != 0) {
			process.stdout.write(z(9, 'buying', ' ').green)
		}
		else if (s.orders['sell'] && s.orders['sell'].length != 0) {
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			position.profit_pct = (position.position_side == 'buy' ? +100 : -100) * (trade.price - position.position_price) / position.position_price
//			debug.msg('updatePositions - max_profit= ' + max_profit, false)
			if (so.profit_stop_enable_pct && position.profit_pct >= (so.profit_stop_enable_pct)) {
				position.profit_stop_limit = (position.position_side == 'buy' ? (Math.max(position.profit_stop_limit || trade.price, trade.price)) : (Math.min(position.profit_stop_limit || trade.price, trade.price)))
				position.profit_stop = position.profit_stop_limit + (position.position_side == 'buy' ? -1 : +1) * (position.profit_stop_limit * (so.profit_stop_pct / 100))
				if (position.profit_pct >= max_trail_profit) {
					max_trail_profit = position.profit_pct
					s.max_profit_position.trail[position.position_side] = position
//					debug.msg('updatePositions - max_profit_position_id.trail.' + position.side + ' = ' + position.id, false)
				...
				s.max_profit_position.trend[position.position_side] = position
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	//Funzione per controllare l'esistenza di un ordine specifico
	function orderExist(signal, sig_type, sig_id) {
		if (s.orders[signal] && s.orders[signal][sig_type] && s.orders[signal][sig_type][sig_id])
			return true
		else
			return false
	}
