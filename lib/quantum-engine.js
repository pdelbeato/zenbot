...
//					order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
					...
					if (order_tmp.position.side == signal) {
						...
						order_tmp.position.fee_open = order_tmp.fee
						...
						order_tmp.position.fee_close = order_tmp.fee
					}
...
//			debug.msg('updatePositions')
			...
			s.positions.forEach(function (position, index) {
				...
				let price_open_tmp = n(position.price_open).add((position.side == 'buy' ? +1 : -1) * fee_open)
