...
//					order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
					order_tmp.fee = n(order_tmp.executed_value).multiply(order_tmp.fee_pct/100).add(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
					...
					if (order_tmp.position.side == signal) {
						...
						order_tmp.position.fee_open = n(order_tmp.position.fee_open).add(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
						...
						order_tmp.position.fee_close = n(order_tmp.position.fee_close).add(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
					}
				...
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					...
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							order_tmp.executed_value = n(order_tmp.filled_size).multiply(order_tmp.price).format(s.product.increment)
//							order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
							order_tmp.fee = n(order_tmp.executed_value).multiply(order_tmp.fee_pct).add(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
							order_tmp.remaining_size = n(order_tmp.size).subtract(api_order.filled_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
							if (order_tmp.position.side == signal) {
