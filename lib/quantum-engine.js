...
module.exports = function (s, conf) {
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		...
		// so.strategy[strategy_name].data = {}
...
//				position._id = position_id
	...
	function writeReport(is_progress = true, period = s.period, callback = function () {}) {
		...
		s.tools.functionStrategies('onReport', opts
			, null //callbackStrategy
			, function (err, strategy_function) { //callbackFinal
				if (s.action) {
...
//				process.stdout.write(s.tools.zeroFill((currency_col.length + 1), currency_col, ' ').green)
	...
	function onTrade(trade, is_preroll, callback) {
		try {
			...
			function goOn() {
				...
				, null
				, function (err, strategy_function) {
	...
	function updateMessage() {
		syncBalance(function (err) {
			...
			s.tools.functionStrategies('onUpdateMessage', undefined, function (result, strategy_name, strategy_function, cb) {
				if (!result) {
					return cb()
				}
