...
module.exports = function (s, conf) {
	...
	//Inizializza una posizione position_id
	function initPosition (position_id) {
		position = {
				_id: position_id,
				id: position_id,
...
//		else {
//			debug.msg('initOrder - Non esiste una posizione ' + orderId + '. Creo una posizione ' + position_id)
//			order.position = initPosition(position_id)
//			order.position.side = orderSignal, //Side della posizione
//		}
//		
//		order.position.status = 'linked'
//		order.id = order.position.id
					...
					if (sig_type === 'standard') {			
						...
						if (s.max_profit_position.trend[signal_opposite]) {
							...
							if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct + so['max_' + signal + '_loss_pct'] < 0)) {
								debug.msg('executeSignal - standard senza position_id - Non ci sono posizioni in profitto (max ' + n(position.profit_pct).format('0.00%') + ')')
							...
							// associo un ordine standard a quella posizione
								...
								debug.msg('executeSignal - standard senza position_id - Prendo max_profit_position.trend.' + signal_opposite + '.id ' + position.id + ' con profitto (' + n(position.profit_pct).format('0.00%') + ')')
						...
						orderSetStatus(signal_opposite, 'standard', undefined, 'canceled', function(){})
					}
...
//			if (s.orders[signal][sig_type][position_id].position_size == null)
//				s.orders[signal][sig_type][position_id].position_size = s.orders[signal][sig_type][position_id].size
...
//			s.orders[signal_id] = trade
		...
		if (position != null) {
			if (position.size != 0) {
//				s.positions[position_id].remaining_size -= my_trade.size
				debug.msg('executeOrder - posizione ' + position_id + ' non chiusa completamente. Rimangono ' + formatAsset(position.remaining_size, s.asset))
				debug.printPosition(position)
...
//			id: orderId,
//			selector: so.selector.normalized,
			order_tmp.position.status = 'free'
//			position_side: //Side della posizione
			order_tmp.position.price = (order_tmp.position.price || n(order_tmp.position.value).divide(order_tmp.position.size).format('0.00'))
...
//			position_value: (s.positions[orderId] ? s.positions[orderId].position_value : 0), //Valore currency attuale della posizione
			order_tmp.position.time = (order_tmp.position.time || order_tmp.time) //Time apertura della posizione
//			signal: orderSignal,	
//			time: null,
//			orig_time: null, //Tempo di chiusura dell'ultimo trade della posizione
//			local_time: null, //Tempo locale di chiusura dell'ultimo trade della posizione
...
//			order_tmp.price = null //Prezzo finale dell'ultimo trade della posizione
			...
			order_tmp.position.buy_stop = (order_tmp.position.side == 'buy' ? so.buy_stop_pct && n(order_tmp.position.price).multiply(1 + so.buy_stop_pct/100).value() : null)
			order_tmp.position.sell_stop = (order_tmp.position.side == 'sell' ? so.sell_stop_pct && n(order_tmp.position.price).multiply(1 - so.sell_stop_pct/100).value() : null)
...
//		delete s.orders[signal][sig_type][position_id]
//		debug.msg('executeOrder - delete s.orders.' + signal + '.' + sig_type + '.' + position_id)
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderSetStatus(signal, sig_type, position_id, status, cb) {
		s.orders.forEach(function (order, index) {
			if ((signal ? order.signal === signal : true) && (sig_type ? order.type === sig_type : true) && (position_id ? order.id === position_id : true)) {
