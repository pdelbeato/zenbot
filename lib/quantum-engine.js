...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			s.orders[signal][sig_type][sig_id].size = (n(fixed_size) || n(s.orders[signal][sig_type][sig_id].remaining_size) || n(so.quantum_size).divide(s.orders[signal][sig_type][sig_id].price)).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
...
//			}
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_type + ' ' + sig_id + ' order over ' + formatAsset(size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance with a expected fee of ' + formatCurrency(expected_fee, s.currency) + ' (' + fee + '%)')
