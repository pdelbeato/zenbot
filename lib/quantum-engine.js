	...
	function nextPriceForQuote(signal) {
		switch (signal) {
			case 'buy': {
				...
				debug.msg('nextPriceForQuote - bid=' + s.quote.bid + ' return=' + npfq)
				return npfq
				break
			}
			case 'sell': {
				...
				debug.msg('nextPriceForQuote - ask=' + s.quote.ask + ' return=' + npfq)
				return npfq
				break
			}
		}
...
//		}
		...
		syncBalance(function (err) {
			...
			//Prepara la quantità
			size = (fixed_size || (working_position ? n(working_position.remaining_size) : n(so.quantum_size).divide(price))).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
