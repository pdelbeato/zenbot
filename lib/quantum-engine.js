...
//				s.signal = 'sell'
	...
	function syncBalance (cb) {
		...
		s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
			...
			s.balance = s.available_balance = balance
...
//				}
	...
	function executeOrder (trade, signal, sig_id) {
		...
		let my_trade = {
			...
			cost: n(price).multiply(s[signal_id + '_order'].orig_size),
...
//			process.stdout.write(z(9, s.signal || '', ' ')[s.signal ? s.signal === ('pump' || 'dump') ? 'white' : s.signal === 'buy' ? 'green' : 'red' : 'grey'])
		...
		// }
		else if (s.max_profit_position_id.trend.buy != undefined || s.max_profit_position_id.trend.sell != undefined) {
			position_buy_profit = -1
			position_sell_profit = -1

			if (s.max_profit_position_id.trend.buy != undefined)
				position_buy_profit = s.my_positions.find(x => x.id == s.max_profit_position_id.trend.buy).profit_pct

			if (s.max_profit_position_id.trend.sell != undefined)	
				position_sell_profit = s.my_positions.find(x => x.id == s.max_profit_position_id.trend.sell).profit_pct

			buysell_profit = (position_buy_profit > position_sell_profit ? ('B' + formatPercent(position_buy_profit)) : ('S' + formatPercent(position_sell_profit)))

			process.stdout.write(z(8, buysell_profit, ' ')[buysell_profit > 0 ? 'green' : 'red'])
		}
		else {
		...
		if (s.max_profit_position_id.trail.buy != undefined || s.max_profit_position_id.trail.sell != undefined) {
			...
			if (s.max_profit_position_id.trail.buy != undefined)
				position_buy_profit = s.my_positions.find(x => x.id == s.max_profit_position_id.trail.buy).profit_pct

			if (s.max_profit_position_id.trail.sell != undefined)	
				position_sell_profit = s.my_positions.find(x => x.id == s.max_profit_position_id.trail.sell).profit_pct

			buysell_profit = (position_buy_profit > position_sell_profit ? ('B' + formatPercent(position_buy_profit)) : ('S' + formatPercent(position_sell_profit)))

			process.stdout.write(z(8, buysell_profit, ' ')['yellow'])
		}
		else {
			process.stdout.write(z(8, '', ' '))
		}
		...
		if (s.orig_capital) {
			...
			if (s.balance.asset != s.available_balance.asset) {
				let available_asset_col = n(s.available_balance.asset).format(s.asset === 'BTC' ? '0.00000' : '0.00000000') + ' ' + s.asset
				available_asset_col_width = Math.max(available_asset_col.length + 1, available_asset_col_width)
				process.stdout.write(z(available_asset_col_width, '(' + available_asset_col + ')', ' ').white)
			}
			...
			if (s.balance.currency != s.available_balance.currency) {
				let available_currency_col = n(s.available_balance.currency).format(isFiat() ? '0.00' : '0.00000000') + ' ' + s.currency
				available_currency_col_width = Math.max(available_currency_col.length + 1, available_currency_col_width)
				process.stdout.write(z(available_currency_col_width, '(' + available_currency_col + ')', ' ').green)
			}
				...
				s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.filled_size) {
							...
							debug.msg('cancelOrder: api_order.filled_size= ' + api_order.filled_size + ' ; order.remaining_size= ' + order.remaining_size)
