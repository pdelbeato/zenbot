...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_kind + ' ' + position_id + ' order over ' + formatAsset(order_tmp.size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance. Expected fee ' + (so.use_fee_asset ? '(FEE ASSET) ' : '') + 'of ' + formatCurrency(expected_fee, s.currency) + ' (' + order_tmp.fill_fees_pct + '%)')
...
//						order_tmp.fill_fees = n(order_tmp.executed_value).multiply(order_tmp.fill_fees_pct).divide(100).add(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						if (!so.use_fee_asset) {
							order_tmp.fill_fees = n(order_tmp.fill_fees).add(api_order.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						}
						else {
							let fill_fees_tmp = n(api_order.executed_value).multiply(order_tmp.fill_fees_pct/100)
							order_tmp.fill_fees = n(order_tmp.fill_fees).add(fill_fees_tmp).format(s.product.increment ? s.product.increment : '0.00000000')
						}
...
//			positionStatus(position, 'Unset', sig_kind)
		...
		updatePositions(order_tmp, function() {
			...
			if (so.stats) {
				...
				order_complete += 'Total ' + formatCurrency((my_trade.executed_value - my_trade.fill_fees), s.currency) + '\n'
				order_complete += 'Fee ' + formatCurrency(my_trade.fill_fees, s.currency) + '\n'
...
//					s.acted_on_trail_stop = false
					...
					if (so.mode !== 'sim' && so.strategy[strategy_name].calc_lookback.length > so.strategy[strategy_name].opts.min_periods) {
						so.strategy[strategy_name].calc_lookback.splice(max_length, (so.strategy[strategy_name].calc_lookback.length - so.strategy[strategy_name].opts.min_periods))
					...
					if (strategyProcessed === array.length) {
						if (so.mode !== 'sim' && s.lookback.length > so.min_periods) {
							s.lookback.splice(max_length, (s.lookback.length - so.min_periods))
