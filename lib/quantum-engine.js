	...
	function executeStop () {
		...
		s.my_positions.forEach( function (position, index) {
			...
			if (!s.acted_on_stop) {
				...
				if (position.sell_stop && s.period.close < position.sell_stop) {
					//	stop_signal = 'sell'
					s.signal = 'sell'
...
//				}
				...
				//Qua sarebbe meglio mettere anche un controllo sul minimo pct di sell, oppure il controllo viene dopo
				// nel momento in cui esegue il segnale di sell? Controllare
		...
		// }
		//Messo dentro i cicli if precedenti
		//		if (stop_signal) {
			...
			if (signal === 'buy') {
				...
				if (is_reorder) {
					if (s[signal + '_order']) {
						...
						debug.msg('executeSignal - is_reorder && s[buy_order]: Remaining size: ' + formatAsset(size, s.asset))
					} else {
						debug.msg('executeSignal - is_reorder && !s[buy_order]: Remaining size: ' + formatAsset(size, s.asset))
					}
				} else {
				...
				if (is_reorder) {
					...
					if (s[signal + '_order']) {
						size = n(s[signal + '_order'].remaining_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
						debug.msg('executeSignal - is_reorder && s[sell_order]: Remaining size: ' + formatAsset(size, s.asset))
					} else {
						debug.msg('executeSignal - is_reorder && !s[sell_order]: Remaining size: ' + formatAsset(size, s.asset))
					}
				} 
...
//				if (position.profit_stop && price > position.profit_stop) {
//					let err = new Error('\nPosition ' + working_position_index + ' Trailing protection')
//					if (so.max_sell_loss_pct > 0) {
//						err.desc = 'refusing to sell at ' + formatCurrency(price, s.currency) + ', sell LOSS of ' + formatPercent(sell_loss / 100) + ' > ' + formatPercent(so.max_sell_loss_pct / 100) + '\n'
//						//pushMessage('Sell LOSS protection', 'aborting')
//					}
...
//						//pushMessage('Sell PROFIT protection', 'aborting')
//					}
//					return cb(err)//					stop_signal = 'sell'
//					s.signal = 'sell'
//						s.acted_on_stop = true
//						console.log(('\nprofit stop triggered at ' + formatPercent(position.profit_pct) + ' trade profit for position ' + index + '\n').green)
//				}
	...
	function writeReport (is_progress, blink_off) {
		...
		if (position_max_trail_profit_index) {
			process.stdout.write(z(9, formatPercent(s.my_positions[position_max_trail_profit_index].profit_pct), ' ')['white'])
		}
...
//		Copia per confronto da onTrade
