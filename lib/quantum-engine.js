...
module.exports = function (s, conf) {
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		so.strategy[strategy_name] = require(path.resolve(__dirname, `../extensions/strategies/${strategy_name}/strategy`))
		...
		if (so.strategy[strategy_name].getOptions) {
			...
			// Alla fine avremo 
			so.strategy[strategy_name].getOptions.call(s.ctx, s)
		}
		...
		if (so.strategy[strategy_name].orderExecuted) {
			s.eventBus.on((strategy_name + '_orderExecuted'), function(signal, position_id) {
				so.strategy[strategy_name].orderExecuted(signal, position_id, conf)
			})
		}
	})
...
//		debug.printPosition(order)
	...
	function updatePeriod(trade) {
		...
		Object.keys(so.strategy).forEach(function (strategy_name, index) {
			so.strategy[strategy_name].calculate(s)
		})
...
//			s.delete_position_id = null
	...
	function writeReport (strategy_name, is_progress, blink_off) {
		...
		if (strategy_name && so.strategy[strategy_name].onReport) {
			let cols = so.strategy[strategy_name].onReport.call(s.ctx, s)
			cols.forEach(function (col) {
				process.stdout.write(col)
			})
		}
		if (orderExist('buy', strategy_name)) {
			process.stdout.write(z(9, 'buying ' + strategy_name, ' ').green)
		}
		else if (orderExist('sell', strategy_name)) {
			process.stdout.write(z(9, 'selling ' + strategy_name, ' ').red)
		}
		else if (s.action) {
		...
		else if (strategy_name && so.strategy[strategy_name].is_dump_watchdog || so.strategy[strategy_name].is_pump_watchdog) {
...
//		updatePositions(trade)
		...
		if (trade.time > s.period.close_time) {
			...
			Object.keys(so.strategy).forEach(function (strategy_name, index, array) {
				...
				so.strategy[strategy_name].onPeriod.call(s.ctx, s, function () {
					strategyProcessed++
