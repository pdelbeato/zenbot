...
//	}
	...
	if (so.catch_order_pct > 0) {
		s.eventBus.on('orderExecuted', (signal, sig_kind, position_id) => {
			...
			if (position && !position.locked && !positionFlags(position, 'status', 'Check', 'catching')) {
				signal_opposite = (signal === 'buy' ? 'sell' : 'buy')
				target_price = n(position.price_open).multiply((signal === 'buy' ? (1 + so.catch_order_pct/100) : (1 - so.catch_order_pct/100))).format(s.product.increment, Math.floor)
				debug.msg('Listener -> catching position ' + signal_opposite + ' ' + sig_kind + ' ' + position_id + ' at ' + target_price)
				//(signal, sig_kind = 'manual', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb)
				executeSignal(signal_opposite, 'catching', position_id, null, target_price, false, false)  
			}
		})
	}
	
	//Funzione per eseguire un metodo di ogni strategia, con i callback per strategia e finale
	function functionStrategies (s, strategy_function, opts = {}, callbackStrategy = function() {}, callbackFinal = function() {}) {
		...
		Object.keys(s.options.strategy).forEach(function (strategy_name, index, array) {
			if (s.options.strategy[strategy_name].lib[strategy_function]) {
				...
				s.options.strategy[strategy_name].lib[strategy_function](s, opts, function () {
					strategyProcessed++
					
					callbackStrategy(strategy_name, strategy_function)
				})
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		...
		s.strategyFlag[strategy_name] = Math.pow(2, (index + 2))
		debug.msg('Inizializzazione strategia - strategyFlag[' + strategy_name + ']= ' + s.strategyFlag[strategy_name])
		console.log(s.strategyFlag)
...
//				profit_stop: null				
	...
	function initOrder (orderSignal, orderKind = 'manual', position_id = null, is_taker = false, cb) {			
		order = {
			id: position_id,
			signal: orderSignal,
			kind: orderKind,
...
//			orig_size: null, //Size inizio trade (serve per i riordini)
		...
		if (orderKind === 'manual')
			positionFlags(order.position, 'locked', 'Set', s.strategyFlags['manual']);
...
//		debug.printPosition(order)
	...
	function updatePeriod(trade) {
		...
		functionStrategies (s, 'onTrade', opts)
...
//				if (position.profit_stop && !position.locked && !positionFlags(position, 'status', 'Check', 'trailstop') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_net_pct > 0) {
//					console.log(('\nProfit stop triggered at ' + formatPercent(position.profit_net_pct/100) + ' trade profit for position ' + position.id + '\n').green)
//					pushMessage('Trailing stop', position.side + ' position ' + position.id + ' (' + formatPercent(position.profit_net_pct/100) + ')', 0)
...
//Controlli timing (da sostituire con funzione?)
				...
				if (!fixed_price && sig_kind != 'trailing_stop' && sig_kind != 'stoploss') {
...
//					}
				...
				if (position_id) {
					...
					if (position) {
						...
						//Controllo se la posizione ha profitto sufficiente e creo l'ordine a partire dai valori della posizione position_id
						else if (so[signal + '_gain_pct'] != null && (position.profit_net_pct - so[signal + '_gain_pct'] > 0)) {
							...
							debug.msg('executeSignal - con position_id. Annullo, perchè la posizione ' + position_id + ' non ha profitto sufficiente (' + position.profit_net_pct + ' < ' + so[signal + '_gain_pct'])
							_cb && _cb(null, null)
							return
						}
					}
...
//							if (so[signal + '_gain_pct'] != null && (position.profit_net_pct - so[signal + '_gain_pct'] > 0)) {
//								debug.msg('executeSignal - ' + sig_kind + ' senza position_id - Prendo max_profit_position.trend.' + signal_opposite + '.id ' + position.id + ' con profitto (' + n(position.profit_net_pct/100).format('0.00%') + ')')
//								
...
//							else {
//								debug.msg('executeSignal - ' + sig_kind + ' senza position_id - Non ci sono posizioni in profitto (max ' + n(position.profit_net_pct/100).format('0.00%') + ')')
//
...
//								if (so.max_positions && s.positions.length >= so.max_positions) {
...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			//Dai fondi disponibili, devo aggiungere i fondi degli ordini catching connessi con la posizione in essere
...
//			Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
			...
			case 'buy': {
				if (Number(tradeable_balance) > Number(available_balance_currency)) {
					...
					s.hold_signal = false
					pushMessage('Buying (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing ' + signal + ' order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(s.quote.bid).subtract(order_tmp.price).format('0.00'), s.currency) + ' under best bid\n', 9)
			...
			case 'sell': {
				if (Number(order_tmp.size) > Number(available_balance_asset)) {
					...
					s.hold_signal = false
					pushMessage('Selling (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(order_tmp.price).subtract(s.quote.ask).format(s.product.increment ? s.product.increment : '0.00000000'), s.currency) + ' over best ask\n', 9)
...
//		debug.msg('executeSignal - exiting ' + signal + ' ' + sig_kind + ' ' + position_id)
		...
		function doOrder () {
			...
			positionFlags(order_tmp.position, 'status', 'Free', undefined, function() {
...
//				delete order_copy.size
			...
			s.exchange[signal](order_copy, function (err, api_order) {					
				...
				positionFlags(order_tmp.position, 'status', 'Set', sig_kind)
...
//		}
		...
		if (!positionFlags(order_tmp.position, 'status', 'Check', sig_kind)) {
...
//						order_tmp.fill_fees = n(order_tmp.executed_value).multiply(order_tmp.fill_fees_pct).divide(100).add(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						if (!so.use_fee_asset) {
...
//Perchè eseguo nuovamene getOrder? Non basta quello fatto sopra?							
							...
							if (!do_reorder) {
								...
								positionFlags(order_tmp.position, 'status', 'Unset', sig_kind)
//								orderDelete(signal, sig_kind, position_id)
							}
						}
					}
...
//		selector: so.selector.normalized,
//		session_id: session.id, //Verrà aggiunto in fase di save nel db Mongo my_trades
//		mode: so.mode
//		}
		...
		updatePositions(order_tmp, function() {
			...
			if (order_tmp.position.id != null) {
				...
				if (order_tmp.position.size != 0 && !isOrderTooSmall(s.product, order_tmp.position.size, order_tmp.price)) {
					...
					functionStrategies (s, 'onPositionUpdate', opts)
...
//					order_tmp.position.sell_stop = ((order_tmp.position.side == 'buy' && so.sell_stop_pct) ? n(order_tmp.position.price_open).multiply(1 - so.sell_stop_pct/100).format(s.product.increment) : null)
				...
				//Se la posizione ha size nullo o comunque inferiore al minimo commerciabile
					...
					//Preparo i dati della posizione chiusa da inserire in s.closed_positions
					position = {
...
//							value: 0, //Valore currency attuale della posizione
							...
							value_close: order_tmp.position.value_close, //n(order_tmp.position.value_open).subtract(order_tmp.position.value).subtract(so.use_fee_asset ? 0 : order_tmp.position.fee_close).format(s.product.increment), //Ricavo currency dalla posizione chiusa
...
//				position._id = position_id	
				...
				functionStrategies (s, 'onPositionOpened', opts)
				
//				position.buy_stop = (position.side == 'sell' ? so.buy_stop_pct && n(position.price_open).multiply(1 + so.buy_stop_pct/100).format(s.product.increment) : null)
//				position.sell_stop = (position.side == 'buy' ? so.sell_stop_pct && n(position.price_open).multiply(1 - so.sell_stop_pct/100).format(s.product.increment) : null)
...
//			updatePositions(order_tmp, function() {
			...
			if (!is_partial) {
				...
				functionStrategies (s, 'onOrderExecuted', opts)
			}
			else {
...
//		debug.msg('writeReport ' + strategy_name)
		...
		volume_display = z(7, volume_display, ' ')
		...
		functionStrategies (s, 'onReport')

//		//		if (strategy_name && so.strategy[strategy_name].lib.onReport) {
//		let cols = so.strategy[strategy_name].lib.onReport(s)
//		cols.forEach(function (col) {
//			process.stdout.write(col)
//		})
//		}
			
		if (order_tmp = orderExist('buy')) {
			process.stdout.write(z(12, 'B ' + order_tmp.kind, ' ').green)
		}
		else if (order_tmp = orderExist('sell')) {
			process.stdout.write(z(12, 'S ' + order_tmp.kind, ' ').red)
		}
		else if (s.action) {
			process.stdout.write(z(12, s.action, ' ')[(s.action === 'bought' || s.action === 'part buy') ? 'green' : 'red'])
		}
		else if (s.signal) {
			process.stdout.write(z(12, s.signal, ' ')['white']) //[s.signal === ('pump' || 'dump') ? 'white' : s.signal === 'buy' ? 'green' : 'red'])
		}
...
//				position_buy_profit = s.max_profit_position.trend.buy.profit_net_pct/100
//
//			if (s.max_profit_position.trend.sell != null)	
//				position_sell_profit = s.max_profit_position.trend.sell.profit_net_pct/100
//
...
//				position_buy_profit = s.max_profit_position.trail.buy.profit_net_pct/100
//
//			if (s.max_profit_position.trail.sell != null)	
//				position_sell_profit = s.max_profit_position.trail.sell.profit_net_pct/100
//				
//			buysell = (position_buy_profit > position_sell_profit ? 'B' : 'S')
//			buysell_profit = (position_buy_profit > position_sell_profit ? formatPercent(position_buy_profit) : formatPercent(position_sell_profit))
//
...
//		debug.msg('withOnPeriod')
		...
		//Aggiorna il period e fa eseguire i calcoli alla strategia (chiamata a onTrade) 
		updatePeriod(trade)
...
//					executeStop()
	...
	function queueTrade(trade, is_preroll) {
		...
		if (s.exchange.debug_exchange) {
			debug.obj('queueTrade - tradeProcessingQueue.workersList: ', tradeProcessingQueue.workersList)
		}
	}
...
//		updatePositions(trade)
		...
		if (trade.time > s.period.close_time) {
			...
			functionStrategies (s, 'onTradePeriod', opts, function(strategy_name) {
				s.action = null
...
//				if (so.strategy[strategy_name].lib.onReport) {
//					writeReport(strategy_name)
//				}
				
				// Se non siamo in sim, ripulisce so.strategy[strategy_name].calc_lookback a un max di valori
				if (so.mode !== 'sim' && so.strategy[strategy_name].calc_lookback.length > so.strategy[strategy_name].opts.min_periods) {
...
//					orderDelete(undefined, undefined, position.id, function() {
					positionFlags(position, 'status', 'Free', undefined, function() {
						//Se siamo in sim, non dobbiamo operare su MongoDB
						//Cancello la posizione dal db delle posizioni aperte e inserisco la posizione chiusa del db delle posizioni chiuse
						if (so.mode != 'sim') {
							s.positionProcessingQueue.push({mode: 'delete', id: position_id})
						}
				...
				position.profit_net_pct = (position.side == 'buy' ? +100 : -100) * n(price_close_tmp).subtract(price_open_tmp).divide(price_open_tmp).value()
				...
				if (!position.locked) {
//					if (so.profit_stop_enable_pct && position.profit_net_pct >= so.profit_stop_enable_pct) {
...
//					}
					...
					//Chiamata alla funzione onPositionUpdate per tutte le strategie
					var opts = {
						position_id: position_id,
						trade: trade,
					};

					functionStrategies (s, 'onPositionUpdated', opts) //, callbackStrategy = function() {}, callbackFinal = function() {})
				}
			})
			s.wait_updatePositions = false
			cb()
...
//			waitCondition('wait_updatePositions', 100, cb)
	...
	function updateMessage() {
		syncBalance(function() {
			...
			functionStrategies (s, 'onUpdateMessage', null, function(result) {
				output_lines += result
			}, function() {
	...
	function orderDelete(signal, sig_kind, position_id, cb = function() {}) {
		s.orders.forEach(function (order, index) {
			if ((signal ? order.signal === signal : true) && (sig_kind ? order.kind === sig_kind : true) && (position_id ? order.id === position_id : true)) {
				...
				if (sig_kind) {
					positionFlags(order.position, 'status', 'Unset', sig_kind)
				}
				else {
					positionFlags(order.position, 'status', 'Free')
				}
...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function orderStatus(order, signal, sig_kind, position_id, mode, status, cb = function() {}) {
		if (order) {
			...
			positionFlags(order.position, 'status', mode, status)
		}
		else {
			s.orders.forEach(function (order, index) {
				if ((signal ? order.signal === signal : true) && (sig_kind ? order.kind === sig_kind : true) && (position_id ? order.id === position_id : true)) {
	...
	function positionFlags(position, flags, mode, value, cb = function() {}) {
		switch (mode) {
		case 'Set': {
			debug.msg('positionFlags - position ' + position.id + ' ' + flags + '= ' + position[flags] + ' -> (Set ' + value + ') -> ' + (position[flags] | s.strategyFlag[value]))
			position[flags] = (position[flags] | s.strategyFlag[value])
			return cb()
			break
		}
		case 'Unset': {
			debug.msg('positionFlags - position ' + position.id + ' ' + flags + '= ' + position[flags] + ' -> (Unset ' + value + ') -> ' + (position[flags] & ~s.strategyFlag[value]))
			position[flags] = (position[flags] & ~s.strategyFlag[value])
			return cb()
			break
		}
		case 'Free': {
			debug.msg('positionFlags - position ' + position.id + ' ' + flags + '= ' + position[flags] + ' -> (Free) -> ' + s.strategyFlag.free)
