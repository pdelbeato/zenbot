...
module.exports = function (s, conf) {
	...
	s.eventBus.on('manual', (signal, position_id, fixed_size, fixed_price, protectionFree, is_reorder, is_taker = false) => {
		...
		executeSignal (signal, 'manual', position_id, fixed_size, fixed_price, protectionFree, false, is_taker)
	})
...
//				debug.msg('functionStrategies - callbackFinal()')
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		...
		s.eventBus.on(strategy_name, (signal, position_id= null, fixed_size= null, fixed_price= null, protectionFree = 0, is_reorder= false, is_taker= false) => {
			...
			executeSignal (signal, strategy_name, position_id, fixed_size, fixed_price, protectionFree, is_reorder, is_taker)
		})
...
//					debug.msg('syncBalance - balance= ' + JSON.stringify(s.balance) + ' ; available_balance= ' + JSON.stringify(s.available_balance))
	...
	function executeSignal (signal, sig_kind = 'manual', position_id = null, fixed_size = null, fixed_price = null, protectionFree = 0, is_reorder, is_taker, _cb) {
...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			let order_tmp_catch = s.tools.orderExist(undefined, 'catching_orders', position_id)
				...
				if (Number(tradeable_balance) > Number(available_balance_currency)) {
					...
					return setTimeout(function () {
						if (s.last_signal === signal) {
							s.hold_signal = true
							executeSignal(signal, sig_kind, position_id, protectionFree, undefined, undefined, true)
						}
...
// Dopo il primo riordine, order.size diventa l'attuale (ovvero il remaining_size), quindi se viene cancellato, il seguente if non viene eseguito 							
							...
							return executeSignal(signal, sig_kind, position_id, protectionFree, null, null, true, is_taker, _cb)
						}
						cb(null, order)
					})
				}, so.order_poll_time)
...
//					orderDelete(undefined, undefined, position.id, function() {
					s.tools.positionFlags(position, 'status', 'Free', undefined, function() {
						...
						//Cancello la posizione dal db delle posizioni aperte e inserisco la posizione chiusa del db delle posizioni chiuse
						if (so.mode != 'sim') {
							s.positionProcessingQueue.push({mode: 'delete', id: position.id})
						}
						else {
							//Elimino la posizione da s.positions
