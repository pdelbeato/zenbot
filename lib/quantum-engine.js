...
// s.balance.asset e s.balance.currency restituiscono i fondi totali. s.balance.asset_hold e s.balance.currency_hold sono quelli on hold.
			switch (signal) {
				case 'buy': {
					if (tradeable_balance > available_balance_currency) {
						let err = new Error('\nInsufficient funds')
					...
					else if (n(available_balance_currency).subtract(s.balance.currency_hold || 0).value() < n(order_tmp.price).multiply(order_tmp.size).value() && s.balance.currency_hold > 0) {
						...
						debug.msg('executeSignal - available_balance_currency ' + available_balance_currency + ' ; s.balance.currency_hold ' + s.balance.currency_hold)
				...
				case 'sell': {
					if (order_tmp.size > available_balance_asset) {
						let err = new Error('\nInsufficient funds')
					...
					else if (n(available_balance_asset).subtract(s.balance.asset_hold || 0).value() < n(order_tmp.size).value()) {
						debug.msg('executeSignal - sell delayed: ' + n(available_balance_asset).subtract(s.balance.asset_hold || 0).value() + ' of funds on hold')
						debug.msg('executeSignal - available_balance_asset ' + available_balance_asset + ' ; s.balance.asset_hold ' + s.balance.asset_hold)
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_kind, position_id)
