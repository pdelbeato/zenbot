...
module.exports = function (s, conf) {
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		...
		s.orders[orderSignal][orderType][orderId] = {
			...
			fixed_price: null,
			size: (s.positions[orderId] ? s.positions[orderId].size : null),
			fee: 0,
			orig_size: (s.positions[orderId] ? s.positions[orderId].orig_size : null),
			remaining_size: (s.positions[orderId] ? s.positions[orderId].remaining_size : null),
			filled_size: 0,
			orig_price: null,
	...
	function executeStop () {
		...
		s.positions.forEach( function (position, index) {
			...
			if (!s.acted_on_trail_stop && position.profit_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
				...
				if (!ordersExist(position_opposite_signal, 'TrailStop', position.id))
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		if (sig_id) {
			...
			//Se non esiste l'ordine associato a [signal][sig_type][sig_id], allora lo devo creare
				...
				if (s.positions[sig_id] && s.positions[sig_id].side == signal) {
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			s.orders[signal][sig_type][sig_id].size = n(fixed_size || s.orders[signal][sig_type][sig_id].remaining_size || n(so.quantum_size).divide(s.orders[signal][sig_type][sig_id].price)).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
			if (!s.orders[signal][sig_type][sig_id].filled_size)
				s.orders[signal][sig_type][sig_id].orig_size = s.orders[signal][sig_type][sig_id].size
			
			if (!s.orders[signal][sig_type][sig_id].filled_size)
				s.orders[signal][sig_type][sig_id].remaining_size = s.orders[signal][sig_type][sig_id].size
				
			debug.msg('executeSignal - Size: ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + ' ; orig_size= ' + s.orders[signal][sig_type][sig_id].orig_size)
...
//				debug.printPosition(s.positions)
			...
			//Archivio la posizione in s.positions
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			position.profit_pct = (position.side == 'buy' ? +1 : -1) * (trade.price - position.cost) / position.cost
