...
//		getQuote(function (err) {
			...
			if (so.use_fee_asset) {
				...
				order_tmp.fee_pct = s.exchange.makerFee
			} else {
				order_tmp.fee_pct = s.exchange.takerFee
			}		

			//Controllo fondi disponibili
			trade_balance = n(order_tmp.size).multiply(order_tmp.price)
			tradeable_balance = n(trade_balance).multiply(100).divide(100 + order_tmp.fee_pct)
...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_kind + ' ' + position_id + ' order over ' + formatAsset(order_tmp.size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance with a expected fee of ' + formatCurrency(expected_fee, s.currency) + ' (' + order_tmp.fee_pct + '%)')
...
//					order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
					order_tmp.fee = n(order_tmp.executed_value).multiply(order_tmp.fee_pct).divide(100).add(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
					...
					if (order_tmp.position.side == signal) {
						...
						order_tmp.position.fee_open = n(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
						...
						order_tmp.position.fee_close = n(order_tmp.fee).format(s.product.increment ? s.product.increment : '0.00000000')
					}
					...
					debug.msg('order_tmp.position.value= ' + order_tmp.position.value + ' ; order_tmp.position.size= ' + order_tmp.position.size + ' ; order_tmp.fee= ' + order_tmp.fee)
					debug.printPosition(order_tmp)				
...
//								orderDelete(signal, sig_kind, position_id)
	...
	function executeOrder(signal, sig_kind, position_id, is_partial = false) {
		...
		let my_trade = {
			...
			value: n(order_tmp.executed_value).format('0.00'),
			order_type: so.order_type || 'taker',
			cancel_after: so.cancel_after, // || null //'day'
			//Tutto ciò che riguarda position va tolto appena quantum-sim è aggiornato con i valori di s.closed_positions
...
//			position_price_open: order_tmp.position.price_open,
//			profit: order_tmp.position.profit_net_pct,
...
//			positionStatus(position, 'Unset', sig_kind)
		...
		updatePositions(order_tmp, true, function() {
			...
			if (so.stats) {
				...
				//Debug per le fee
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					debug.msg('executeOrder - getOrder:')
					console.log((api_order).yellow)
				})
			}
