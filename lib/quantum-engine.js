...
//        pushMessage('Price limit protection', 'aborting')
        ...
        //Controllo se è un riordine o un nuovo ordine
    ...
    if (!s.in_preroll) {
      ...
      if (!so.manual) {
        ...
        if (s.signal) {
	      ...
	      if (moment() > (s.last_executeSignal + 1000)) {
	    	debug.msg('withOnPeriod - chiamo executeSignal ' + s.signal + ' Time= ' + moment())
	    	s.last_executeSignal = moment()
	    	executeSignal(s.signal)
	      } else {
	    	debug.msg('withOnPeriod - non chiamo executeSignal: ' + moment() + ' < ' + s.last_executeSignal)
	      }
          //Fine tentativo
...
//        	executeSignal(s.signal)
        	...
        	if (moment() > (s.last_executeSignal + 1000)) {
        		...
        		debug.msg('onTrade - non chiamo executeSignal: ' + moment() + ' < ' + s.last_executeSignal)
        	}
        	//Fine tentativo
          }
        }
