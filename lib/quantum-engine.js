    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if (is_reorder) {
          ...
          debug.msg('Remaining size: ' + formatAsset(s[signal + '_order'].remaining_size, s.asset))
        } else {
...
//          executeSignal(s.signal)
        	...
        	// Con setTimeout, executeSignal viene eseguita dopo 1 secondo. Si presuppone che la precedente,
        	// quindi, abbia il tempo di chiamare placeOrder e creare una istanza di s[___order] per bloccare
        	// le seguenti chiamate.
        	setTimeout(executeSignal(s.signal),1000)
        	s.signal = null
        }
      }
    }
  ...
  function updateMessage() {
      ...
      output_lines.push('\n' + s.my_positions.length + ' positions opened.')
