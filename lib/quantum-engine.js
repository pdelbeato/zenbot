		...
		switch (signal) {
			case 'buy': {
				...
				if (n(npfq).subtract(s.quote.ask).value() > 0) {
					debug.msg('nextPriceForQuote - npfq > ask. Correcting...')
			...
			case 'sell': {
				...
				if (n(npfq).subtract(s.quote.ask).value() < 0) {
					debug.msg('nextPriceForQuote - npfq < bid. Correcting...')
...
//			Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
			...
			case 'sell': {
				if (Number(order_tmp.size) > Number(available_balance_asset)) {
					...
					s.hold_signal = false
					pushMessage('Selling (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(order_tmp.price - s.quote.ask).format(s.product.increment ? s.product.increment : '0.00000000'), s.currency) + ' over best ask\n', 9)
