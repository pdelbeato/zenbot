...
//        }
  ...
  function updateMessage() {
	  var output_lines = []
      output_lines.push('Balance ' + s.exchange.name.toUpperCase(), 'Virtual ' + formatCurrency(s.real_capital, s.currency))
      output_lines.push(' (' + formatCurrency(s.balance.currency, s.currency) + ' ' + formatAsset(s.balance.asset, s.asset) + ')')
      output_lines.push(s.my_trades.length + ' trades over ' + s.day_count + ' days (avg ' + n(s.my_trades.length / s.day_count).format('0.00') + ' trades/day)')
      output_lines.push(s.my_positions.length + ' positions opened.')
	  pushMessage(output_lines)
  }
