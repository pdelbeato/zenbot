...
module.exports = function (s, conf) {
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		...
		if (s.positions[orderId])
			debug.msg('initOrder - Copio i valori dalla posizione ' + orderId)
		
		s.orders[orderSignal][orderType][orderId] = {
			id: orderId,
			selector: so.selector.normalized,
			status: (s.positions[orderId] ? 'copied' : 'created'),
			position_side: (s.positions[orderId] ? s.positions[orderId].position_side : orderSignal), //Side della posizione
			position_price: (s.positions[orderId] ? s.positions[orderId].position_price : null), //Prezzo medio della posizione
			...
			position_time: (s.positions[orderId] ? s.positions[orderId].position_time : 0), //Time apertura della posizione
			signal: orderSignal,	
			time: null,
			...
			orig_size: (s.positions[orderId] ? s.positions[orderId].position_size : null), //Size inizio trade (serve per i riordini)
			...
			sell_stop: (s.positions[orderId] ? s.positions[orderId].sell_stop : null),
			buy_stop: (s.positions[orderId] ? s.positions[orderId].buy_stop : null),
		...
		debug.msg('initOrder - ordine creato:')
	...
	function executeStop () {
		...
		s.positions.forEach( function (position, index) {
			...
			if (!s.acted_on_stop && position_stop && ((position.position_side == 'buy' ? +1 : -1) * (s.period.close - position_stop) < 0)) {
				...
				console.log(('\n' + position.position_side.toUpperCase() + ' stop triggered at ' + formatPercent(position.profit_pct) + ' trade profit for position ' + position.id + '\n').red)
				pushMessage('Stop Loss Protection', position.position_side + ' position ' + position.id + ' (' + formatPercent(position.profit_pct) + ')', 0)
			...
			if (!s.acted_on_trail_stop && position.profit_stop && ((position.position_side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
				...
				pushMessage('Trailing stop', position.position_side + ' position ' + position.id + ' (' + formatPercent(position.profit_pct) + ')', 0)
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		//Se esiste sig_id, allora è un ordine specifico su sig_id
		if (sig_id) {
			//Se esiste l'ordine associato, allora è un riordine, quindi proseguo con quello
			if (orderExist(signal, sig_type, sig_id)) {
				debug.msg('executeSignal - con sig_id (' + sig_type + ' ' + sig_id + ') - ordine già esistente')
			}
			//Se non esiste l'ordine associato a [signal][sig_type][sig_id], allora lo devo creare
			else {
				//Se contemporaneamente c'è un ordine standard aperto su questa posizione, annullo questo executeSignal
				// Non ripristino il listener, perchè deve essere ripristinato dall'ordine standard quando si chiuderà
...
//					if (s.positions[sig_id].side == signal) {
//						debug.msg('executeSignal - con sig_id. Annullo, perchè signal ' + signal + ' non coerente con la posizione (side = ' + s.positions[sig_id].side + ')')
//						_cb && _cb(null, null)
//						return
//					}
//					//Creo l'ordine a partire dai valori della posizione sig_id
//					else {
//						s.orders[signal][sig_type][sig_id] = s.posiitons[sig_id]
//					}
//				}
//				//Non esiste una posizione sig_id, quindi...
//				else {
				//Creo l'ordine, prendendo i valori di interesse dalla posizione sig_id
				debug.msg('executeSignal - con sig_id (' + sig_type + ' ' + sig_id + ') - Creo nuovo ordine.')
			...
			if (sig_type == 'standard') {			
				...
				if (s.max_profit_position.trend[signal_opposite]) {
					...
					getQuote(function (err) {
						...
						if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
							debug.msg('executeSignal - standard senza sig_id - Non ci sono posizioni in profitto (max ' + n(position.profit_pct*100).format('0.00%') + ')')
							//no operation
						}
						//La posizione individuata ha profitto sufficiente, quindi cancello l'ordine creato e
						// ne creo un ordine standard associato a quella posizione
						else {
							debug.msg('executeSignal - standard senza sig_id - Prendo max_profit_position.trend.' + signal_opposite + '.id ' + position.id + ' con profitto (' + position.profit_pct + ')')
							delete s.orders[signal][sig_type][sig_id]
...
//					let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
					...
					return executeSignal(signal, sig_type, sig_id)
				}
				cb(null, order)
			})
		}
...
//			cancel_after: s.cancel_after // s.cancel_after || 'day'
			...
			debug.msg('executeOrder - prima di cancellare order -> length ' + s.positions.length)
			delete s.orders[signal][sig_type][sig_id]
			debug.msg('executeOrder - dopo aver cancellato order -> length ' + s.positions.length)
		}
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			position.profit_pct = (position.position_side == 'buy' ? +1 : -1) * (trade.price - position.position_price) / position.position_price
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function updateMessage() {
		...
		output_lines.push('\n' + s.positions.length + ' positions opened' + (side_trend_max_profit ? (' (' + side_trend_max_profit[0] + formatPercent(pct_trend_max_profit) + ').') : '.'))
