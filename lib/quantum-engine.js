...
module.exports = function (s, conf) {
	...
	if (so.catch_order_pct > 0) {
		eventBus.on('orderExecuted', (signal, position_id) => {
			...
			if (position && position.status === 'free') {
				...
				executeSignal(signal_opposite, 'catch', position_id, null, target_price)  
			}
		})
	}
...
//			orig_size: null, //Size inizio trade (serve per i riordini)
		...
		if (position) {
			...
			order.position.status = orderType
		}
		else {
			...
			order.position.status = orderType + ' new'
		}
			...
			getQuote(function (err) {
				...
				s.positions.forEach(function (position, index) {
					if (position.side === 'buy')
						s.available_balance.asset -= n(position.size).format('0.00000000')
					else
						s.available_balance.currency -= n(position.value).format(s.product.increment)	
				})
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		if (is_reorder) {
			...
			if (order_tmp) {
				...
				order_tmp.position.status = sig_type
			}
				...
				if (position_id) {
					...
					if (position) {
						...
						//Creo l'ordine a partire dai valori della posizione position_id
							...
							debug.msg('executeSignal - con position_id. Inizializzo una posizione ' + signal + ' ' + sig_type + ' ' + position_id)
...
//								debug.msg('executeSignal - Ordine standard ' + position_id + '. Cancello TUTTI gli ordini connessi con la posizione ancora in essere.')
//								orderSetStatus(undefined, undefined, position_id, 'canceled', function() {
									order_tmp = initOrder(signal, 'standard', position_id, is_taker)
//								})
							}
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			if (!is_reorder && !s.acted_on_stop && !fixed_price && order_tmp.position.profit_pct && so['max_' + signal + '_loss_pct'] != null && (order_tmp.position.profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
			...
			order_tmp.size = Number(n(fixed_size || order_tmp.remaining_size || order_tmp.position.size || n(so.quantum_size).divide(order_tmp.price)).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000'))
			...
			switch (signal) {
				case 'buy': {
					if (tradeable_balance >= s.available_balance.currency) {
						let err = new Error('\nInsufficient funds')
					...
					else if (n(s.available_balance.currency).subtract(s.balance.currency_hold || 0).value() < n(order_tmp.price).multiply(order_tmp.size).value() && s.balance.currency_hold > 0) {
						debug.msg('executeSignal - buy delayed: ' + n(s.available_balance.currency).subtract(s.balance.currency_hold || 0).value() + ' of funds on hold')
						debug.msg('executeSignal - s.available_balance.currency ' + s.available_balance.currency + ' ; s.balance.currency_hold ' + s.balance.currency_hold)
				...
				case 'sell': {
					if (order_tmp.size >= s.available_balance.asset) {
						let err = new Error('\nInsufficient funds')
					...
					else if (n(s.available_balance.asset).subtract(s.balance.asset_hold || 0).value() < n(order_tmp.size).value()) {
						debug.msg('executeSignal - sell delayed: ' + n(s.available_balance.asset).subtract(s.balance.asset_hold || 0).value() + ' of funds on hold')
						debug.msg('executeSignal - s.available_balance.asset ' + s.available_balance.asset + ' ; s.balance.asset_hold ' + s.balance.asset_hold)
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_type, position_id)
		...
		function doOrder () {
			...
			debug.msg('executeSignal - doOrder - Ordine ' + signal + ' ' + sig_type + ' ' + position_id + '. Cancello TUTTI gli ordini connessi con la posizione ancora in essere.')
			orderSetStatus(undefined, undefined, position_id, 'canceled', order_tmp, function() {
...
//						let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
		...
		function placeOrder (cb) {
			...
			s.exchange[signal](order_copy, function (err, api_order) {
				...
				debug.msg('placeOrder - ' + signal + ' ' + sig_type + ' ' + position_id + ' order (id ' + api_order.id + ') placed at ' + formatCurrency(order_tmp.price, s.currency))
		...
		s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					order_tmp.executed_value = order_tmp.executed_value + api_order.executed_value
					order_tmp.remaining_size = order_tmp.size - api_order.filled_size //).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
...
//						order_tmp.position.size = n(order_tmp.position.size).add(api_order.filled_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
						order_tmp.position.value = order_tmp.position.value + api_order.executed_value
						order_tmp.position.size = order_tmp.position.size + api_order.filled_size
					}
					else {
						debug.msg('checkOrder - getOrder - ' + position_id + ' side != signal')
						order_tmp.position.value = order_tmp.position.value - api_order.executed_value
					...
					debug.msg('checkOrder -  getOrder - ' + position_id + ' (order_id ' + order_tmp.order_id + ') done')
					debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size)
				...
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					...
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							order_tmp.filled_size = api_order.filled_size
							order_tmp.executed_value = order_tmp.executed_value + api_order.executed_value
							order_tmp.remaining_size = order_tmp.size - api_order.filled_size
							if (order_tmp.position.side == signal) {
							...
							debug.msg('cancelOrder - getOrder - ' + position_id + ' (order_id ' + order_tmp.order_id + ') cancel failed - order done or partially done')
							debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size)
							...
							if (!do_reorder || !((s.product.min_size && (order_tmp.remaining_size >= Number(s.product.min_size))) || (s.product.min_total && n(order_tmp.remaining_size).multiply(order_tmp.price).value() >= Number(s.product.min_total)))) {
								...
								executeOrder(signal, sig_type, position_id, true)
							}
						}
					}
					//L'ordine non esiste sull'exchange
					else {
						orderDelete(signal, sig_type, position_id)
					}
...
//************** Da controllare questo punto.						
	...
	function executeOrder (signal, sig_type, position_id, is_partial = false) {
		order_tmp = s.orders.find(x => (x.signal === signal && x.type === sig_type && x.id === position_id))
...
//			s.orders[signal_id] = trade
		...
		if (order_tmp.position.id != null) {
			...
			if (order_tmp.position.size != 0 && (s.product.min_size ? (order_tmp.position.size > Number(s.product.min_size)) : true) && (s.product.min_total ? (n(order_tmp.position.size).multiply(order_tmp.price).value() > Number(s.product.min_total)) : true)) {
//			if (order_tmp.position.size != 0 && (s.product.min_size ? (n(order_tmp.position.size).value() > s.product.min_size) : true)) {
				...
				orderSetStatus(undefined, undefined, position_id, 'canceled', order_tmp)
...
//			position._id = position_id
		...
		if (so.stats) {
			let order_complete = '\n**** ' + signal.toUpperCase() + (is_partial ? ' partial' : '') + ' order completed at ' + moment(my_trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n'
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderSetStatus(signal, sig_type, position_id, status, except_order, cb = function() {}) {
		except_index = s.orders.findIndex(x => x === except_order)
		
		s.orders.forEach(function (order, index) {
			if (index != except_index && (signal ? order.signal === signal : true) && (sig_type ? order.type === sig_type : true) && (position_id ? order.id === position_id : true)) {
