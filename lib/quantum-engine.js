...
//		debug.msg('writeReport ' + strategy_name)
		...
		if (orderExist('buy', strategy_name)) {
			process.stdout.write(z(11, 'B ' + strategy_name, ' ').green)
		}
		else if (orderExist('sell', strategy_name)) {
			process.stdout.write(z(11, 'S ' + strategy_name, ' ').red)
		}
		else if (s.action) {
			process.stdout.write(z(11, s.action, ' ')[(s.action === 'bought' || s.action === 'part buy') ? 'green' : 'red'])
		}
		else if (s.signal) {
			process.stdout.write(z(11, s.signal, ' ')[s.signal === ('pump' || 'dump') ? 'white' : s.signal === 'buy' ? 'green' : 'red'])
		}
		else if (s.is_dump_watchdog || s.is_pump_watchdog) {
			process.stdout.write(z(11, 'P/D Calm', ' ').white)
		}
		else {
			process.stdout.write(z(11, '', ' '))
		}
...
//			process.stdout.write(z((currency_col.length + 1), currency_col, ' ').green)
			process.stdout.write(currency_col.length.green)
