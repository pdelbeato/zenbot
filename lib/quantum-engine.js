...
//		}
		...
		await updateOnePosition(order_tmp, order_tmp.position)
		...
		if (order_tmp.position.id != null) {
			...
			//Se la posizione ha size nullo o comunque inferiore al minimo commerciabile
				...
				// debug.obj('Posizione chiusa:', s.positions[position_index])
				debug.obj('Posizione chiusa:', s.positions[position_index])

				//Chiamata alla funzione onPositionClosed per le strategie
				var opts = {
					position_id: position_id,
				}
				s.tools.functionStrategies('onPositionClosed', opts
					, null //callbackStrategy
					, async function () { //callbackFinal
						...
						if (so.mode != 'sim') {
							opts.mode = 'delete';
							await s.positionProcessingQueue.push(opts)
						}

						//...e la inserisco nel db delle posizioni chiuse
						s.db_my_closed_positions.updateOne({ '_id': position_id }, { $set: position }, { multi: false, upsert: true }, function (err) {
							if (err) {
...
//		       			if (so.mode !== 'sim' && s.lookback.length > so.min_periods) {
	...
	async function updateOnePosition(trade, position) {
