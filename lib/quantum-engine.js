	...
	if (so.catch_order_pct > 0) {
		eventBus.on('orderExecuted', (signal, position_id) => {
			...
			if (position && position.status === 'free') {
				...
				target_price = position.price + ((signal === 'buy' ? +1 : -1) * position.price * so.catch_order_pct/100)
				debug.msg('Listener -> catch ' + signal_opposite + ' ' + position_id + ' at ' + formatCurrency(target_price, s.currency))
