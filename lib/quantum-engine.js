...
//				else {
		...
		//Se sig_id non esiste, allora è un ordine non specifico
			...
			if (sig_type == 'standard') {			
				...
				if (s.orders[signal] && s.orders[signal]['standard'].length != 0) {
		...
		s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					debug.msg('checkOrder - getOrder - done - api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.cost= ' + s.orders[signal][sig_type][sig_id].cost)
				...
				s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.cost= ' + s.orders[signal][sig_type][sig_id].cost, false)
