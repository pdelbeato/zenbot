	...
	function executeSignal (signal, sig_kind = 'standard', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		...
		if (is_reorder) {
			...
			if (order_tmp) {
				...
				//Lo status verrà configurato quando l'ordine è immesso sull'exchange
//				orderStatus(order, undefined, undefined, undefined, 'Set', sig_kind)
			}
...
// s.balance.asset e s.balance.currency restituiscono i fondi totali. s.balance.asset_hold e s.balance.currency_hold sono quelli on hold.
		...
		function doOrder () {
			...
			orderStatus(undefined, undefined, undefined, position_id, 'Free', undefined, function() {
...
//				debug.msg('placeOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + ' - now() ' + now() + ' ; s.next_order ' + s.next_order)
				...
				debug.msg('placeOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + '. Attendo...')
...
//								order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
			...
			debug.msg('checkOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + '. Attendo...')
...
//							************** Da controllare questo punto.		
			...
			debug.msg('cancelOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + '. Attendo...')
...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function positionStatus(position, mode, status, cb = function() {}) {
		...
		case 'Set': {
			...
			debug.msg('positionStatus - position ' + position.id + ' Set ' + statusByte[status] + ' -> status=' + position.status)
			return cb()
			break
		}
		case 'Unset': {
			position.status = (position.status & ~statusByte[status])
			debug.msg('positionStatus - position ' + position.id + ' Unset ' + statusByte[status] + ' -> status=' + position.status)
			return cb()
			break
		}
		case 'Free': {
			position.status = statusByte.free
			debug.msg('positionStatus - position ' + position.id + ' Free ' + statusByte.free + ' -> status=' + position.status)
			return cb()
			break
		}
		case 'Check': {
