	...
	function executeSignal (signal, sig_kind = 'standard', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		...
		if (is_reorder) {
			...
			if (order_tmp) {
				...
				orderStatus(order, undefined, undefined, undefined, 'Set', statusByte[sig_kind])
			}
...
//		debug.msg('checkOrder')
		...
		if (!positionStatus(order_tmp.position, 'Check', statusByte[sig_kind])) {
...
//			position._id = position_id
			...
			positionStatus(position, 'Unset', statusByte[sig_kind])
	...
	function orderDelete(signal, sig_kind, position_id) {
		s.orders.forEach(function (order, index) {
			if ((signal ? order.signal === signal : true) && (sig_kind ? order.kind === sig_kind : true) && (position_id ? order.id === position_id : true)) {
				...
				if (sig_kind) {
					positionStatus(order.position, 'Unset', statusByte[sig_kind])
				}
				else {
