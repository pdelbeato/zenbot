...
//	quantumTools(s, conf)
	...
	s.eventBus.on('manual', (signal, position_id, fixed_size, fixed_price, protectionFree, locking = 'manual', is_reorder = false, is_taker = so.order_type) => {
		debug.msg('Listener -> manual ' + signal + (fixed_size? (' ' + formatAsset(fixed_size, s.asset)) : '') + (fixed_price? (' at ' + formatCurrency(fixed_price, s.currency)) : '') + (is_taker? ' TAKER' : ' MAKER'))
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		...
		s.eventBus.on(strategy_name, (signal, position_id= null, fixed_size= null, fixed_price= null, protectionFree = 0, locking = 'free', is_reorder= false, is_taker = so.order_type) => {
			debug.msg('Listener -> ' + strategy_name + ' ' + signal + (position_id? (' ' + position_id) : '') + (fixed_size? (' size= ' + fixed_size) : '') + (fixed_price? (' price= ' + fixed_price) : '') + (protectionFree? (' protectionFree= ' + protectionFree) : '') + (is_reorder? ' reorder' : '') + (is_taker ? ' TAKER' : ' MAKER'))
...
//			orig_size: null, //Size inizio trade (serve per i riordini)
			...
			funds: 0,
			order_type: is_taker ? 'taker' : 'maker',
			order_id: null,
			order_status: null,
...
//					debug.msg('syncBalance - balance= ' + JSON.stringify(s.balance) + ' ; available_balance= ' + JSON.stringify(s.available_balance))
	...
	function executeSignal (signal, sig_kind = 'manual', position_id = null, fixed_size = null, fixed_price = null, protectionFree = 0, locking = 'free', is_reorder, is_taker = undefined, _cb) {
		...
		if (is_taker === undefined) {
			is_taker = so.order_type
		}
				...
				if (Number(tradeable_balance) > Number(available_balance_currency)) {
					...
					return setTimeout(function () {
						if (s.last_signal === signal) {
							s.hold_signal = true
							executeSignal(signal, sig_kind, position_id, undefined, undefined, protectionFree, locking, true, is_taker)
						}
