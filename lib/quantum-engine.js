...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function positionStatus(position, mode, status, cb = function() {}) {
		switch (mode) {
		case 'Set': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Set ' + statusByte[status] + ') -> ' + (position.status | statusByte[status]))
			position.status = (position.status | statusByte[status])
			return cb()
			break
		}
		case 'Unset': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Unset ' + statusByte[status] + ') -> ' + (position.status & ~statusByte[status]))
			position.status = (position.status & ~statusByte[status])
			return cb()
			break
		}
		case 'Free': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Free ' + statusByte.free + ') -> ' + statusByte.free)
