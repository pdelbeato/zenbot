...
module.exports = function (s, conf) {
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		...
		s.orders[orderSignal][orderType][orderId] = {
			id: orderId,
			side: (s.positions[orderId] ? s.positions[orderId].side : null),
			signal: orderSignal,
			...
			orig_price: null,
			price: null,
			position_price: (s.positions[orderId] ? s.positions[orderId].position_price : null),
			cost: (s.positions[orderId] ? s.positions[orderId].cost : 0),
			fixed_price: null,
			fee: 0,
			size: (s.positions[orderId] ? s.positions[orderId].position_size : null),
			orig_size: (s.positions[orderId] ? s.positions[orderId].position_size : null),
			position_size: (s.positions[orderId] ? s.positions[orderId].position_size : null),
			remaining_size: null,
			filled_size: 0,
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			if (!s.orders[signal][sig_type][sig_id].orig_price)
				s.orders[signal][sig_type][sig_id].orig_price = s.orders[signal][sig_type][sig_id].price
			...
			if (!s.orders[signal][sig_type][sig_id].fixed_price && so.max_slippage_pct != null) {
				slippage = Math.abs(n(s.orders[signal][sig_type][sig_id].orig_price).subtract(s.orders[signal][sig_type][sig_id].price).divide(s.orders[signal][sig_type][sig_id].orig_price).multiply(100).value())
			...
			if (s.orders[signal][sig_type][sig_id].filled_size == 0)
				s.orders[signal][sig_type][sig_id].remaining_size = s.orders[signal][sig_type][sig_id].size
				
			debug.msg('executeSignal - Size: ' + formatAsset(s.orders[signal][sig_type][sig_id].size, s.asset) + ' ; position_size= ' + s.orders[signal][sig_type][sig_id].position_size)
		...
		s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					s.orders[signal][sig_type][sig_id].remaining_size = api_order.remaining_size
					s.orders[signal][sig_type][sig_id].cost += n(api_order.filled_size).multiply(s.orders[signal][sig_type][sig_id].price)
					debug.msg('checkOrder - getOrder - done - order.cost= ' + s.orders[signal][sig_type][sig_id].cost)
				...
				s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done') {
							...
							s.orders[signal][sig_type][sig_id].remaining_size = api_order.remaining_size
							s.orders[signal][sig_type][sig_id].cost += n(api_order.filled_size).multiply(s.orders[signal][sig_type][sig_id].price)
							debug.msg('cancelOrder - cancel failed, order done, executing - s.orders[signal][sig_type][sig_id].cost= ' + s.orders[signal][sig_type][sig_id].cost)
							executeOrder(signal, syg_type, sig_id)
							return syncBalance(function () {
						...
						if (api_order.filled_size) {
							s.orders[signal][sig_type][sig_id].filled_size = api_order.filled_size
							s.orders[signal][sig_type][sig_id].remaining_size = n(s.orders[signal][sig_type][sig_id].orig_size).subtract(api_order.filled_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
							s.orders[signal][sig_type][sig_id].cost += n(api_order.filled_size).multiply(api_order.price)
							debug.msg('cancelOrder - api_order.filled_size= ' + api_order.filled_size + ' ; s.orders[signal][sig_type][sig_id].remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; order.cost= ' + s.orders[signal][sig_type][sig_id].cost)
...
//			s.orders[signal_id] = trade
		...
		let my_trade = {
			...
			slippage: (signal == 'buy' ? 1 : -1) * n(s.orders[signal][sig_type][sig_id].price).subtract(s.orders[signal][sig_type][sig_id].orig_price).divide(s.orders[signal][sig_type][sig_id].orig_price).value(),
			side: signal,
			size: s.orders[signal][sig_type][sig_id].filled_size,
			fee: fee,
			price: n(s.orders[signal][sig_type][sig_id].filled_size).divide (s.orders[signal][sig_type][sig_id].cost).format('0.00'),
			//Al cost manca il fee... da aggiungere una volta capito come calcolarlo
			cost: s.orders[signal][sig_type][sig_id].cost,
			...
			position_span: (s.positions[sig_id] != null ? (moment.duration(s.orders[signal][sig_type][sig_id].time - s.positions[sig_id].time).humanize()) : null),
...
//			order_complete += (s[signal_id + '_order'].status != null ? s[signal_id + '_order'].status : '')
		...
		if (s.positions[sig_id] != null) {
			if (s.positions[sig_id].remaining_size != my_trade.size) {
...
//				debug.printPosition(s.positions)
			...
			s.orders[signal][sig_type][sig_id].side = signal
			s.orders[signal][sig_type][sig_id].status = 'opened'
			s.orders[signal][sig_type][sig_id].buy_stop = (signal == 'buy' ? so.buy_stop_pct && n(s.orders[signal][sig_type][sig_id].price).multiply(1 + so.buy_stop_pct/100).value() : null)
			s.orders[signal][sig_type][sig_id].sell_stop = (signal == 'sell' ? so.sell_stop_pct && n(s.orders[signal][sig_type][sig_id].price).multiply(1 - so.sell_stop_pct/100).value() : null)
			s.orders[signal][sig_type][sig_id].price = n(s.orders[signal][sig_type][sig_id].filled_size).divide(s.orders[signal][sig_type][sig_id].cost).format('0.00')
			s.orders[signal][sig_type][sig_id].size = s.orders[signal][sig_type][sig_id].filled_size
			s.orders[signal][sig_type][sig_id].remaining_size = s.orders[signal][sig_type][sig_id].filled_size
			s.orders[signal][sig_type][sig_id].filled_size = 0
			s.orders[signal][sig_type][sig_id].fixed_price = null
			s.orders[signal][sig_type][sig_id].orig_time: null,
