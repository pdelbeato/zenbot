let tb = require('timebucket')
	...
	, _ = require('lodash')
	//, rsi = require('./rsi')
	, ta_rsi = require('./ta_rsi')
	, async = require('async')
	, lolex = require('@sinonjs/fake-timers')
...
module.exports = function (s, conf) {
	...
	s.tradeProcessingQueue = async.queue(function ({ trade, is_preroll }, callback) {
		if (s.exchange.debug_exchange) {
			debug.obj('tradeProcessingQueue:', trade, false)
		}

		onTrade(trade, is_preroll, callback)
	})

	s.eventBus.on('trade', (trade, callback) => {
		s.tradeProcessingQueue.push({ trade, is_preroll: false }, callback)
	})
	...
	products.forEach(function (product) {
		if (product.asset === s.asset && product.currency === s.currency) {
			...
			s.product.increment = s.tools.removeTrailingZeroes(s.product.increment)
			s.product.asset_increment = s.tools.removeTrailingZeroes(s.product.asset_increment)
			
			debug.obj('s.product:', s.product, false)
		}
	})
	if (!s.product) {
	...
	s.eventBus.on('manual', (signal, position_id, fixed_size, fixed_price, protectionFree, locking = 'manual', is_reorder = false, maker_taker = undefined) => {
		if (maker_taker === undefined) {
			maker_taker = so.order_type
		}
		debug.msg('Listener -> manual ' + signal + (fixed_size? (' ' + formatAsset(fixed_size, s.asset)) + ' ' : ' ') + (maker_taker === 'maker' ? (fixed_price ? ('MAKER at ' + formatCurrency(fixed_price, s.currency)) : 'MAKER') : 'TAKER'))
		executeSignal (signal, 'manual', position_id, fixed_size, fixed_price, protectionFree, locking, false, maker_taker)
	})
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		so.strategy[strategy_name].lib = require(path.resolve(__dirname, `../extensions/quantum_strategies/${strategy_name}/strategy`))
		...
		so.strategy[strategy_name].calc_close_time= 0

		if (so.strategy[strategy_name].lib.init) {
			so.strategy[strategy_name].lib.init(s, function() {
		...
		if (so.strategy[strategy_name].lib.getOptions) {
			...
			// Alla fine avremo 
			so.strategy[strategy_name].lib.getOptions.call(s.ctx, strategy_name)
		}

		//Attiva il listener per gli ordini della strategia
		s.eventBus.on(strategy_name, (signal, position_id= null, fixed_size= null, fixed_price= null, protectionFree = 0, locking = 'free', is_reorder= false, maker_taker = undefined) => {
			if (maker_taker === undefined) {
				maker_taker = so.order_type
			}
			debug.msg('Listener -> ' + strategy_name + ' ' + signal + (position_id? (' ' + position_id) : '') + (fixed_size? (' size= ' + fixed_size) : '') + (fixed_price? (' price= ' + fixed_price) : '') + (protectionFree? (' protectionFree= ' + protectionFree) : '') + (is_reorder? ' reorder ' : ' ') + maker_taker.toUpperCase())
			executeSignal (signal, strategy_name, position_id, fixed_size, fixed_price, protectionFree, locking, is_reorder, maker_taker)
		})
	...
	debug.obj('Fine inizializzazioni strategie - s.strategyFlag:', s.strategyFlag, false)
	debug.obj('s.noHoldCheckStrategy:', s.noHoldCheckStrategy, false)
	...
	//Registra il vecchio periodo in s.lookback (compresi i dati delle strategie), inizializza un periodo e lo aggiorna con i valori del trade
	function initPeriod(trade, callback = function() {}) {
		try {
			if (s.period) {
				...
				Object.keys(so.strategy).forEach(function (strategy_name, index) {
					s.lookback[0].strategy[strategy_name] = {
						data: (so.strategy[strategy_name].data ? JSON.parse(JSON.stringify(so.strategy[strategy_name].data)) : null),
						period: (so.strategy[strategy_name].period ? JSON.parse(JSON.stringify(so.strategy[strategy_name].period)) : null)
					}
				})

				//Registra il vecchio periodo nel db_periods
				if (!so.minimal_db) {
					s.db_periods.updateOne({ '_id': s.lookback[0]._id }, { $set: s.lookback[0] }, { multi: false, upsert: true }, function (err) {
						if (err) {
							console.error('\n' + moment().format('YYYY-MM-DD HH:mm:ss') + ': quantum-engine - onTrade - Error saving db_periods:')
							console.error(err)
						}
						initPeriodGoOn()
					})
			...
			function initPeriodGoOn() {
				s.period = {}
				s.tools.initPeriod(s.period, trade, so.period_length, function() {
					s.period.rsi = (s.lookback[0] ? s.lookback[0].rsi : null)
					callback()
				})
			}
		}
		catch (err) {
			console.log('initPeriod - err= ' + err)
			callback()
		}
	}

	//Aggiorna i dati di s.period e dei periodi di strategia con quelli del trade
	function updatePeriod(trade, callback = function() {}) {
		s.vol_since_last_blink += trade.size

		Object.keys(so.strategy).forEach(function (strategy_name, index) {
			if (so.strategy[strategy_name].period) {
				s.tools.updatePeriod(so.strategy[strategy_name].period, trade)
			}
		})

		s.tools.updatePeriod(s.period, trade, callback)
	}
	...
	function initPosition(position_id = null) {
		position = {
			...
			price_open_with_fees: null, //Prezzo medio apertura posizione, incluse fee
			price_close_with_fees: null, //Prezzo medio chiusura posizione, incluse fee
			...
			profit: null, //Profitto della posizione chiusa
	...
	function initOrder (orderSignal, orderKind = 'manual', position_id = null, locking = 'free', maker_taker = 'maker', cb) {			
		order = {
			id: position_id,
			signal: orderSignal,
			kind: orderKind,
		...
		switch (signal) {
			...
			case 'sell': {
				...
				if (n(npfq).subtract(s.quote.bid).value() < 0) {
					...
					npfq = n(s.quote.bid).add(s.product.increment).format(s.product.increment, Math.ceil)
				}
	...
	function syncBalance(cb = function () { }) {
		if (!s.wait_syncBalance) {
			...
			s.exchange.getBalance({ currency: s.currency, asset: s.asset }, function (err, balance) {
				if (err) {
					console.error('syncBalance - err: ' + err)
					return cb(err)
				}
				s.balance = balance
				s.available_balance = {
				...
				getQuote(function (err) {
					if (err) {
						console.error('syncBalance - getQuote - err: ' + err)
						return cb(err)
					}
					...
					s.orders.forEach(function (order) {
						//Eseguo i calcoli solo se l'ordine è effettivamente sull'exchange, e quindi se ha order_id
						if(order.order_id) {
							...
							if (order.signal === 'buy') {
								...
								//Gli ordini vanno tutti considerati. Poi dentro executeSignal verranno tolti quelli che non interessano il trade
...
//								}
	...
	function executeSignal (signal, sig_kind = 'manual', position_id = null, fixed_size = null, fixed_price = null, protectionFree = 0, locking = 'free', is_reorder, maker_taker = 'maker', _cb) {
		if (s.in_preroll || so.manual) {
			return _cb
		}

		let trade_balance, tradeable_balance
		let available_balance_currency, available_balance_asset
		let order_tmp
		...
		//Se non è un riordine...
			...
			//...altrimenti continuo con i controlli
				...
				if (!(protectionFree & s.protectionFlag['calmdown'])) {
					...
					if (so[signal + '_calmdown'] && ((now() - (so[signal + '_calmdown'] * 60 * 1000)) < s['last_' + signal + '_time'])) {
						let err = new Error('\nBuySell calmdown')
						err.desc = moment().format('YYYY-MM-DD HH:mm:ss') + ' - refusing to ' + signal + '. Calmdown is active! Last trade '
						err.desc += moment(s.last_buy_time).format('YYYY-MM-DD HH:mm:ss') + ' Positions opened: ' + s.positions.length + '\n'
						return cb(err)
					}
				...
				if (position_id) {
					...
					if (position) {
						...
						// La posizione ha profitto sufficiente, quindi creo l'ordine a partire dai valori della posizione position_id
							...
							let title_tmp = ('executeSignal - con position_id. Inizializzo un ordine ' + signal + ' ' + sig_kind + ' su posizione ' + position_id)
							debug.obj(title_tmp, position)
							order_tmp = initOrder(signal, sig_kind, position_id, locking, maker_taker)
						}
					}
				...
				//Se position_id non esiste, allora è un ordine non specifico
					...
					//Se è un ordine di stragegia e ce ne sono altri in piedi, non creo questo
		...
		syncBalance(function (err) {
			...
			available_balance_currency = n(s.available_balance.currency)
			available_balance_asset = n(s.available_balance.asset)

			//Esamino tutti gli ordini e se ce ne sono connessi alla posizione in esame, allora aggiorno available_balance di conseguenza
			// (balance reale meno le quantità bloccate dalle posizioni e dagli ordini aperti più la quantità della posizione in esame, se esiste)
			s.orders.forEach(function (order) {
				if (order.id === order_tmp.id) {
					if (order.signal === 'buy') {
						available_balance_currency = n(available_balance_currency).add(order.position.value)
					}
					else {
						available_balance_asset = n(available_balance_asset).add(order_tmp.position.size)
					}
				}
			})
			
			//Se è un riordine, non considero il valore bloccato dall'ordine originale
			if (is_reorder) {
				if (order_tmp.signal === 'buy') {
					let order_value = n(order_tmp.size).multiply(order_tmp.price).add(order_tmp.expected_fee)
					available_balance_currency = n(available_balance_currency).add(order_value)
				}
				else {
					available_balance_asset = n(available_balance_asset).add(order_tmp.size)
				}
			}			
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + maker_taker.toUpperCase() + ' ' + sig_kind + ' ' + position_id + ' order over ' + formatAsset(order_tmp.size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance. Expected fee ' + (so.use_fee_asset ? '(FEE ASSET) ' : '') + 'of ' + formatCurrency(order_tmp.expected_fee, s.currency) + ' (' + order_tmp.fill_fees_pct + '%)')
					...
					if (Number(tradeable_balance) > Number(available_balance_currency)) {
						...
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_kind, position_id, undefined, undefined, protectionFree, locking, true, maker_taker)
							}
				...
				setTimeout(function () {
					placeOrder(function (err, order) {
						...
						if (!order) {
							...
							return executeSignal(signal, sig_kind, position_id, null, null, protectionFree, locking, true, maker_taker, _cb)
						}
						cb(null, order)
					})
				}, so.order_poll_time)
		...
		function placeOrder(cb) {
			...
			s.exchange[signal](order_copy, function (err, api_order) {
				...
				debug.obj('placeOrder - order:', order_tmp)
				...
				if (api_order.status === 'done') {
					s.exchange.getOrder({ order_id: order_tmp.order_id, product_id: s.product_id }, true, function (err, api_order) {
						...
						if (order_tmp.position.side == signal) {
							...
							let fee_open_tmp = (order_tmp.position.side == 'buy' ? +1 : -1) *  order_tmp.position.fee_open
							order_tmp.position.price_open_with_fees = n(order_tmp.position.value).add(fee_open_tmp).divide(order_tmp.position.size).format(s.product.increment)
						}
						else {
							debug.msg('checkOrder - getOrder - ' + position_id + ' side != signal')
						...
						let title_tmp = ('order_tmp.position.value= ' + order_tmp.position.value + ' ; order_tmp.position.size= ' + order_tmp.position.size + ' ; order_tmp.fill_fees= ' + order_tmp.fill_fees)
						debug.obj(title_tmp, order_tmp)
	...
	function cancelOrder(signal, sig_kind, position_id, do_reorder, cb) {
		...
		if (order_tmp.id != position_id) {
			debug.obj('cancelOrder - order_tmp.id <> position_id !!!!', s.orders)
		}
	...
	async function executeOrder(signal, sig_kind, position_id, is_partial = false) {
		try {
			...
			//Aggiorno la posizione (che al momento è ancora temporanea)
			await updateOnePosition(order_tmp, order_tmp.position, true)
			...
			if (order_tmp.position.id != null) {
				...
				//La posizione ha size non nullo e superiore al minimo commerciabile
				if (order_tmp.position.size != 0 && !isOrderTooSmall(s.product, order_tmp.position.size, order_tmp.price)) {
					...
					let title_tmp = ('executeOrder - Posizione ' + position_id + ' parzialmente modificata. Size attuale ' + formatAsset(order_tmp.position.size, s.asset))
					debug.obj(title_tmp, order_tmp.position)
				...
				//Se la posizione ha size nullo o comunque inferiore al minimo commerciabile
					...
					let position_profit = (order_tmp.position.side == 'buy' ? +1 : -1) * n(order_tmp.position.value_close).subtract(order_tmp.position.value_open).value()
					position_profit = n(position_profit).subtract(order_tmp.position.fee_open).subtract(order_tmp.position.fee_close).value()
					s.total_profit += position_profit
					let fee_close_tmp = (order_tmp.position.side == 'buy' ? -1 : +1) *  order_tmp.position.fee_close
					...
					s.positions[position_index] = {
						...
						closed_by: sig_kind,
						//locked: order_tmp.position.locked,
						...
						// price_open_with_fees: null, //Prezzo medio apertura posizione, incluse fee
						price_close_with_fees: n(order_tmp.position.value_close).add(fee_close_tmp).divide(order_tmp.position.initial_size).format(s.product.increment),
						// size: 0, //Valore asset attuale della posizione
					...
					s.positions[position_index]._id = s.positions[position_index].id
					...
					//Punto da vedere: quali ordini dovrebbero ancora essere connessi con la posizione?
					//Più avanti cancello la posizione, quindi cosa succede agli ordini che avevano ancora questa posizione collegata?
					//Gli ordini connessi alla posizione sono stati cancellati quando è stato eseguito executeSignal
					//(che cancella tutti gli ordini connessi con la posizione)
					s.tools.orderStatus(undefined, undefined, undefined, position_id, 'Free')

					//Se non siamo in sim, cancello la posizione dal db delle posizioni aperte...
					if (so.mode != 'sim') {
						let opts = {
							mode: 'delete',
							position_id: s.positions[position_index].id
						}
						s.positionProcessingQueue.push(opts)
					}

					//... la cancello dall'array delle posizioni aperte, inserendola in position...
					position = s.positions.splice(position_index, 1)[0]

					//... eseguo la chiamata alla funzione onPositionClosed per le strategie...
					var opts = {
						position_id: position_id,
						position: position
					}
					s.tools.functionStrategies('onPositionClosed', opts
						, null //callbackStrategy
						, function (err) { //callbackFinal
							if (err) {
								console.error('executeOrder - onPositionClosed - Error: ' + err)
							}
							//...e la inserisco nel db delle posizioni chiuse
							s.db_my_closed_positions.updateOne({ '_id': position_id }, { $set: position }, { multi: false, upsert: true }
								, function (err) {
									if (err) {
									...
									debug.obj('executeOrder - Posizione ' + position_id + ' chiusa (rimanenti ' + s.positions.length + '):', position)
								})
						})
				}
			}
...
//				position._id = position_id
				...
				debug.obj('executeOrder - posizione ' + position_id + ' aperta.', s.positions[s.positions.length - 1])
			...
			if (so.stats) {
				let order_complete = '\n**** ' + signal.toUpperCase() + ' (' + my_trade.order_type + ')' + (is_partial ? ' partial' : '') + ' (' + sig_kind + ') order completed at ' + moment(my_trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n'
				...
				order_complete += 'Total gross ' + formatCurrency(my_trade.executed_value, s.currency) + '\n'
				order_complete += 'Fee ' + formatCurrency(my_trade.fill_fees, s.currency) + (order_tmp.position.fee_asset ? ' (estimated fee in asset)\n' : '\n')
				...
				order_complete += 'Positions left: ' + s.positions.length
				...
				if (is_closed) {
					...
					order_complete += '\nPosition net profit: ' + formatCurrency(position.profit, s.currency)
			...
			if (!is_partial) {
				...
				//Cancello l'ordine che è stato eseguito, dopodichè eseguo syncBalance, emetto il segnale orderExecuted e chiamo onOrderExecuted per le strategie
				s.tools.orderDelete(signal, sig_kind, position_id, function () {
					syncBalance()
			...
			console.err('executeOrder - error: ' + err)
		}
	}
	...
	function writeReport(is_progress = true, period = s.period, callback = function () {}) {
		//Se siamo in preroll, non stampa i trade interni ad un trade period.
		if (is_progress && (s.in_preroll || so.mode === 'sim' || so.mode === 'train')) {
		...
		else if (is_progress && s.vol_since_last_blink) {
			s.vol_since_last_blink = 0
			setTimeout(function () {
				readline.cursorTo(process.stdout, 0)
				process.stdout.write(moment(period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss')['grey'])
			}, 300)
			setTimeout(function () {
				readline.cursorTo(process.stdout, 0)
				process.stdout.write(moment(period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss')['bgBlue'])
			}, 600)
			setTimeout(function () {
				readline.cursorTo(process.stdout, 0)
				process.stdout.write(moment(period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss')['grey'])
		...
		readline.cursorTo(process.stdout, 0)
		process.stdout.write(moment(is_progress ? period.latest_trade_time : period.time).format('YYYY-MM-DD HH:mm:ss')[is_progress ? 'bgBlue' : 'grey'])
		process.stdout.write(' ' + formatCurrency(period.close, s.currency, true, true, true))// + ' ' + s.product_id.grey)
		if (s.lookback[0]) {
			let diff = (period.close - s.lookback[0].close) / s.lookback[0].close
		...
		let volume_display = (period.volume > 999 ? abbreviate(period.volume, 1) : n(period.volume).format('0.00'))
		volume_display = s.tools.zeroFill(6, volume_display, ' ')
		//			if (volume_display.indexOf('.') === -1) {
		//			volume_display = ' ' + volume_display
		//			}
		//			process.stdout.write(volume_display[is_progress && blink_off ? 'cyan' : 'grey'])
		process.stdout.write(volume_display['cyan'])
		...
		if (period.rsi) {
			...
			if (period.rsi >= 50) {
				stars = Math.min(Math.round(((period.rsi - 50) / 50) * half) + 1, half)
				...
				stars = Math.min(Math.round(((50 - period.rsi) / 50) * half) + 1, half)
				bar += ' '.repeat(half - stars)
			...
			process.stdout.write(s.tools.zeroFill(13, '', ' '))
		}
		
		if (so.active_long_position) {
			process.stdout.write('L'.green)
		}
		else {
			process.stdout.write(' ')
		}

		if (so.active_short_position) {
		...
		s.tools.functionStrategies('onReport', opts
			, null //callbackStrategy
			, function (err) { //callbackFinal
				if (err) {
					console.error('writeReport - onReport - error: ' + err)
					return
				}
				if (s.action) {
...
//				process.stdout.write(s.tools.zeroFill((currency_col.length + 1), currency_col, ' ').green)
	...
	function onTrade(trade, is_preroll, callback) {
		try {
			if (s.exchange.debug_exchange) {
				debug.obj('onTrade', trade, false)
			}
			...
			if (s.period && trade.time < s.period.time) {
				let title_tmp = ('onTrade - Faccio scorrere gli ordini. trade.time= ' + trade.time + ' ; s.period.time= ' + s.period.time)
				debug.obj(title_tmp, trade, false)
			...
			//All'inizio, non esiste un period, quindi ne inizializza uno con i valori del trade in esame
			if (!s.period) {
				initPeriod(trade)

				//Inizializza i period per tutte le strategie (solo se esiste period_calc)
				Object.keys(so.strategy).forEach(function (strategy_name, index) {
					if (so.strategy[strategy_name].opts.period_calc) {
						so.strategy[strategy_name].period = {}
						s.tools.initPeriod(so.strategy[strategy_name].period, trade, so.strategy[strategy_name].opts.period_calc)
					}
				})
			}
			...
			//Memorizza il periodo prima di eventuale archiviazione
			let lastPeriod = JSON.parse(JSON.stringify(s.period))

			if (!s.in_preroll) {
				//Se siamo in sim o in train, non calcola rsi
				if (so.mode === 'sim' || so.mode === 'train') {
					goOn()
				}
				else {
					ta_rsi(s, 'close', undefined, so.rsi_periods)
						.then(function (result) {
							s.period.rsi = lastPeriod.rsi = result
							goOn()
						})
						.catch(function (err) {
							goOn()
				...
				//Aggiorna i valori variabili di tutte le posizioni aperte e chiama onPositionUpdate per ogni posizione
...
//				}
			...
			function goOn() {
				...
				, function (err) {
					if (err) {
						console.error('onTrade - onTrade - error: ' + err)
					}
					//Se il trade è fuori dal period, allora faccio eseguire onTradePeriod alle strategie
					if (trade.time > s.period.close_time) {
						writeReport(false, undefined, function () {
							initPeriod(trade, function () {

								var opts = {
									trade: trade,
									is_preroll: is_preroll
								}
								s.tools.functionStrategies('onTradePeriod', opts
									//Funzione da eseguire al termine di OGNI onTradePeriod di strategia
									, null
									//Funzione da eseguire al termine di TUTTI gli onTradePeriod di strategia
									, function (err) {
										if (err) {
											console.error('onTrade - onTradePeriod - Error: ' +err)
										}
										writeReport(undefined, undefined, callback)

										//  Ripulisce s.lookback a un max di valori
					...
					//Sono dentro il trade period
					else {
						//Aggiorna il periodo
						updatePeriod(trade, function () {
							writeReport(undefined, undefined, callback)
						})
					}
				})
			...
			console.log('onTrade - Catch err= ' + err)
		}
	}
	...
	function onTrades(trades, is_preroll, cb) {
		...
		while ((trade = local_trades.shift()) !== undefined) {
			s.tradeProcessingQueue.push({ trade, is_preroll }, function (err, result) {}) //console.log(result)})
		}
		if (_.isFunction(cb)) {
			cb()
		}
	...
	async function updatePositions(trade, cb = function () { }) {
		s.positions.forEach(async function (position, index) {
			await updateOnePosition(trade, position)
		})

		cb()
	}

	function updateOnePosition(trade, position, is_temp = false) {
		//Serve solo per il primo passagio del db vecchio delle posizioni al db comprendente il prezzo con fees
		if (!position.price_open_with_fees) {
			position.price_open_with_fees = position.price_open
		}
		...
		let price_close_with_fees = n(trade.price).divide(100).multiply(position.side == 'buy' ? (100 - s.exchange.makerFee) : (100 + s.exchange.makerFee))
		position.profit_net_pct = (position.side == 'buy' ? +100 : -100) * n(price_close_with_fees).subtract(position.price_open_with_fees).divide(position.price_open).value()
		...
		if (!is_temp) {
			...
			s.tools.functionStrategies('onPositionUpdated', opts)
		}
	}
	...
	function updateMessage() {
		syncBalance(function (err) {
			...
			s.tools.functionStrategies('onUpdateMessage', undefined
			, function (result, strategy_name, strategy_function, cb) {
				if (!result) {
					return cb()
				}
