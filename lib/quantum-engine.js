...
module.exports = function (s, conf) {
	...
	//Attiva il listener per gli ordini di tipo catch, se l'opzione è attiva
	if (so.catch_order_pct > 0) {
		eventBus.on('orderExecuted', (signal, position_id) => {
			...
			if (position && !positionStatus(position, 'Check', 'catching')) {
				...
				debug.msg('Listener -> catch ' + signal_opposite + ' ' + position_id + ' at ' + target_price)
			...
			switch (signal) {
				case 'buy': {
					if (Number(tradeable_balance) > Number(available_balance_currency)) {
						...
						debug.msg('executeSignal - s.balance.currency ' + s.balance.currency + ' - s.balance.currency_hold ' + s.balance.currency_hold + ' + order_tmp.position.value ' + order_tmp.position.value)
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_kind, position_id)
							}
...
//								order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
						...
						if (order_tmp.status === 'rejected' && (order_tmp.reject_reason === 'post only' || api_order.reject_reason === 'post only')) {
							debug.msg('checkOrder - post-only ' + signal + ' ' + sig_kind + ' ' + position_id + ' failed, re-ordering')
							return cb(null, null)
						}
...
//								order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
								...
								debug.msg('cancelOrder - Ordine esistente su exchange, ma status != done e filled_size = 0')
								if (!do_reorder) {
									debug.msg('cancelOrder - Non è un reorder, quindi unset flag della posizione e cancello order')
