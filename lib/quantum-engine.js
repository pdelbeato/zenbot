		...
		s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					debug.msg('checkOrder - getOrder - done - api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; surplus= ' + s.orders[signal][sig_type][sig_id].position_value)
				...
				s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + s.orders[signal][sig_type][sig_id].remaining_size + ' ; surplus= ' + s.orders[signal][sig_type][sig_id].position_value, false)
...
//			s.orders[signal_id] = trade
		...
		//Se esiste una posizione associata, la identifico insieme al suo index
		position = s.positions.find(x => x.id === sig_id)
		position_index = s.positions.findIndex(x => x.id === sig_id)
		...
		let my_trade = {
			...
			profit: (position != null ? position.profit_pct : null),
			position_span: (position != null ? (moment.duration(s.orders[signal][sig_type][sig_id].time - position.position_time).humanize()) : null),
		...
		if (so.stats) {
			...
			if (position != null) {
				order_complete += '\nOriginal price: ' + formatCurrency(position.position_price, s.currency)
...
//				s.positions[sig_id].remaining_size -= my_trade.size
				debug.msg('executeOrder - posizione ' + sig_id + ' non chiusa completamente. Rimangono ' + formatAsset(position.remaining_size, s.asset))
				debug.printPosition(position)
				...
				debug.msg('executeOrder - delete s.position ' + sig_id + '(index ' + position_index + ')')
