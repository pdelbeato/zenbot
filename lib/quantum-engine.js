...
//		stoploss: 8,
	...
	function functionStrategies (s, strategy_function, opts = {}, callbackStrategy = function() {}, callbackFinal = function() {}) {
		...
		Object.keys(s.options.strategy).forEach(function (strategy_name, index, array) {
			if (s.options.strategy[strategy_name].lib[strategy_function]) {
				debug.message('functionStrategies - strategy_name= ' + strategy_name + '; strategy_function= ' + strategy_function)
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		...
		s.orderFlag[strategy_name] = Math.pow(2, (index + 3))
		debug.msg('Inizializzazione strategia - orderFlag[' + strategy_name + ']= ' + s.orderFlag[strategy_name])
		console.log(s.orderFlag)
		...
		if (so.strategy[strategy_name].lib.orderExecuted) {
			s.eventBus.on('orderExecuted', (signal, sig_kind, position_id) => {
				if (sig_kind === strategy_name) {
					so.strategy[strategy_name].lib.orderExecuted(s, signal, position_id)
				}
			})
		}
	})
...
//	}
	...
	function initPosition (position_id = null) {
		position = {
				...
				status: s.orderFlag.free,
				opened_by: s.orderFlag.free,
				closed_by: s.orderFlag.free,
				locked: false,
				side: null,
...
//		orderStatus(order, undefined, undefined, undefined, 'Set', s.orderFlag[orderKind])
...
//		debug.printPosition(order)
	...
	function updatePeriod(trade) {
		...
		Object.keys(so.strategy).forEach(function (strategy_name, index) {
			so.strategy[strategy_name].lib.onTrade(s)
		})
	...
	function executeStop () {
		...
		if (now() > s.next_check) {
			...
			s.positions.forEach( function (position, index) {
				...
				if (position.profit_stop && !position.locked && !positionStatus(position, 'Check', 'trailstop') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_net_pct > 0) {
...
//		debug.msg('withOnPeriod')
		...
		//Aggiorna il period e fa eseguire i calcoli alla strategia (chiamata a calculate, senza inviare segnali di trade) 
		updatePeriod(trade)
...
//		updatePositions(trade)
		...
		if (trade.time > s.period.close_time) {
			...
			functionStrategies (s, 'onTradePeriod', null, function(strategy_name) {
...
//					orderDelete(undefined, undefined, position.id, function() {
					positionStatus(position, 'Free', undefined, function() {
...
//							debug.msg('updatePositions - max_profit_position_id.trail.' + position.side + ' = ' + position.id, false)
					...
					else if (position.profit_net_pct >= max_profit) {
...
//						debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function updateMessage() {
		...
		if (s.max_profit_position.trend.buy != null || s.max_profit_position.trend.sell != null) {
			side_trend_max_profit = ((s.max_profit_position.trend.buy ? s.max_profit_position.trend.buy.profit_net_pct : -100) > (s.max_profit_position.trend.sell ? s.max_profit_position.trend.sell.profit_net_pct : -100) ? 'buy' : 'sell')
			pct_trend_max_profit = s.max_profit_position.trend[side_trend_max_profit].profit_net_pct
		}
		
		if (s.max_profit_position.trail.buy != null || s.max_profit_position.trail.sell != null) {
			side_trail_max_profit = ((s.max_profit_position.trail.buy ? s.max_profit_position.trail.buy.profit_net_pct : -100) > (s.max_profit_position.trail.sell ? s.max_profit_position.trail.sell.profit_net_pct : -100) ? 'buy' : 'sell')
			pct_trail_max_profit = s.max_profit_position.trail[side_trail_max_profit].profit_net_pct
		}
		syncBalance(function() {
			var output_lines = '';
			output_lines += '\n' + (so.active_long_position? 'Long ' : '') + (so.active_short_position ? 'Short ' : '') + 'position'
...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function positionStatus(position, mode, status, cb = function() {}) {
		switch (mode) {
		case 'Set': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Set ' + status + ') -> ' + (position.status | s.orderFlag[status]))
			position.status = (position.status | s.orderFlag[status])
			return cb()
			break
		}
		case 'Unset': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Unset ' + status + ') -> ' + (position.status & ~s.orderFlag[status]))
			position.status = (position.status & ~s.orderFlag[status])
			return cb()
			break
		}
		case 'Free': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Free) -> ' + s.orderFlag.free)
			position.status = s.orderFlag.free
			return cb()
			break
		}
		case 'Check': {
//			debug.msg('positionStatus - position ' + position.id + ' Check ' + status + ' (position.status= ' + position.status + ')')
			return (position.status & s.orderFlag[status])
		}
		}
	}
