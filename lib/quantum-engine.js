...
let clock
let nice_errors = new RegExp(/(slippage protection|ProfitLoss protection|max quantum reached|pumpdump watchdog|BuySell calmdown)/)
...
module.exports = function (s, conf) {
    ...
    s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
      ...
      getQuote(function (err, quote) {
        ...
        if (!s.start_capital) {
          ...
          if (so.mode !== 'sim') {
            pushMessage('Balance ' + s.exchange.name.toUpperCase(), 'Virtual balance ' + formatCurrency(s.real_capital, s.currency)  + ' (' + formatCurrency(s.balance.currency, s.currency) + ' ' + formatAsset(s.balance.asset, s.asset) + ')')
          }
        } else {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    //Controllo se è passato il buy_calmdown
    ...
    if (!is_reorder && s[signal + '_order']) {
      //debug.msg('Not is_reorder && esiste s[signal + "_order"]')
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (is_reorder && s[signal + '_order']) {
        ...
        debug.msg('price changed, resizing order, ' + formatCurrency(s[signal + '_order'].remaining_size, s.asset) + ' remain')
        size = null
      }
      // }
      ...
      if (signal === 'buy') {
        ...
        if (is_reorder) {
          ...
          debug.msg('Remaining size: ' + formatAsset(s[signal + '_order'].remaining_size, s.asset) + ' su ' + formatCurrency(s.balance.deposit, s.currency))
        } else {
          size = n(so.quantum_size).divide(price).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
          debug.msg('Size: ' + formatAsset(size, s.asset) + ' (' + formatCurrency(so.quantum_size, s.currency) + ' su ' + formatCurrency(s.balance.deposit, s.currency) + ')')
        }
        // }
        ...
        if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
          ...
          if (n(s.balance.deposit).subtract(s.balance.currency_hold || 0).value() < n(price).multiply(size).value() && s.balance.currency_hold > 0) {
            ...
            pushMessage('Buying ' + s.exchange.name.toUpperCase(), 'placing buy order at ' + formatCurrency(price, s.currency) + ', ' + formatCurrency(quote.bid - Number(price), s.currency).format('0.00') + ' under best bid\n')
            doOrder()
          }
        }
        else {
        ...
        if (is_reorder) {
          debug.msg('is_reorder')
          size = n(s[signal + '_order'].remaining_size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
        } else if (s.my_positions.length) {
          debug.msg('Not is_reorder: position_max_profit_index ' + position_max_profit_index)
        ...
        if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
          if (s.product.max_size && Number(size) > Number(s.product.max_size)) {
            debug.msg('size = s.product.max_size')
            size = s.product.max_size
          }
          ...
          if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
            debug.msg('sell delayed: ' + formatPercent(n(s.balance.asset_hold || 0).divide(s.balance.asset).value()) + ' of funds (' + formatAsset(s.balance.asset_hold, s.asset) + ') on hold')
            debug.msg('s.balance.asset ' + s.balance.asset + ' s.balance.asset_hold ' + s.balance.asset_hold + ' size ' + size)
            return setTimeout(function () {
              if (s.last_signal === signal) {
                executeSignal(signal, cb, size, true)
              }
            ...
            pushMessage('Selling ' + formatAsset(size, s.asset) + ' on ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + formatCurrency(price, s.currency) + ', ' + formatCurrency(Number(price) - quote.bid, s.currency).format('0.00') + ' over best ask\n')
            //debug.msg('Selling -> doOrder')
            doOrder()
          }
        }
        else {
          debug.msg('size < s.product.min_size.')
      ...
      placeOrder(signal, {
        ...
        if (!order) {
          ...
          if (remaining_size !== size) {
            debug.msg('remaining size: ' + remaining_size + ' of ' + s[signal + '_order'].size)
          }
  ...
  function executeOrder (trade, type) {
    ...
    //Ordine sell
      ...
      let my_trade = {
        ...
        profit: s.my_positions[working_position_index] && (price - s.my_positions[working_position_index].price) / s.my_positions[working_position_index]
      }
      s.my_trades.push(my_trade)
      ...
      debug.msg('delete position ' + working_position_index + ' (lenght attuale ' + s.my_positions.length +')')
      s.working_position_id = s.my_positions[working_position_index].id
      s.my_positions.splice(working_position_index, 1)


      //debug
      debug.msg('Lista posizioni rimaste')
  ...
  function onTrade(trade, is_preroll, cb) {
    ...
    //Se sono ancora nel periodo, esco
