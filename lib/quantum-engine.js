				...
				getQuote(function (err) {
					...
					if (signal === 'buy') {
						if (order_tmp.price < s.quote.bid) {
							debug.msg('checkOrder - ' + n(s.quote.bid).format(s.product.increment) + ' > our ' + order_tmp.price)
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			position.profit_pct = n((position.side == 'buy' ? +100 : -100) * (trade.price - position.price) / position.price).format('0.000%')
