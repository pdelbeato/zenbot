...
//	s.last_executeSignal = 0
	...
	function nextSellForQuote(s, quote) {
		...
		debug.msg('nextSellForQuote - bid=' + quote.ask + ' return=' + )
		return nsfq
	}
	...
	function placeOrder (signal, sig_id, opts, cb) {
		signal_id = signal + sig_id.toString()
	...
	function executeSignal (signal, sig_id, size, price, is_reorder, is_taker, _cb) {
		...
		if (!is_reorder) {
			if (!sig_id) {
				...
				if (s.max_profit_position_id.trend[signal])
					working_position_index = s.my_positions.findIndex(x => x.id == s.max_profit_position_id.trend[signal])
			}
			else {
				let working_position_index = s.my_positions.findIndex(x => x.id == sig_id)
		...
		if (working_position_index)
			let working_position = s.my_positions[working_position_index]
		...
		//Questi controlli solo se sono ordini dettati dalla strategia
		if (!sig_id) {
		...
		syncBalance(function (err, {balance, quote}) {
			...
			// (la posizione con il massimo profitto)
			switch (is_reorder) {
				case true: {	
					if (s[signal_id + '_order']) {
				...
				case false: {
					if (working_position) {
						...
						size = (working_position.size < s.balance.asset) ? n(working_position.size).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000') : s.balance.asset
								debug.msg('executeSignal - not is_reorder: Position: ' + working_position_index + ' Size: ' + formatAsset(size, s.asset) + ' (' + formatCurrency(so.quantum_size, s.currency) + ' su ' + formatCurrency(s.balance.currency, s.currency) + ')')
			...
			switch (signal) {
				...
				case 'sell': {
					...
					if (tradeable_balance >= s.balance.asset) {
						let err = new Error('\nInsufficient funds')
						err.desc = 'refusing to sell ' + formatAsset(tradeable_balance, s.asset) + '. Insufficient funds (' + formatAsset(s.balance.asset, s.asset) + ')'
						return cb(err)
					}
					break
				}
			...
			if (working_position) {
				switch (signal) {
					case 'buy': {
						if (so.max_buy_loss_pct != null && (working_position.profit_pct + so.max_buy_loss_pct > 0) && !s.acted_on_stop) {
							let err = new Error('\nPosition ' + working_position_index + ' ProfitLoss protection')
							if (so.max_buy_loss_pct > 0) {
								err.desc = 'refusing to buy at ' + formatCurrency(price, s.currency) + ', buy LOSS of ' + formatPercent(sell_loss / 100) + ' > ' + formatPercent(so.max_sell_loss_pct / 100) + '\n'
								//pushMessage('Sell LOSS protection', 'aborting')
							}
							else {
								err.desc = 'refusing to buy at ' + formatCurrency(price, s.currency) + ', buy PROFIT of ' + formatPercent(-sell_loss / 100) + ' < ' + formatPercent(-so.max_sell_loss_pct / 100) + '\n'
					...
					case 'sell': {
						if (so.max_sell_loss_pct != null && (working_position.profit_pct + so.max_sell_loss_pct > 0) && !s.acted_on_stop) {
							let err = new Error('\nPosition ' + working_position_index + ' ProfitLoss protection')
...
//				}
				...
				//Controllo slippage
				if (s[signal + '_order'] && so.max_slippage_pct != null) {
					let slippage = abs(n(s[signal + '_order'].orig_price).subtract(price).divide(s[signal + '_order'.orig_price).multiply(100).value())
...
//			if (s.buy_order) {
		...
		let my_trade = {
			...
			profit: (position_index ? ((price - s.my_positions[position_index].price) / s.my_positions[position_index].price) : null),
			position_span: (position_index ? (moment.duration(trade.time - s.my_positions[position_index].time).humanize()) : null)
			cancel_after: so.cancel_after || null //'day'
		}
		...
		if (so.stats) {
			...
			order_complete += 'Positions: ' + s.my_positions.length + '\n'
			if (position_index) {
		...
		if (position_index) {
			if (s.my_position[position_index].size != my_trades.size) {
				s.my_position[position_index].size -= my_trades.size
...
//						if (type === 'buy') on_hold = n(s.balance.deposit).subtract(s.balance.currency_hold || 0).value() < n(order.price).multiply(order.remaining_size).value()
	...
	function checkOrder (order, signal, sig_id, cb) {
		...
		s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (!sig_id && (now() - order.local_time >= so.order_adjust_time)) {
				getQuote(function (err, quote) {
					if (err) {
...
//		debug.msg('updatePositions')
		...
		position_max_profit_index = null
