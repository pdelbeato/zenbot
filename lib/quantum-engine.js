	...
	if (so.catch_order_pct > 0) {
		eventBus.on('orderExecuted', (signal, position_id) => {
			...
			if (position && position.status === 'free') {
				...
				target_price = n(position.price).multiply((signal === 'buy' ? (1 + so.catch_order_pct/100) : (1 - so.catch_order_pct/100))).value()
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			//Controllo profitto della posizione, solo se non sto eseguendo uno stop loss e non è un riordine
			if (!is_reorder && !s.acted_on_stop && order_tmp.position.profit_pct && so['max_' + signal + '_loss_pct'] != null && (order_tmp.position.profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
				...
				getQuote(function (err) {
					...
					if (signal === 'buy') {
						if (order_tmp.price < s.quote.bid) {
							debug.msg('checkOrder - ' + position_id + ' - ' + n(s.quote.bid).format(s.product.increment) + ' > our ' + Number(order_tmp.price))
