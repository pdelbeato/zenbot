...
module.exports = function (s, conf) {
	...
	let available_asset_col_width = 0
	let available_currency_col_width = 0
	s.lookback = []
	s.calc_lookback = []
	s.day_count = 1
	s.my_trades = []
...
//				}
	...
	function executeOrder (trade, signal, sig_id) {
		...
		let my_trade = {
			...
			cost: n(price).multiply(s[signal_id + '_order'].orig_size).value(),
...
//			process.stdout.write(z(9, s.signal || '', ' ')[s.signal ? s.signal === ('pump' || 'dump') ? 'white' : s.signal === 'buy' ? 'green' : 'red' : 'grey'])
		...
		// }
			...
			process.stdout.write(z(8, buysell + buysell_profit, ' ')[buysell_profit > 0 ? 'green' : 'red'])
		}
		else {
