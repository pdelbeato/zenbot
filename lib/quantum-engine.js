...
//Cancella tutti gli ordini prima di aver fatto tutti i controlli, rischiando di cancellare ordini che non c'entravano nulla!!!						
...
//			s.orders[signal_id] = trade
		...
		if (order_tmp.position.id != null) {
			...
			if (order_tmp.position.size != 0 && (s.product.min_size ? n(order_tmp.position.size).value() > s.product.min_size : true)) {
				...
				position.price = n(position.value).divide(position.size).format(s.product.increment)
