...
module.exports = function (s, conf) {
	...
	if (so.catch_order_pct > 0) {
		s.eventBus.on('orderExecuted', (signal, sig_kind, position_id) => {
			...
			if (position && !position.locked && !positionStatus(position, 'Check', 'catching')) {
				...
				target_price = n(position.open_price).multiply((signal === 'buy' ? (1 + so.catch_order_pct/100) : (1 - so.catch_order_pct/100))).format(s.product.increment, Math.floor)
	...
	function initPosition (position_id = null) {
		position = {
				...
				locked: false,
				side: null,
				open_price: null, //Prezzo medio apertura posizione
				close_price: null, //Prezzo medio chiusura posizione
				...
				initial_value: 0, //Valore currency della posizione aperta
...
//					order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
					...
					if (order_tmp.position.side == signal) {
						...
						order_tmp.position.initial_value = order_tmp.position.value
						...
						//Il segnale è lo stesso della posizione: sto accumulando, quindi devo aggiornare il prezzo
						// Se il segnale fosse opposto, allora sto chiudendo la posizione, quindi il prezzo non cambia
						order_tmp.position.open_price = n(order_tmp.position.value).divide(order_tmp.position.size).format(s.product.increment)
					}
					else {
						debug.msg('checkOrder - getOrder - ' + position_id + ' side != signal')
...
//								orderDelete(signal, sig_kind, position_id)
	...
	function executeOrder(signal, sig_kind, position_id, is_partial = false) {
		...
		let my_trade = {
			...
			position_open_price: order_tmp.position.open_price,
...
//				order_tmp.position.price = n(order_tmp.position.value).divide(order_tmp.position.size).format(s.product.increment)
				...
				order_tmp.position.buy_stop = ((order_tmp.position.side == 'sell' && so.buy_stop_pct) ? n(order_tmp.position.open_price).multiply(1 + so.buy_stop_pct/100).format(s.product.increment) : null)
				order_tmp.position.sell_stop = ((order_tmp.position.side == 'buy' && so.sell_stop_pct) ? n(order_tmp.position.open_price).multiply(1 - so.sell_stop_pct/100).format(s.product.increment) : null)
...
//					locked: false,
					...
					open_price: order_tmp.position.open_price, //Prezzo medio apertura posizione
					close_price: n(order_tmp.position.initial_value).subtract(order_tmp.position.value).divide(order_tmp.position.initial_size).format(s.product.increment) //Prezzo medio chiusura posizione
//					size: 0, //Valore asset attuale della posizione
					initial_size: order_tmp.position.initial_size, //Valore asset della posizione aperta
//					value: 0, //Valore currency attuale della posizione
					initial_value: order_tmp.position.initial_value, //Valore currency della posizione aperta
...
//			position.price = n(order_tmp.position.value).divide(order_tmp.position.size).format(s.product.increment)
			...
			position.buy_stop = (position.side == 'sell' ? so.buy_stop_pct && n(position.open_price).multiply(1 + so.buy_stop_pct/100).format(s.product.increment) : null)
			position.sell_stop = (position.side == 'buy' ? so.sell_stop_pct && n(position.open_price).multiply(1 - so.sell_stop_pct/100).format(s.product.increment) : null)
...
//			//Numero massimo di valori per lookback e calc_lookback quando non siamo in sim
//			let max_length = 100
...
//					s.acted_on_trail_stop = false
					...
					if (strategyProcessed === array.length) {
						//Aggiungi il periodo a s.lookback, lo completa con i valori calcolati dalla strategia 
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			...
			if (isOrderTooSmall(s.product, position.size, position.open_price)) {
				let err = new Error('\nMinimum size')
			...
			position.profit_pct = (position.side == 'buy' ? +100 : -100) * n(trade.price).subtract(position.open_price).divide(position.open_price).value()
