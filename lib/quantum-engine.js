...
//					locked: false,
					...
					price_close: n(order_tmp.position.value_open).add(Math.abs(order_tmp.position.value)).divide(order_tmp.position.initial_size).format(s.product.increment), //Prezzo medio chiusura posizione
//					size: 0, //Valore asset attuale della posizione
...
//					value: 0, //Valore currency attuale della posizione
					...
					value_close: n(order_tmp.position.value_open).add(Math.abs(order_tmp.position.value)).format(s.product.increment), //Ricavo currency dalla posizione chiusa
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			...
			if (isOrderTooSmall(s.product, position.size, position.price_open)) {
				let err = new Error('\nMinimum size')
