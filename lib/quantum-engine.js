...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function positionStatus(position, mode, status, cb = function() {}) {
		...
		case 'Set': {
			...
			debug.msg('positionStatus - position ' + position.id + ' Set statusByte= ' + statusByte[status] + ' status=' + position.status)
			return cb()
			break
		}
		case 'Unset': {
			position.status = (position.status & ~statusByte[status])
			debug.msg('positionStatus - position ' + position.id + ' Unset statusByte= ' + statusByte[status] + ' status=' + position.status)
			return cb()
			break
		}
		case 'Free': {
			position.status = statusByte.free
			debug.msg('positionStatus - position ' + position.id + ' Free statusByte= ' + statusByte[status] + ' status=' + position.status)
			return cb()
			break
		}
		case 'Check': {
