			...
			//...altrimenti continuo con i controlli
				...
				//Se sig_id non esiste, allora è un ordine non specifico
					...
					if (sig_type == 'standard') {			
						...
						orderSetStatus(signal_opposite, undefined, sig_id, 'canceled')
					}
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderSetType(signal, sig_type, sig_id, type) {
		s.orders.forEach(function (order, index) {
			if ((signal ? order.signal === signal : true) && (sig_type ? order.type === sig_type : true) && (sig_id ? order.id === sig_id : true)) {
				debug.msg('orderSetType - s.orders(' + order.signal + ', ' + order.type + ', ' + order.id + ').type = ' + type)
				order.type = type
			}
		})
	}
