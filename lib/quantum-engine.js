...
//	}
	...
	if (so.catch_order_pct > 0) {
		s.eventBus.on('orderExecuted', (signal, sig_kind, position_id) => {
			...
			if (position && !position.locked && !tools.positionFlags(s, position, 'status', 'Check', 'catching')) {
...
//			orig_size: null, //Size inizio trade (serve per i riordini)
		...
		if (orderKind === 'manual')
			tools.positionFlags(s, order.position, 'locked', 'Set', 'manual');
...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			let order_tmp_catch = s.tools.orderExist(undefined, 'catching', position_id)
...
//			Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
			...
			case 'buy': {
				if (Number(tradeable_balance) > Number(available_balance_currency)) {
					...
					s.hold_signal = false
					s.tools.pushMessage('Buying (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing ' + signal + ' order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(s.quote.bid).subtract(order_tmp.price).format('0.00'), s.currency) + ' under best bid\n', 9)
...
//		updatePositions(trade)
		...
		if (s.period && trade.time < s.period.time) {
			...
			s.tools.pushMessage('onTrade', 'Sono dentro if misterioso', 9)
...
//		positionFlags: tools.positionFlags,
//		pushMessage: tools.pushMessage,
	}
}
