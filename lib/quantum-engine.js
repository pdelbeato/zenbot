...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			if (s.product.min_size && (order_tmp.size.value() < Number(s.product.min_size) || ('min_total' in s.product && s.product.min_total && n(order_tmp.size).multiply(order_tmp.price).value() < Number(s.product.min_total)))) {
				let err = new Error('\nMinimum size')
				err.desc = 'refusing to ' + signal + ' ' + order_tmp.size.value() + ' ' + s.asset + '. Minimum size not reached'
				return cb(err)
			}
			
			if (s.product.max_size && order_tmp.size.value() > Number(s.product.max_size)) {
		...
		s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (!order_tmp.fixed_price && (now() - order_tmp.local_time >= so.order_adjust_time)) {
				getQuote(function (err) {
					...
					if (signal === 'buy') {
						if (order_tmp.price < n(s.quote.bid).format(s.product.increment)) {
					...
					if (signal === 'sell') {
						if (order_tmp.price > n(s.quote.ask).format(s.product.increment)) {
				...
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							if (!do_reorder || !((s.product.min_size && (order_tmp.remaining_size.value() >= Number(s.product.min_size))) || (s.product.min_total && n(order_tmp.remaining_size).multiply(order_tmp.price).value() >= Number(s.product.min_total)))) {
								debug.msg('cancelOrder - getOrder - not do_reorder || order done || remaining_size < minimo ordine possibile')
								executeOrder(signal, sig_type, position_id)
								return syncBalance(function () {
									cb(null, true)
								})
								...
								debug.msg('cancelOrder - getOrder -> executeOrder parziale')
...
//			s.orders[signal_id] = trade
		...
		let my_trade = {
			...
			value: n(order_tmp.position.value).format(s.product.increment),
		...
		if (order_tmp.position.id != null) {
			...
			if (order_tmp.position.size != 0 && (s.product.min_size ? order_tmp.position.size.value() > s.product.min_size : true)) {
				debug.msg('executeOrder - Posizione ' + position_id + ' parzialmente modificata. Rimangono ' + formatAsset(order_tmp.position.size, s.asset))
