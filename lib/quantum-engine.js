...
//		debug.printPosition(order)
	...
	var wait_syncBalance = false
	
	function waitCondition (condition, interval, cb) {
		debug.msg('waitCondition - condition= ' + condition + '. Waiting...')
		if (condition) {
			setTimeout (function() { waitCondition(condition, interval, cb) }, interval)
		}
		else {
			debug.msg('waitCondition - condition= ' + condition + '. Continuing...')
			cb()
		}
	}
...
//DA CONTROLLARE!!! la posizione non è detto che sia aperta, e tra l'altro index non esiste in questo scope!!!!
			//Controllo su min e max accettati dall'exchange
			if (isOrderTooSmall(s.product, order_tmp.size, order_tmp.price)) {
				let err = new Error('\nMinimum size')
				err.desc = 'refusing to ' + signal + ' ' + order_tmp.size + ' ' + s.asset + ' < minimum size (' + s.product.min_size + '). Position ' + position_id + '. Clearing the position';
				orderDelete(undefined, undefined, position.id, function() {
					...
					if (so.mode != 'sim') {
						//La funzione seguente elimina anche la posizione da s.positions
