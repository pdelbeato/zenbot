...
module.exports = function (s, conf) {
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		...
		s.orders[orderSignal][orderType][orderId] = {
			...
			position_side: (s.positions[orderId] ? s.positions[orderId].side : orderSignal), //Side della posizione
	...
	function executeStop () {
		...
		s.positions.forEach( function (position, index) {
			...
			if (!s.acted_on_stop && position_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position_stop) < 0)) {
				...
				console.log(('\n' + position.side.toUpperCase() + ' stop triggered at ' + formatPercent(position.profit_pct) + ' trade profit for position ' + position.id + '\n').red)
		...
		s.exchange.getOrder({order_id: s.orders[signal][sig_type][sig_id].order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					if (s.orders[signal][sig_type][sig_id].side == signal) {
...
//			local_time: null, //Tempo locale di chiusura dell'ultimo trade della posizione
			...
			s.orders[signal][sig_type][sig_id].buy_stop = (s.orders[signal][sig_type][sig_id].side == 'buy' ? so.buy_stop_pct && n(s.orders[signal][sig_type][sig_id].position_price).multiply(1 + so.buy_stop_pct/100).value() : null)
			s.orders[signal][sig_type][sig_id].sell_stop = (s.orders[signal][sig_type][sig_id].side == 'sell' ? so.sell_stop_pct && n(s.orders[signal][sig_type][sig_id].position_price).multiply(1 - so.sell_stop_pct/100).value() : null)
//			profit_pct: (s.positions[orderId] ? s.positions[orderId].profit_pct : null),
//			profit_stop_limit: (s.positions[orderId] ? s.positions[orderId].profit_stop_limit : null),
//			profit_stop: (s.positions[orderId] ? s.positions[orderId].profit_stop : null),
//			order_type: is_taker ? 'taker' : so.order_type,
