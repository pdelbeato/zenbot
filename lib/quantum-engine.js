				...
				if (position_id) {
					...
					if (position) {
						...
						//Controllo se la posizione ha profitto sufficiente e creo l'ordine a partire dai valori della posizione position_id
							...
							debug.msg('executeSignal - con position_id. Annullo, perchè la posizione ' + position_id + ' non ha profitto sufficiente (' + formatPercent(position.profit_net_pct/100) + ' < ' + so[signal + '_gain_pct'] + ')')
							_cb && _cb(null, null)
							return
						}
					}
...
//					orderDelete(undefined, undefined, position.id, function() {
				...
				if (position_counter === array.length) {
