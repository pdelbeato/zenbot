...
//			positionStatus(position, 'Unset', sig_kind)
		...
		if (so.stats) {
			...
			if (position_index != null) {
				order_complete += '\n\nPosition Id: ' + order_tmp.position.id + (is_partial ? ' (partial)' : '')
				order_complete += '\nOriginal price: ' + formatCurrency(order_tmp.position.price_open, s.currency)
				order_complete += '\nPosition Size: ' + formatAsset(order_tmp.position.size, s.asset)
				order_complete += '\nPosition value: ' + formatCurrency(order_tmp.position.value, s.currency)
				order_complete += '\nProfit: ' + n(order_tmp.position.profit_pct/100).format('0.0000%')
				order_complete += '\nPosition span: ' + moment.duration(order_tmp.position.time_close - order_tmp.position.time_open).humanize()
			}
