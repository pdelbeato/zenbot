			...
			s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
				...
				getQuote(function (err) {
					if (err)
						...
						s.asset_in currency = n(s.balance.asset).multiply(s.quote.ask).value()
						s.real_capital = n(s.balance.currency).add(s.asset_in currency).value()
...
//						})
						...
						if (!s.start_capital) {
							...
							s.start_capital = n(s.balance.currency).add(s.asset_in currency).value()
						}
