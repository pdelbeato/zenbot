					...
					if (err)
						...
						s.orders.forEach(function (order) {
							if (order.kind === 'manualcatch') {
								...
								if (order.signal === 'buy') {
									s.available_balance.currency = n(s.available_balance.currency).subtract(order.value).format(s.product.increment)
									s.available_balance.currency_hold = n(s.available_balance.currency_hold).subtract(order.value).format(s.product.increment)
								}
								else {
...
//		Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
		...
		case 'buy': {
			...
			else if (s.balance.currency_hold > 0 && n(s.balance.currency).subtract(s.balance.currency_hold).value() < n(order_tmp.price).multiply(order_tmp.size).value()) {
		...
		case 'sell': {
			...
			else if (s.balance.asset_hold > 0 && n(s.balance.asset).subtract(s.balance.asset_hold).value() < n(order_tmp.size).value()) {
...
//				s.delete_position_id = null
				s.positionProcessingQueue.push({'update', position_id})
			} 
			else {
//				s.update_position_id = null
//				s.delete_position_id = position_id
				s.positionProcessingQueue.push({'delete', position_id})
