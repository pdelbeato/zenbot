	...
	function initPeriod(trade, callback = function() {}) {
		try {
			...
			if (s.period) {
				...
				//Registra il vecchio periodo nel db_pèriods
				if (!so.minimal_db) {
		...
		try {
			...
			if (order_tmp.position.id != null) {
				...
				//Se la posizione ha size nullo o comunque inferiore al minimo commerciabile
					...
					let position_profit = (order_tmp.position.side == 'buy' ? +1 : -1) * n(order_tmp.position.value_close).subtract(order_tmp.position.value_open).subtract(order_tmp.position.fee_open).subtract(order_tmp.position.fee_close).value()
					...
					// //Se siamo in sim, inserisco la posizione chiusa nel db_my_closed_positions
					// else {
					// 	s.db_my_closed_positions.updateOne({ '_id': position._id }, { $set: position }, { multi: false, upsert: true }, function (err) {
					// 		if (err) {
					// 			console.error('\n' + moment().format('YYYY-MM-DD HH:mm:ss') + ' - quantum-engine - Error saving in db_my_closed_positions:')
					// 			console.error(err)
					// 		}
					// 	})
					// }
		...
		try {
			...
			async function goOn() {
				...
				if (trade.time > s.period.close_time) {
					await writeReport(false)//, lastPeriod)

					await initPeriod(trade)
					// //Aggiungi il periodo a s.lookback
					// s.lookback.unshift(s.period)
					// s.lookback[0].strategy = {}
					...
					s.tools.functionStrategies('onTradePeriod', opts
						...
						, async function (err, strategy_function) {
							...
							// 			s.period.rsi = lastPeriod.rsi
							// 			writeReport(undefined, undefined, callback)
							// 		})
							// 	})
							// }
							// else {
							// 	//Inizializza un period, aggiorna calc_close_time delle strategie, e aggiorna il period con i valori del trade
							// 	initPeriod(trade, function() {
							// 		s.period.rsi = lastPeriod.rsi
							// 		writeReport(undefined, undefined, callback)
							// 	})
							...
							//  Se non siamo in sim, ripulisce s.lookback a un max di valori
