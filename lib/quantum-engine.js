...
//				debug.msg('syncBalance - balance= ' + JSON.stringify(s.balance) + ' ; available_balance= ' + JSON.stringify(s.available_balance))
	...
	function executeSignal (signal, sig_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		...
		if (is_reorder) {
			debug.msg('executeSignal - is_reorder: assegno a working_position i valori di s.orders[' + signal_id + ']')
			working_position = s.orders[signal_id]
		}
		else if (sig_id) {
			...
			if (s.orders[signal] && s.orders[signal].id == sig_id) {
			...
			if (s.orders[signal_opposite])
				delete s.orders[signal_opposite]
...
//				working_position = s.my_positions[working_position_index]
		...
		debug.msg('executeSignal - ' + (s.orders.[signal_id]? ('s.orders[' + signal_id + '] esiste') : ('s.orders[' + signal_id + '] non esiste')))
		
		let cb = function (err, order) {
			if (!order) {
				debug.msg('executeSignal - cb: cancello s.orders[' + signal_id + ']')
				if (working_position) {
...
//		}
		...
		syncBalance(function (err) {
			...
			if (s.orders[signal_id] && !s.orders[signal_id].fixed_price && so.max_slippage_pct != null) {
				slippage = Math.abs(n(s.orders[signal_id].orig_price).subtract(price).divide(s.orders[signal_id].orig_price).multiply(100).value())
				...
				if (!order) {
					...
					let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
						if (remaining_size != size) {
							debug.msg('executeSignal - doOrder - remaining size: ' + remaining_size + ' of ' + s.orders[signal_id].size)
						}
...
//************** Da controllare questo punto.						
	...
	function executeOrder (trade, signal, sig_id) {
		...
		position_index = s.orders[signal_id].position_index

		price = s.orders[signal_id].price
		
		if (so.order_type === 'maker') {
			if (s.exchange.makerFee)
				fee = n(s.orders[signal_id].size).multiply(s.exchange.makerFee / 100).value()
		}
		if (so.order_type === 'taker') {
			if (s.exchange.takerFee)
				fee = n(s.orders[signal_id].size).multiply(s.exchange.takerFee / 100).value()
		}
		...
		let my_trade = {
			id: s.orders[signal_id].id,
			_id: s.orders[signal_id].id,
			order_id: trade.order_id,
			time: trade.time,
			execution_time: trade.time - s.orders[signal_id].orig_time,
			slippage: (signal == 'buy' ? 1 : -1) * n(price).subtract(s.orders[signal_id].orig_price).divide(s.orders[signal_id].orig_price).value(),
			side: signal,
			size: s.orders[signal_id].filled_size,
			fee: fee,
			price: price,
			//Al cost manca il fee... da aggiungere una volta capito come calcolarlo
			cost: s.orders[signal_id].cost,
...
//				debug.printPosition(s.my_positions)
		...
		delete s.orders[signal_id]
		debug.msg('executeOrder - delete s.orders[' + signal_id + ']')
		
		//Cancella anche tutti gli ordini connessi con la posizione
		if (s.orders[signal + my_trade.id]) {
			delete s.orders[signal + my_trade.id]
			debug.msg('executeOrder - delete s.orders[' + signal + my_trade.id + ']')
		}
