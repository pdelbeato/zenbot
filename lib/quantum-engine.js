...
//				s.acted_on_stop = true
			if (position_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position_stop) < 0)) {
				position_opposite_signal = (position.side === 'buy' ? 'sell' : 'buy')
				console.log(('\n' + position.side.toUpperCase() + ' stop triggered at ' + formatPercent(position.profit_pct/100) + ' trade profit for position ' + position.id + '\n').red)
...
//				s.acted_on_trail_stop = true
			if (position.profit_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
				position_opposite_signal = (position.side === 'buy' ? 'sell' : 'buy')
...
//				debug.msg('placeOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + ' - now() ' + now() + ' ; s.next_order ' + s.next_order)
				...
				s.exchange[signal](order_copy, function (err, api_order) {
					...
					orderStatus(order_tmp, undefined, undefined, undefined, 'Set', sig_kind)
				...
				setTimeout(function() { placeOrder(cb) }, (s.next_order - now()))
			}
		}
	}
...
//								order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value)
			...
			setTimeout(function() { checkOrder(signal, sig_kind, position_id, cb) }, (s.next_order - now()))
		}
	}
...
//							************** Da controllare questo punto.		
			...
			setTimeout(function() { cancelOrder(signal, sig_kind, position_id, do_reorder, cb) }, (s.next_order - now()))
		}
	}
...
//			if (order_tmp.position.size != 0 && (s.product.min_size ? (n(order_tmp.position.size).value() > s.product.min_size) : true)) {
				...
				//Cancella anche tutti gli ordini connessi con la posizione, eccetto quello attuale (che viene cancellato dopo)
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderDelete(signal, sig_kind, position_id) {
