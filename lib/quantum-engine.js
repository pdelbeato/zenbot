...
//						if (s.orders[signal] && s.orders[signal]['standard'] && s.orders[signal]['standard'].length != 0) {
						if (s.orders[signal] && !_.isEmpty(s.orders[signal]['standard'])) {
...
//			s.orders[signal_id] = trade
		...
		let my_trade = {
			...
			position_profit: (position != null ? position.profit_pct : null),
			position_price: (position != null ? position.position_price : null),
...
//			cancel_after: s.cancel_after // s.cancel_after || 'day'
		...
		if (so.stats) {
			...
			if (position != null) {
				order_complete += '\nOriginal price: ' + formatCurrency(my_trade.position_price, s.currency)
				order_complete += '\nProfit: ' + n(my_trade.position_profit).format('0.0000%')
				order_complete += '\nExecution: ' + moment.duration(my_trade.execution_time).humanize()
