...
//	switchOnListener()
	...
	function initBuffer (trade) {
		...
		Object.keys(so.strategy).forEach(function (strategy_name, index) {
			...
			s.period.strategy[strategy_name] = {}
		})
	}
...
//					s.acted_on_trail_stop = false
					...
					//  se non siamo in sim ripulisci so.strategy[strategy_name].calc_lookback a un max di valori
					...
					if (strategyProcessed === array.length) {
						...
						//  se non siamo in sim ripulisci s.lookback a un max di valori
