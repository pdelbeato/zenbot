...
module.exports = function (s, conf) {
  ...
  function nextBuyForQuote(s, quote) {
    ...
    // else {
    debug.msg('\nnextBuyForQuote: bid=' + quote.bid + ' return=' + n(quote.bid).subtract(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).add(so.best_bid ? s.product.increment : 0).format(s.product.increment, Math.floor))
  ...
  function nextSellForQuote(s, quote) {
    ...
    // else
    debug.msg('\nnextSellForQuote: bid=' + quote.ask + ' return=' + n(quote.ask).add(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).subtract(so.best_ask ? s.product.increment : 0).format(s.product.increment, Math.ceil))
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
          ...
          debug.msg('size < s.product.min_size. Deleting position ' + working_position_index)
