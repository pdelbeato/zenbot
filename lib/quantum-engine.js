...
//					sell_stop: null,
//					buy_stop: null,
					profit_pct: order_tmp.position.profit_pct,
//					profit_stop_limit: null,
//					profit_stop: null				
				}		
		...
		updatePositions(order_tmp, true, function() {
			...
			if (so.stats) {
				...
				if (is_closed) {					
					...
					order_complete += '\nProfit: ' + n(position.profit_pct/100).format('0.0000%')
...
//			debug.msg('updatePositions')
			...
			s.positions.forEach(function (position, index) {
				...
				position.profit_pct = (position.side == 'buy' ? +100 : -100) * n(trade.price).subtract(position.price_open).divide(position.price_open).value()
