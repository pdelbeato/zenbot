...
//			Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
			...
			case 'buy': {
				if (Number(tradeable_balance) > Number(available_balance_currency)) {
					...
					s.hold_signal = false
					pushMessage('Buying (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing ' + signal + ' order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(s.quote.bid - order_tmp.price).format('0.00'), s.currency) + ' under best bid\n', 9)
			...
			case 'sell': {
				if (Number(order_tmp.size) > Number(available_balance_asset)) {
					...
					s.hold_signal = false
					pushMessage('Selling (' + sig_kind + ') ' + formatAsset(order_tmp.size, s.asset, s.product.asset_increment) + ' on ' + s.exchange.name.toUpperCase(), 'placing sell order at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + formatCurrency(n(order_tmp.price - s.quote.ask).format('0.00'), s.currency) + ' over best ask\n', 9)
...
//			positionStatus(position, 'Unset', sig_kind)
		...
		updatePositions(order_tmp, function() {
			...
			if (so.stats) {
				...
				order_complete += formatAsset(my_trade.size, s.asset, s.product.asset_increment) + ' at ' + formatCurrency(my_trade.price, s.currency) + '\n'
...
//			process.stdout.write(z(7, formatPercent(over_buy_hold_pct), ' ')[over_buy_hold_pct >= 0 ? 'green' : 'red'])
	...
	function withOnPeriod (trade, cb) {
...
//				so.strategy[strategy_name].lib.onPeriod.call(s.ctx, s, function () {
				so.strategy[strategy_name].lib.onPeriod(s, function () {
					strategyProcessed++
