...
//				position._id = position_id	
	...
	function writeReport (is_progress, period, callback = function() {}) {
//		debug.msg('writeReport')
		ti_rsi(s, so.rsi_periods)
		.then( function(result) {
			s.period.rsi = result.rsi
			...
			if (s.lookback[1]) {
				let diff = (period.close - s.lookback[1].close) / s.lookback[1].close
...
//        }		
		...
		.catch( function(error) {
			console.log('quantum_engine - Errore in ti_rsi: ', error)
			callback()
		})
	}

	//onTrade viene eseguito per ogni trade. I trade vengono scaricati ogni period_length minuti.
	function onTrade(trade, is_preroll, callback = function() {}) {
		if (s.exchange.debug_exchange) {
...
//						debug.msg('onTrade - s.lookback ridotto a ' + s.lookback.length)
			...
			//Stampa a video il trade in progress
			let tmp_period = JSON.parse(JSON.stringify(s.period))
			writeReport(true, tmp_period, callback)
		}
