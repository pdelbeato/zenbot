...
module.exports = function (s, conf) {
	...
	Object.keys(so.strategy).forEach(function (strategy_name, index) {
		s.strategy[strategy_name] = require(path.resolve(__dirname, `../extensions/strategies/${strategy_name}/strategy`))
		...
		if (s.strategy[strategy_name].getOptions) {
			//Applica a s.ctx il metodo getOptions preso da s.strategy[strategy_name] 
			s.strategy[strategy_name].getOptions.call(s.ctx, s)
		}
		...
		if (s.strategy[strategy_name].orderExecuted) {
			s.eventBus.on((strategy_name + '_orderExecuted'), function(signal, position_id) {
				s.strategy[strategy_name].orderExecuted(signal, position_id, conf)
			})
		}
	})
