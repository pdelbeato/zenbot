			...
			s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
				...
				getQuote(function (err) {
					...
					s.orders.forEach(function (order) {
						...
						if (order.signal === 'buy') {
							let order_value = n(order.size).multiply(order.price)
							let expected_fees = n(order_value).multiply(order.fill_fees_pct)
							order_value = n(order.value).add(expected_fees)
...
//						order_tmp.fill_fees = n(order_tmp.executed_value).multiply(order_tmp.fill_fees_pct).divide(100).add(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						...
						s.total_fees += order_tmp.fill_fees
