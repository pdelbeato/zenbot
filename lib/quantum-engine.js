...
module.exports = function (s, conf) {
 quantum_unstable
  ...
  eventBus.on('manualcatch', (signal, position_id, fixed_size, fixed_price) => {
    debug.msg('Listener -> manualcatch ' + signal + ' ' + formatAsset(fixed_size, s.asset) + ' at ' + formatCurrency(fixed_price, s.currency))
    executeSignal (signal, 'manualcatch', position_id, fixed_size, fixed_price)
  })
  ...
  function executeStop () {
    ...
    //		else {
    //			setTimeout(function() { executeStop() }, (s.next_check - now() + 1))
    //		}
  }
  ...
  function executeSignal (signal, sig_kind = 'standard', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
    ...
    //		if (!is_reorder && (sig_kind != 'stoploss') && !fixed_price && order_tmp.position.profit_pct && so['max_' + signal + '_loss_pct'] != null && (order_tmp.position.profit_pct + so['max_' + signal + '_loss_pct'] < 0)) {
    if ((sig_kind != 'stoploss') && !fixed_price && order_tmp.position.profit_pct && so[signal + '_gain_pct'] != null && (order_tmp.position.profit_pct - so[signal + '_gain_pct'] < 0)) {
      let err = new Error('\nPosition ' + position_id + ' Profit protection')
      err.desc = 'refusing to ' + signal + ' at ' + formatCurrency(order_tmp.price, s.currency) + ', PROFIT of ' + formatPercent(order_tmp.position.profit_pct) + ' (limit ' + formatPercent(so[signal + '_gain_pct'] / 100) + ')\n'
    ...
    //  (balance reale meno le quantità bloccate dalle posizioni e dagli ordini manualcatch più la quantità della posizione in esame, se esiste)
    //Se è un ordine manualcatch, allora il controllo deve essere fatto su balance reale meno le quantità bloccate dalle posizioni e dagli ordini manualcatch
    // Secondo me, è possibile togliere il controllo su manualcatch. Verificare.
    available_balance_currency = (sig_kind === 'manualcatch' ? s.available_balance.currency : (n(s.available_balance.currency).add(order_tmp.position.value)))
    available_balance_asset = (sig_kind === 'manualcatch' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
  ...
  function updateMessage() {
    ...
    var output_lines = []
    output_lines.push('Balance ' + formatCurrency(s.balance.currency, s.currency) + ' - ' + formatAsset(s.balance.asset, s.asset))
    output_lines.push('Balance in currency ' + formatCurrency(s.real_capital, s.currency))
    output_lines.push('Balance in asset ' + formatCurrency(s.asset_capital, s.asset))
...
//	s.next_check = 0
	...
	//Attiva il listener per gli ordini di tipo manual catch
	eventBus.on('manual', (signal, position_id, fixed_size, fixed_price) => {
		debug.msg('Listener -> manual ' + signal + ' ' + formatAsset(fixed_size, s.asset) + ' at ' + formatCurrency(fixed_price, s.currency))
		executeSignal (signal, 'manual', position_id, fixed_size, fixed_price)
	})
	...
	function executeStop () {
		...
		if (now() > s.next_check) {
			...
			s.positions.forEach( function (position, index) {
				...
				if (position_stop && !positionStatus(position, 'Check', 'manual') && !positionStatus(position, 'Check', 'stoploss') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position_stop) < 0)) {
					console.log(('\n' + position_opposite_signal.toUpperCase() + ' stop loss triggered at ' + formatPercent(position.profit_pct/100) + ' trade profit for position ' + position.id + '\n').red)
					pushMessage('Stop Loss Protection', position.side + ' position ' + position.id + ' (' + formatPercent(position.profit_pct/100) + ')', 0)
					executeSignal(position_opposite_signal, 'stoploss', position.id, undefined, undefined, false, true)
					return
				...
				if (position.profit_stop && !positionStatus(position, 'Check', 'manual') && !positionStatus(position, 'Check', 'trailstop') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
					console.log(('\nProfit stop triggered at ' + formatPercent(position.profit_pct/100) + ' trade profit for position ' + position.id + '\n').green)
					pushMessage('Trailing stop', position.side + ' position ' + position.id + ' (' + formatPercent(position.profit_pct/100) + ')', 0)
					executeSignal(position_opposite_signal, 'trailstop', position.id, undefined, undefined, false, false)
					return
...
//			positionStatus(position, 'Unset', sig_kind)
			...
			//Se la posizione deriva da un ordine 'manual', allora attivo il flag 'locked', per evitare che venga usata nelle strategie
			if (sig_kind === 'manual')
				positionStatus(position, 'Set', 'locked');
...
//					s.delete_position_id = position.id
			...
			if (!positionStatus(position, 'Check', 'manual')) {
				if (so.profit_stop_enable_pct && position.profit_pct >= so.profit_stop_enable_pct) {
					position.profit_stop_limit = (position.side === 'buy' ? (Math.max(position.profit_stop_limit || trade.price, trade.price)) : (Math.min(position.profit_stop_limit || trade.price, trade.price)))
					position.profit_stop = position.profit_stop_limit + (position.side === 'buy' ? -1 : +1) * (position.profit_stop_limit * (so.profit_stop_pct / 100))
					if (position.profit_pct >= max_trail_profit) {
