...
//				position._id = position_id	
	...
	function writeReport (is_progress, cb = function() {}) {
...
//					writeReport(true, true)
					...
					process.stdout.write(moment(s.period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss')['grey'])
//					process.stdout.write(' ' + formatCurrency(s.period.close, s.currency, true, true, true) + ' ' + s.product_id.grey)
				}, 300)
				setTimeout(function () {
//					writeReport(true, false)
					readline.cursorTo(process.stdout, 0)
					process.stdout.write(moment(s.period.latest_trade_time).format('YYYY-MM-DD HH:mm:ss')['bgBlue'])
//					process.stdout.write(' ' + formatCurrency(s.period.close, s.currency, true, true, true) + ' ' + s.product_id.grey)
				}, 600)
				setTimeout(function () {
//					writeReport(true, true)
...
//					process.stdout.write(' ' + formatCurrency(s.period.close, s.currency, true, true, true) + ' ' + s.product_id.grey)
			...
			readline.cursorTo(process.stdout, 0)
			process.stdout.write(moment(is_progress ? s.period.latest_trade_time : s.period.time).format('YYYY-MM-DD HH:mm:ss')[is_progress ? 'bgBlue' : 'grey'])
			process.stdout.write(' ' + formatCurrency(s.period.close, s.currency, true, true, true) + ' ' + s.product_id.grey)
			if (s.lookback[1]) {
