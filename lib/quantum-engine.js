...
module.exports = function (s, conf) {
	...
	s.eventBus.on('manual', (signal, position_id, fixed_size, fixed_price, false, is_taker= false) => {
		debug.msg('Listener -> manual ' + signal + ' ' + formatAsset(fixed_size, s.asset) + ' at ' + formatCurrency(fixed_price, s.currency))
		executeSignal (signal, 'manual', position_id, fixed_size, fixed_price, is_reorder, is_taker)
	})
	...
	if (so.catch_order_pct > 0) {
		s.eventBus.on('orderExecuted', (signal, position_id) => {
	...
	//Funzione per assegnare alle opzioni della strategia i valori di default qualora non fossero definiti nei file di configurazione
	s.ctx = {
		option: function (strategy_name, option_name, desc, type, def) {
			if (typeof so.strategy[strategy_name][option_name] === 'undefined') {
				so.strategy[strategy_name][option_name] = def
			}
		}
	}
	
	so.strategy.forEach(function (strategy, index) {
		s.strategy[strategy.name] = require(path.resolve(__dirname, `../extensions/strategies/${strategy.name}/strategy`))
		
		//Aggiunge la strategia alla lista degli orderFlag
		orderFlag[strategy.name] = 2 ^ (index + 4)
		debug.msg('Inizializzazione strategia - orderFlag[' + strategy.name + ']= ' + orderFlag[strategy.name])
		console.log(orderFlag)
			
		if (s.strategy[strategy.name].getOptions) {
			//Applica a s.ctx il metodo getOptions preso da s.strategy[strategy_name] 
			s.strategy[strategy.name].getOptions.call(s.ctx, s)
		}
		
		//Attiva il listener per gli ordini della strategia
		s.eventBus.on(strategy.name, (signal, position_id= null, fixed_size= null, fixed_price= null, is_reorder= false, is_taker= false) => {
			debug.msg('Listener -> ' + strategy.name + ' ' + signal + (position_id? (' ' + position_id) : '') + (fixed_size? (' size= ' + fixed_size) : '') + (fixed_price? (' price= ' + fixed_price) : '') + (is_reorder? ? ' reorder' : '') + (is_taker? ' taker' : ''))
			executeSignal (signal, strategy.name, position_id, fixed_size, fixed_price, is_reorder, is_taker)
		})
		
		if (s.strategy[strategy.name].orderExecuted) {
			s.eventBus.on((strategy.name + '_orderExecuted'), function(signal, position_id) {
				s.strategy[strategy.name].orderExecuted(signal, position_id, conf)
			})
		}
	}
...
//			s.delete_position_id = null
		...
		if (!is_partial) {
			...
			emitSignal((sig_kind + '_orderExecuted'), signal, position_id)
		}
		else {
