...
//			orig_size: null, //Size inizio trade (serve per i riordini)
		...
		if (position) {
			...
			order.position.status = orderKind
		}
		else {
			...
			order.position.status = orderKind + ' new'
		}
	...
	function executeSignal (signal, sig_kind = 'standard', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		...
		if (is_reorder) {
			...
			if (order_tmp) {
				...
				order_tmp.position.status = sig_kind
			}
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderSetStatus(signal, sig_kind, position_id, status, except_order, cb = function() {}) {
		except_index = s.orders.findIndex(x => x === except_order)
		
		s.orders.forEach(function (order, index) {
			if (index != except_index && (signal ? order.signal === signal : true) && (sig_kind ? order.kind === sig_kind : true) && (position_id ? order.id === position_id : true)) {
				debug.msg('orderSetStatus - s.orders(' + order.signal + ', ' + order.kind + ', ' + order.id + ').status = ' + status)
