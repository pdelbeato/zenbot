...
//							})
				...
				//Se position_id non esiste, allora è un ordine non specifico
					...
					if (sig_type === 'standard') {			
						...
						//Infilo tutto dentro i controlli seguenti, per evitare di inizializzare un ordine e doverlo cancellare subito dopo
...
//		debug.msg('checkOrder')
		var order_tmp = s.orders.find(x => (x.signal === signal && x.type === sig_type && x.id === position_id))
		...
		s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (!order_tmp.fixed_price && (now() - order_tmp.local_time >= so.order_adjust_time)) {
				getQuote(function (err) {
					...
					//Per controllare la corretta esecuzione con i prezzi volatili. Sembra che funzioni tutto sia in sell che in buy.
