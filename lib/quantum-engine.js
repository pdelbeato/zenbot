...
//		id: (orderId ? orderId : crypto.randomBytes(4).toString('hex'))
		
		s.orders[orderSignal][orderType][orderId] = {
			id: id,
			side: null,
			signal: orderSignal,
			status: (orderId ? 'copied' : 'created'),
			selector: so.selector.normalized,
			time: null,
			orig_time: null,
			local_time: null,
			price: null,
			cost: (orderId ? s.positions[orderId].cost : 0),
			fixed_price: null,
			size: (orderId ? s.positions[orderId].size : null),
			fee: 0,
			orig_size: (orderId ? s.positions[orderId].orig_size : null),
			remaining_size: (orderId ? s.positions[orderId].remaining_size : null),
			filled_size: 0,
			orig_price: null,
			sell_stop: (orderId ? s.positions[orderId].sell_stop : null),
			buy_stop: (orderId ? s.positions[orderId].buy_stop : null),
			profit_pct: (orderId ? s.positions[orderId].profit_pct : null),
			profit_stop_limit: (orderId ? s.positions[orderId].profit_stop_limit : null),
			profit_stop: (orderId ? s.positions[orderId].profit_stop : null),
			...
			if (sig_type == 'standard') {			
				...
				if (s.max_profit_position.trend[signal_opposite]) {
					...
					getQuote(function (err) {
						...
						//La posizione individuata non ha profitto sufficiente, quindi creo un nuovo ordine standard
						if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
							...
							debug.msg('executeSignal - standard senza sig_id - max_profit_position.trend.' + signal_opposite + '.id ' + position.id + ' con profitto basso (' + position.profit_pct + '). Proseguo con ordine standard')
				...
				if (s.orders[signal_opposite]['standard'])
