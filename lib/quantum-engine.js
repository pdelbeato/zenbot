...
module.exports = function (s, conf) {
	...
	function initPosition (position_id = null) {
		position = {
				...
				opened_by: orderFlag.free,
				closed_by: orderFlag.free,
				locked: false,
				side: null,
...
//					debug.msg('syncBalance - balance= ' + JSON.stringify(s.balance) + ' ; available_balance= ' + JSON.stringify(s.available_balance))
			...
			waitCondition('wait_syncBalance', 100, cb)
		}
	}
...
//		getQuote(function (err) {
			...
			order_tmp.funds = n(so.quantum_value).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
			...
			if (isOrderTooSmall(s.product, order_tmp.size, order_tmp.price)) {
				let err = new Error('\nMinimum size')
				err.desc = 'refusing to ' + signal + ' ' + order_tmp.size + ' ' + s.asset + ' < minimum size (' + s.product.min_size + '). Position ' + position_id;
				return cb(err, null)
			}
			...
			//Calcolo fee
			if (so.use_fee_asset) {
				order_tmp.fill_fees_pct = 0
			} else if (order_tmp.order_type === 'maker') {
...
//			available_balance_asset = (sig_kind === 'manual' ? s.available_balance.asset : (n(s.available_balance.asset).add(order_tmp.position.size)))
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_kind + ' ' + position_id + ' order over ' + formatAsset(order_tmp.size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance with a expected fee of ' + formatCurrency(expected_fee, s.currency) + ' (' + order_tmp.fill_fees_pct + '%)')
...
//							let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
		...
		function placeOrder (cb) {
			...
			//Se è un ordine limit, allora funds non serve
			//Se è un ordine market, allora size non serve
			if (order_copy.type === 'limit') {
...
//						order_tmp.fill_fees = n(order_tmp.executed_value).multiply(order_tmp.fill_fees_pct).divide(100).add(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						order_tmp.fill_fees = n(order_tmp.fill_fees).add(api_order.fill_fees)
						...
						if (order_tmp.position.side == signal) {
							...
							order_tmp.position.fee_open = n(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
							order_tmp.position.value_open = n(order_tmp.position.value).subtract(order_tmp.position.fee_open).format(s.product.increment ? s.product.increment : '0.00000000')
							...
							order_tmp.position.fee_close = n(order_tmp.fill_fees).format(s.product.increment ? s.product.increment : '0.00000000')
						}
...
//						})
			...
			//Se non è un ordine fixed_price, esguo il controllo sul prezzo
			
			// Questo è da togliere, se funziona bene così
...
//		}		
		...
		let my_trade = api_order
//		{ id: 'c8bd03a5-e82d-47a6-b961-598178f72e05',
//		  size: '0.07081671',
//		  product_id: 'BTC-USD',
//		  side: 'buy',
//		  funds: '997008.9730807500000000',
//		  type: 'market',
//		  post_only: false,
//		  created_at: '2019-03-19T21:57:33.342114Z',
//		  done_at: '2019-03-19T21:57:33.351Z',
//		  done_reason: 'filled',
//		  fill_fees: '0.7740682237500000',
//		  filled_size: '0.07081671',
//		  executed_value: '258.0227412500000000',
//		  status: 'done',
//		  settled: true }
		
		my_trade.execution_time= (new Date(api_order.done_at).getTime() - new Date(api_order.created_at).getTime())
		my_trade.slippage= (signal == 'buy' ? 1 : -1) * n(order_tmp.price).subtract(order_tmp.initial_price).divide(order_tmp.initial_price).value()
		my_trade.price= n(order_tmp.price).format('0.00')
		my_trade.initial_price= n(order_tmp.initial_price).format('0.00')
		my_trade.cancel_after= so.cancel_after // || null //'day'
		my_trade.position_id= position_id
...
//					value: 0, //Valore currency attuale della posizione
					...
					value_close: n(order_tmp.position.value_open).subtract(order_tmp.position.value).format(s.product.increment), //Ricavo currency dalla posizione chiusa
...
//			positionStatus(position, 'Unset', sig_kind)
		...
		updatePositions(order_tmp, true, function() {
			...
			if (so.stats) {
				...
				order_complete += 'Total ' + formatCurrency((my_trade.value - my_trade.fee), s.currency) + '\n'
				order_complete += 'Fee ' + formatCurrency(my_trade.fee, s.currency) + '\n'
				...
				if (is_closed) {					
					...
					order_complete += '\nPosition Size: ' + formatAsset(position.initial_size, s.asset)
					...
					order_complete += '\nPosition opening fee: ' + formatCurrency(position.fee_open, s.currency)
					order_complete += '\nPosition closing fee: ' + formatCurrency(position.fee_close, s.currency)
...
//		syncBalance( function () {
		...
		if (!s.in_preroll) {
			...
			updatePositions(trade, false, function() {
		...
		if (trade.time > s.period.close_time) {
			...
			Object.keys(so.strategy).forEach(function (strategy_name, index, array) {
				so.strategy[strategy_name].lib.onPeriod.call(s.ctx, s, function () {
					strategyProcessed++
					let max_length = s.options.strategy[strategy_name].opts.size + 1
...
//					s.acted_on_trail_stop = false
					...
					//Aggiungi il periodo a so.strategy[strategy_name].calc_lookback
					//  e, se non siamo in sim, ripulisce so.strategy[strategy_name].calc_lookback a un max di valori

					if (trade.time > so.strategy[strategy_name].calc_close_time) {
						so.strategy[strategy_name].calc_lookback.unshift(s.period)
					}

					if (so.mode !== 'sim' && so.strategy[strategy_name].calc_lookback.length > max_length) {
					...
					if (strategyProcessed === array.length) {
						if (so.mode !== 'sim' && s.lookback.length > max_length) {
...
//			debug.msg('onTrade: chiamo withOnPeriod dentro period ' + s.signal + ' Time= ' + moment())
	...
	function updatePositions(trade, checkTooSmall, cb) {
...
//			debug.msg('updatePositions')
			...
			s.positions.forEach(function (position, index) {
				...
				if (checkTooSmall && isOrderTooSmall(s.product, position.size, position.price_open)) {
				...
				let price_open_tmp = n(position.price_open).add((position.side == 'buy' ? +1 : -1) * position.fee_open)
