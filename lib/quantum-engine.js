...
//Controlli timing (da sostituire con funzione?)
				...
				if (position_id) {
					...
					if (position) {
						...
						//Controllo se la posizione ha profitto sufficiente (solo se non è stoploss) e creo l'ordine a partire dai valori della posizione position_id
						else if (sig_kind === 'stoploss' || (so[signal + '_gain_pct'] != null && (position.profit_net_pct - so[signal + '_gain_pct'] > 0))) {
