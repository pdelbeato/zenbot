				...
				s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done') {
							...
							order.filled_size = api_order.filled_size
							order.remaining_size = api_order.remaining_size
							debug.msg('cancelOrder - cancel failed, order done, executing')
							executeOrder(order, signal, sig_id)
							return syncBalance(function () {
								cb(null, order)
							})
...
//								order.size = order.filled_size
					...
					//Se una parte dell'ordine è stata eseguita (magari in un passaggio precedente, e in questo riordine non appare)
					// allora apre/aggiorna lo stesso la posizione
...
//************** Da controllare questo punto.						
						if (signal === 'buy') on_hold = n(s.available_balance.currency).subtract(s.balance.currency_hold || 0).value() < n(order.price).multiply(order.remaining_size).value()
						else on_hold = n(s.available_balance.asset).subtract(s.balance.asset_hold || 0).value() < n(order.remaining_size).value()
...
//			process.stdout.write(z(9, s.signal || '', ' ')[s.signal ? s.signal === ('pump' || 'dump') ? 'white' : s.signal === 'buy' ? 'green' : 'red' : 'grey'])
		...
		if (s.orig_capital) {
			...
			asset_col += ' ' + s.asset
			let asset_col_width = Math.max(asset_col.length + 1, asset_col_width)
			...
			let currency_col_width = Math.max(currency_col.length + 1, currency_col_width)
