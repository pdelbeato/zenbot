...
//				debug.msg('placeOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + ' - now() ' + now() + ' ; s.next_order ' + s.next_order)
				...
				s.exchange[signal](order_copy, function (err, api_order) {					
					...
					//Ripete il controllo dopo so.order_poll_time. Se è un catching order, dopo so.catch_order_poll_time'
					setTimeout(function() { checkOrder(signal, sig_kind, position_id, cb) }, (sig_kind === 'catching' ? so.catch_order_poll_time : so.order_poll_time))
				})
			}
			else {
