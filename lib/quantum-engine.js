...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			debug.msg('executeSignal - Size: ' + n(fixed_size))
			debug.msg('executeSignal - Size: ' + n(s.orders[signal][sig_type][sig_id].remaining_size))
			debug.msg('executeSignal - Size: ' + n(so.quantum_size).divide(s.orders[signal][sig_type][sig_id].price))
...
//				debug.printPosition(s.positions)
			...
			s.orders[signal][sig_type][sig_id] = {
					status: 'opened',
					buy_stop: (signal == 'buy' ? so.buy_stop_pct && n(s.orders[signal][sig_type][sig_id].price).multiply(1 + so.buy_stop_pct/100).value() : null),
					sell_stop: (signal == 'sell' ? so.sell_stop_pct && n(s.orders[signal][sig_type][sig_id].price).multiply(1 - so.sell_stop_pct/100).value() : null)
			}
