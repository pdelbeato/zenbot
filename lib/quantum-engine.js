...
module.exports = function (s, conf) {
	...
	eventBus.once('buy', () => {
		debug.msg('Listener -> buy')
		executeSignal('buy')
	})
	eventBus.once('sell', () => {
		debug.msg('Listener -> sell')
		executeSignal('sell')
	})
	
	function emitSignal (event, argument) {
		eventBus.emit(event, argument)
	}
	...
	function executeStop () {
		...
		s.positions.forEach( function (position, index) {
			...
			if (!s.acted_on_trail_stop && position.profit_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
				...
				executeSignal(position_opposite_signal, 'TrailStop', position.id)
				return
			}
		})
	}
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		if (sig_id) {
			...
			//Se non esiste l'ordine associato a [signal][sig_type][sig_id], allora lo devo creare
				...
				if (s.orders[signal] && s.orders[signal]['standard'] && s.orders[signal][sig_type][sig_id]) {
			...
			if (sig_type == 'standard') {			
				...
				if (s.max_profit_position.trend[signal_opposite]) {
					...
					getQuote(function (err) {
						...
						//La posizione individuata ha profitto sufficiente, quindi creo un ordine standard associato a quella posizione
						else {
				...
				//Non ci sono posizioni da considerare. L'ordine standard è già in piedi.
					...
					if ((signal == 'buy' && !so.active_long_position) || (signal == 'sell' && !so.active_short_position)) {
						...
						_cb && _cb(null, null)
						eventBus.once(signal, () => {
							debug.msg('Listener -> ' + signal)
...
//		executeSignal('sell', undefined, undefined, undefined, undefined, my_position.id)
	...
	function writeReport (is_progress, blink_off) {
		...
		if (s.orders.['buy'].length != 0) {
			process.stdout.write(z(9, 'buying', ' ').green)
		}
		else if (s.orders.['sell'].length != 0) {
