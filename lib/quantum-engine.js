...
//				}
	...
	function placeOrder (type, type_orig, signal_id, opts, cb) {
		if (!s[type + '_order']) {
			s[type + '_order'] = {
				price: opts.price,
				size: opts.size,
		...
		s.exchange[type_orig](order_copy, function (err, api_order) {
			...
			setTimeout(function() { checkOrder(order, type, type_orig, cb, signal_id) }, so.order_poll_time)
		})
	}
	...
	function executeSignal (signal, _cb, size, price, is_reorder, is_taker, signal_id) {
		let expected_fee
		let signal_orig = (signal.includes('buy') ? 'buy' : (signal.includes('sell') ? 'sell' : null))
		...
		//Se non ho configurato l'opzione catch_position_pct, allora non eseguire gli id order
		if (!so.catch_position_pct)
			signal_id = null
			
		//Da inglobare in signal
		if (signal_id)
			signal = signal + '_' + signal_id.toString()
			
		debug.msg('executeSignal: ' + signal + ' ' + is_reorder)
		debug.msg(s[signal + '_order']? ('s[' + signal + '_order] esiste') : ('s[' + signal + '_order] non esiste'), false)
		debug.msg('executeSignal - signal_orig: ' + signal_orig)
...
//				else delete s.sell_order
		...
		syncBalance(function (err, {balance, quote}) {
			...
			if (signal_orig === 'buy') {
				if (!price) 
					price = nextBuyForQuote(s, s.quote)
...
//						debug.msg('executeSignal - buy delayed: ' + formatPercent(n(s.balance.currency_hold || 0).divide(s.balance.deposit).value()) + ' of funds (' + formatCurrency(s.balance.currency_hold, s.currency) + ') on hold')
						...
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, cb, size, undefined, true)
							}
			...
			else if (signal_orig === 'sell' && s.my_positions.length) {
				if (!price)
...
//					working_position_index = position_max_profit_index
				...
				if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
					...
					if (!signal_id && n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
						...
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, cb, size, undefined, true, undefined, signal_id)
							}
						}, conf.wait_for_settlement)
					}
					else {
		...
		function doOrder () {
			//debug.msg('doOrder')
			placeOrder(signal, signal_orig, signal_id, {
				size: size,
				price: price,
				...
				if (!order) {
					...
					if (!signal_id) {
						...
						return executeSignal(signal, _cb, remaining_size, undefined, true)
					}
				}
				cb(null, order)
			})
	...
	function executeOrder (trade, type, type_orig, signal_id) {
		let price, fee = 0
		if (!so.order_type) {
			so.order_type = 'maker'
		}
		...
		if (s.buy_order) {
			...
			let my_position = {
				order_id: trade.order_id,
				id: signal_id || crypto.randomBytes(4).toString('hex'),
			...
			if (so.stats) {
				...
				order_complete += (signal_id != null ? 'Catch position\n' : '')
...
//			executeSignal('sell', undefined, undefined, undefined, undefined, my_position.id)
		...
		//Ordine sell
			...
			type = type_orig + '_' + s.my_positions[working_position_index].id
			debug.msg('executeOrder - type = ' + type + ' ; type_orig = ' + type_orig)
			...
			delete s[type_orig + '_order']
			debug.msg('executeOrder - delete s[' + type_orig + '_order] (type_orig)')
			if (s[type + '_order']) {
				debug.msg('executeOrder - delete s[' + type + '_order] (type)')
...
//		s.last_period_id = period_id
	...
	function cancelOrder (order, type, type_orig, do_reorder, cb, signal_id) {
				...
				s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						if (api_order.status === 'done') {
							...
							executeOrder(order, type, undefined, signal_id)
...
//						if (type === 'buy') on_hold = n(s.balance.deposit).subtract(s.balance.currency_hold || 0).value() < n(order.price).multiply(order.remaining_size).value()
	...
	function checkOrder (order, type, type_orig, cb, signal_id) {
		...
		if (!s[type + '_order']) {
			...
			return cancelOrder(order, type, type_orig, false, cb, signal_id)
		}
		s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
			if (err) return cb(err)
			s.api_order = api_order
			...
			if (api_order.status === 'done') {
				order.time = new Date(api_order.done_at).getTime()
				order.price = api_order.price || order.price // Use actual price if possible. In market order the actual price (api_order.price) could be very different from trade price
				executeOrder(order, type, type_orig, signal_id)
			...
			if (!signal_id && (now() - order.local_time >= so.order_adjust_time)) {
				getQuote(function (err, quote) {
					if (err) {
