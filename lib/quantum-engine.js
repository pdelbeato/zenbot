...
module.exports = function (s, conf) {
	...
	function initOrder (orderSignal, orderType = 'standard', orderId = null, is_taker = null, cb) {
		let position = s.positions.find(x => x.id === orderId)
		if (position) {
			debug.msg('initOrder - Esiste una posizione ' + orderId + '. Associo un s.orders ' + orderSignal + ' ' + orderType + ' alla posizione')
		}
		else {
			debug.msg('initOrder - Non esiste una posizione ' + orderId + '. Creo un s.orders ' + orderSignal + ' ' + orderType + ' ' + orderId)
		}
			
		position = {
			id: (position ? position.id : orderId),
			selector: so.selector.normalized,
			status: 'linked',
			position_side: (position ? position.position_side : orderSignal), //Side della posizione
			position_price: (position ? position.position_price : null), //Prezzo medio della posizione
			position_size: (position ? position.position_size : 0), //Valore asset attuale della posizione
			position_value: (position ? position.position_value : 0), //Valore currency attuale della posizione
			position_time: (position ? position.position_time : 0), //Time apertura della posizione
			signal: orderSignal,	
			type: orderType,
			time: null,
			...
			sell_stop: (position ? position.sell_stop : null),
			buy_stop: (position ? position.buy_stop : null),
			profit_pct: (position ? position.profit_pct : null),
			profit_stop_limit: (position ? position.profit_stop_limit : null),
			profit_stop: (position ? position.profit_stop : null),
...
//			console.log('listeners ' + eventBus.listeners(signal))
		...
		if (is_reorder) {
			...
			order_tmp = orderExist(signal, sig_type, sig_id)
			if (order_tmp) {
			...
			//...altrimenti continuo con i controlli
				...
				if (sig_id) {
					...
					if (position) {
						...
						//Creo l'ordine a partire dai valori della posizione sig_id
							...
							order_tmp = initOrder(signal, sig_type, sig_id, is_taker)
						}
					}
				...
				//Se sig_id non esiste, allora è un ordine non specifico
					...
					if (sig_type == 'standard') {			
						...
						order_tmp = initOrder(signal, 'standard', sig_id, is_taker)
						...
						if (s.max_profit_position.trend[signal_opposite]) {
							...
							if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct + so['max_' + signal + '_loss_pct'] < 0)) {
								...
								if ((signal == 'buy' && !so.active_long_position) || (signal == 'sell' && !so.active_short_position)) {
									...
									_cb && _cb(null, null)
									return orderDelete(signal, sig_type, sig_id)
								}	
							}
							...
							// associo un ordine standard a quella posizione
								...
								orderDelete(signal, sig_type, sig_id)
						...
						debug.msg('executeSignal - Ordine standard senza sig_id. Cancello TUTTI gli ordini di senso opposto ancora in essere su questa id.')
						orderDelete(signal_opposite, undefined, sig_id)
					}
...
//			s.acted_on_trend = false
		...
		syncBalance(function (err) {
			...
			order_tmp.size = n(fixed_size || order_tmp.remaining_size || order_tmp.position_size || n(so.quantum_size).divide(order_tmp.price)).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
...
//				s.orders[signal][sig_type][sig_id].position_size = s.orders[signal][sig_type][sig_id].size
			...
			debug.msg('executeSignal - Size: ' + formatAsset(order_tmp.size, s.asset))
...
//			}
			...
			debug.msg('executeSignal - preparing ' + signal + ' ' + sig_type + ' ' + sig_id + ' order over ' + formatAsset(order_tmp.size, s.asset) + ' of ' + formatCurrency(tradeable_balance, s.currency) + ' tradeable balance with a expected fee of ' + formatCurrency(expected_fee, s.currency) + ' (' + fee + '%)')
...
//					let remaining_size = s.orders[signal_id] ? s.orders[signal_id].remaining_size : size
		...
		function placeOrder (cb) {
			...
			s.exchange[signal](order_copy, function (err, api_order) {
				...
				debug.msg('placeOrder - ' + signal + ' ' + sig_type + ' ' + sig_id + ' order placed at ' + formatCurrency(order_tmp.price, s.currency))
	...
	function checkOrder (signal, sig_type, sig_id, cb) {
		...
		order_tmp = s.orders.find(x => (x.signal === signal && x.type === sig_type && x.id === sig_id))
		...
		s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					if (order_tmp.position_side == signal) {
						debug.msg('checkOrder - getOrder - side == signal')
						order_tmp.position_value += api_order.executed_value
						order_tmp.position_size += api_order.filled_size
					}
					else {
						debug.msg('checkOrder - getOrder - side != signal')
						order_tmp.position_value -= api_order.executed_value
					...
					debug.msg('checkOrder - getOrder - done - api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + order_tmp.remaining_size + ' ; surplus= ' + order_tmp.position_value)
				...
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							debug.msg('api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + order_tmp.remaining_size + ' ; surplus= ' + order_tmp.position_value, false)
...
//			s.orders[signal_id] = trade
		...
		let my_trade = {
			...
			value: order_tmp.position_value,
			...
			position_span: (position != null ? (moment.duration(order_tmp.time - position.position_time).humanize()) : null),
...
//				s.positions[sig_id].remaining_size -= my_trade.size
				...
				orderDelete(undefined, undefined, sig_id)
...
//			id: orderId,
//			selector: so.selector.normalized,
			order_tmp.status = 'opened'
//			position_side: //Side della posizione
			order_tmp.position_price = (order_tmp.position_price || n(order_tmp.position_value).divide(order_tmp.position_size).format('0.00'))
...
//			position_value: (s.positions[orderId] ? s.positions[orderId].position_value : 0), //Valore currency attuale della posizione
			order_tmp.position_time = (order_tmp.position_time || order_tmp.time) //Time apertura della posizione
//			signal: orderSignal,	
//			time: null,
//			orig_time: null, //Tempo di chiusura dell'ultimo trade della posizione
//			local_time: null, //Tempo locale di chiusura dell'ultimo trade della posizione
			order_tmp.initial_price = null //Prezzo iniziale dell'ultimo trade della posizione
//Trovare il modo per passare il prezzo finale alla funzione updatePositions
//			order_tmp.price = null //Prezzo finale dell'ultimo trade della posizione
			order_tmp.fixed_price = null
			order_tmp.fee = 0
			order_tmp.size = null //Size attuale del trade (variabile di servizio)
			order_tmp.orig_size = null //Size inizio trade (serve per i riordini)
			order_tmp.remaining_size = null //Rimanenza del trade (serve per i riordini)
			order_tmp.filled_size = 0 //Size commerciato del trade (variabile di servizio)
			order_tmp.executed_value = 0 //Value commerciato del trade (variabile di servizio)
			order_tmp.buy_stop = (order_tmp.position_side == 'buy' ? so.buy_stop_pct && n(order_tmp.position_price).multiply(1 + so.buy_stop_pct/100).value() : null)
			order_tmp.sell_stop = (order_tmp.position_side == 'sell' ? so.sell_stop_pct && n(order_tmp.position_price).multiply(1 - so.sell_stop_pct/100).value() : null)
...
//				debug.msg('updatePositions - position_max_profit_index= ' + position_max_profit_index, false)
	...
	function orderExist(signal, sig_type, sig_id) {
		return s.orders.find(x => ((signal != undefined ? (x.signal === signal) : true) && (sig_type != undefined ? (x.type === sig_type) : true) && (sig_id != undefined ? (x.id === sig_id) : true)))
	}
	
	//Funzione per cancellare un ordine da s.orders
	function orderDelete(signal, sig_type, sig_id) {
		s.orders.forEach(function (order, index) {
			if ((signal ? order.signal === signal : true) && (sig_type ? order.type === sig_type : true) && (sig_id ? order.id === sig_id : true)) {
