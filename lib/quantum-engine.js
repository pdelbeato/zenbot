		...
		s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
			...
			if (api_order) {
				...
				if (api_order.status === 'done') {
					...
					order_tmp.executed_value = n(order_tmp.executed_value).add(api_order.executed_value).format(s.product.increment ? s.product.increment : '0.00000000')
					...
					if (order_tmp.position.side == signal) {
						...
						debug.msg('checkOrder - getOrder - ' + position_id + ' side != signal')
						order_tmp.position.value = n(order_tmp.position.value).subtract(api_order.executed_value).format(s.product.increment ? s.product.increment : '0.00000000')
					...
					debug.msg('checkOrder -  getOrder - ' + position_id + ' done - api_order.executed_value= ' + api_order.executed_value + ' ; api_order.filled_size= ' + api_order.filled_size + ' ; remaining_size= ' + order_tmp.remaining_size + ' ; surplus= ' + order_tmp.position.value)
				...
				getQuote(function (err) {
					...
					if (signal === 'buy') {
						if (order_tmp.price < s.quote.bid) {
							debug.msg('checkOrder - ' + position_id + ' - ' + s.quote.bid + ' > our ' + Number(order_tmp.price))
					...
					if (signal === 'sell') {
						if (order_tmp.price > s.quote.ask) {
							debug.msg('checkOrder - ' + position_id + ' - ' + s.quote.ask + ' < our ' + Number(order_tmp.price))
							cancelOrder(signal, sig_type, position_id, true, cb)
						}
						else {
							order_tmp.local_time = now()
				...
				s.exchange.getOrder({order_id: order_tmp.order_id, product_id: s.product_id}, function (err, api_order) {
					...
					if (api_order) {
						...
						if (api_order.status === 'done' || api_order.filled_size) {
							...
							debug.msg('cancelOrder - getOrder - ' + position_id + ' cancel failed - order done or partially done')
...
//			s.orders[signal_id] = trade
		...
		let my_trade = {
			...
			position_size: n(order_tmp.position.size).format(s.product.increment), //Size totale della posizione
