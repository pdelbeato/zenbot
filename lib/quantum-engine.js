...
//		updatePositions(trade)
		...
		if (s.period && trade.time < s.period.time) {
			...
			setTimeout (function() { s.period.time = trade.time -1 }, 10000)
...
//				so.strategy[strategy_name].lib.onPeriod.call(s.ctx, s, function () {
				so.strategy[strategy_name].lib.onPeriod(s, function () {
					...
					// e, se esiste nella strategia, esegui calculateCloseTime.
					if (trade.time > so.strategy[strategy_name].calc_close_time) {
						so.strategy[strategy_name].calc_lookback.unshift(s.period)
						if (typeof so.strategy[strategy_name].lib.calculateCloseTime === 'function') {
							so.strategy[strategy_name].lib.calculateCloseTime(s)
						}
					}
