		...
		try {
			...
			if (s.period) {
				...
				Object.keys(so.strategy).forEach(function (strategy_name, index) {
					s.lookback[0].strategy[strategy_name] = {
						data: JSON.parse(JSON.stringify(so.strategy[strategy_name].data))
					}
				})
			...
			function initPeriodGoOn() {
				s.period = {
					...
					human_time: moment(d.toMilliseconds()).format('YYYY-MM-DD HH:mm'),
					open: trade.price,
					high: trade.price,
					low: trade.price,
					close: trade.price,
					...
					rsi: (s.lookback[0] ? s.lookback[0].rsi : null),
				}
				s.period._id = s.period.id

				updatePeriod(trade, callback)
			}
		}
		catch (err) {
...
//				position._id = position_id
	...
	function writeReport(is_progress = true, period = s.period, callback = function () {}) {
		...
		s.tools.functionStrategies('onReport', opts
			, null //callbackStrategy
			, function (result, strategy_function) { //callbackFinal
				if (s.action) {
...
//				process.stdout.write(s.tools.zeroFill((currency_col.length + 1), currency_col, ' ').green)
	...
	async function onTrade(trade, is_preroll, callback) {
		try {
			if (s.exchange.debug_exchange) {
			...
			async function goOn() {
				...
				await s.tools.functionStrategies('onTrade', opts)
				...
				if (trade.time > s.period.close_time) {
					...
					await initPeriod(trade, function() {
					...
					s.tools.functionStrategies('onTradePeriod', opts
						...
						, function (result, strategy_name, strategy_function, cb_resolve) {
							s.action = null

							// Ripulisce so.strategy[strategy_name].calc_lookback a un max di valori
							if (so.strategy[strategy_name].opts.min_periods && (so.strategy[strategy_name].calc_lookback.length > so.strategy[strategy_name].opts.min_periods)) {
