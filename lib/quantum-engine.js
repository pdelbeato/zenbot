...
module.exports = function (s, conf) {
	...
	//Attiva il listener per gli ordini di tipo manual catch
	eventBus.on('catching', (signal, position_id, fixed_size, fixed_price) => {
		debug.msg('Listener -> catching ' + signal + ' ' + formatAsset(fixed_size, s.asset) + ' at ' + formatCurrency(fixed_price, s.currency))
		executeSignal (signal, 'catching', position_id, fixed_size, fixed_price)
	})
	...
	if (so.catch_order_pct > 0) {
		eventBus.on('orderExecuted', (signal, position_id) => {
			position = s.positions.find(x => x.id === position_id)
	
			if (position && !positionStatus(position, 'Check', 'catching')) {
				...
				debug.msg('Listener -> catching position ' + signal_opposite + ' ' + position_id + ' at ' + target_price)
...
//			})
	...
	function emitSignal (sig_kind, signal, position_id, fixed_size, fixed_price, is_reorder, is_taker) {
		eventBus.emit(sig_kind, signal, position_id, fixed_size, fixed_price, is_reorder, is_taker)
	}
	...
	if (so.strategy) {
		...
		if (s.strategy.orderExecuted) {
			eventBus.on('orderExecuted', function(signal, position_id) {
				s.strategy.orderExecuted(signal, position_id, conf)
			})
		}
	}
	...
	function initOrder (orderSignal, orderKind = 'standard', position_id = null, is_taker = null, cb) {			
		order = {
			id: position_id,
			signal: orderSignal,
			kind: orderKind,
...
//		orderStatus(order, undefined, undefined, undefined, 'Set', statusByte[orderKind])
...
//Così facendo, eseguo uno StopLoss alla volta, senza scatenare l'inferno
//Non serve a molto. Se c'è un riordine, alla cancellazione del primo, s.acted_on_stop diventa null, quindi al prossimo executeStop rientra in
// questo if, esegue executeSignal e viene bloccato dal fatto che esiste già un ordine StopLoss con lo stesso signal e id.
// Da pensare se è il caso di togliere questo flag.
//				s.acted_on_stop = true
			if (position_stop && !positionStatus(position, 'Check', 'stoploss') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position_stop) < 0)) {
				...
				//Tolgo questo controllo perchè se l'ordine è già in piedi, se ne accorge executeSignal e non va più avanti
...
//			if (!s.acted_on_trail_stop && position.profit_stop && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
				//Così facendo, eseguo un TrailStop alla volta, senza scatenare l'inferno
				//Non serve a molto. Se c'è un riordine, alla cancellazione del primo, s.acted_on_trail_stop diventa null, quindi al prossimo executeStop rientra in
				// questo if, esegue executeSignal e viene bloccato dal fatto che esiste già un ordine TrailStop con lo stesso signal e id.
				// Da pensare se è il caso di togliere questo flag
//				s.acted_on_trail_stop = true
			if (position.profit_stop && !positionStatus(position, 'Check', 'trailstop') && ((position.side == 'buy' ? +1 : -1) * (s.period.close - position.profit_stop) < 0) && position.profit_pct > 0) {
...
//		debug.msg('syncBalance - Inizio... (now()=' + now() + ' ; s.next_order ' + s.next_order + ')')
		if (now() > s.next_order) {
...
//						})
						...
						if (!s.start_capital) {
							...
							//Il primo messaggio lo inserisco tra quelli che invia quantum-trade
//							if (so.mode !== 'sim') {
...
//			debug.msg('syncBalance - Attendo... (now()=' + now() + ' ; s.next_order ' + s.next_order + ')')
			setTimeout(function() { syncBalance(cb) }, (s.next_order - now() + 1))
		}
	}
...
//		}
//		else {
////			debug.msg('getQuote - Attendo... (now()=' + now() + ' ; s.next_order ' + s.next_order + ')')
//			setTimeout(function() { getQuote(cb) }, (s.next_order - now() + 1))
//		}
	}
	...
	function executeSignal (signal, sig_kind = 'standard', position_id = null, fixed_size = null, fixed_price = null, is_reorder, is_taker, _cb) {
		let expected_fee = 0
		let order_tmp
					...
					if (sig_kind === 'standard') {			
						...
						if (s.max_profit_position.trend[signal_opposite]) {
							...
							if (so['max_' + signal + '_loss_pct'] != null && (position.profit_pct + so['max_' + signal + '_loss_pct'] < 0)) {
								...
								debug.msg('executeSignal - Creo nuovo ordine ' + signal + ' standard ' + position_id)
						...
						//E' un ordine standard senza position_id, quindi dettato dalla strategia. 
						// Cancello TUTTI gli ordini standard di senso opposto ancora in essere
						debug.msg('executeSignal - Ordine standard. Cancello TUTTI gli ordini standard di senso opposto ancora in essere.')
						orderStatus(undefined, signal_opposite, 'standard', undefined, 'Unset', 'standard')
					}
...
//			}
			...
			if (!is_reorder && (sig_kind != 'stoploss') && !fixed_price && order_tmp.position.profit_pct && so['max_' + signal + '_loss_pct'] != null && (order_tmp.position.profit_pct*100 + so['max_' + signal + '_loss_pct'] < 0)) {
				let err = new Error('\nPosition ' + position_id + ' ProfitLoss protection')
				err.desc = 'refusing to ' + signal + ' at ' + formatCurrency(order_tmp.price, s.currency) + ', ' + (so['max_' + signal + '_loss_pct'] > 0 ? 'LOSS of ' : 'PROFIT of ') + formatPercent(order_tmp.position.profit_pct) + ' (limit ' + formatPercent(-so['max_' + signal + '_loss_pct'] / 100) + ')\n'
			...
			if (s.product.min_size && (Number(order_tmp.size) < Number(s.product.min_size) || ('min_total' in s.product && s.product.min_total && n(order_tmp.size).multiply(order_tmp.price).value() < Number(s.product.min_total)))) {
				let err = new Error('\nMinimum size')
			...
			//Se è un ordine standard, allora controllo di quanto balance effettivo posso disporre (balance reale meno le quantità bloccate
			// dalle posizioni più la quantità della posizione in esame, se esiste)
			available_balance_currency = (sig_kind === 'standard' ? (n(s.available_balance.currency).add(order_tmp.position.value)) : s.balance.currency)
			available_balance_asset = (sig_kind === 'standard' ? (n(s.available_balance.asset).add(order_tmp.position.size)) : s.balance.asset)
...
//Cosa succede se tutti i fondi sono hold? In qualche modo me ne devo accorgere e liberarli o considerarli prima di fare questo controllo.
			...
			switch (signal) {
				case 'buy': {
					...
					else if (n(s.balance.currency).subtract(s.balance.currency_hold || 0).add(order_tmp.position.value).value() < n(order_tmp.price).multiply(order_tmp.size).value() && s.balance.currency_hold > 0) {
						debug.msg('executeSignal - buy delayed: ' + n(available_balance_currency).subtract(s.balance.currency_hold || 0).value() + ' of funds on hold')
						debug.msg('executeSignal - s.available_balance.currency ' + s.available_balance.currency + ' s.balance.currency ' + s.balance.currency + ' - s.balance.currency_hold ' + s.balance.currency_hold + ' + order_tmp.position.value ' + order_tmp.position.value)
				...
				case 'sell': {
					...
					else if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).add(order_tmp.position.size).value() < n(order_tmp.size).value()) {
						debug.msg('executeSignal - sell delayed: ' + n(available_balance_asset).subtract(s.balance.asset_hold || 0).value() + ' of funds on hold')
						debug.msg('executeSignal - s.balance.asset ' + s.balance.asset + ' - s.balance.asset_hold ' + s.balance.asset_hold + ' + order_tmp.position.size ' + order_tmp.position.size)
						return setTimeout(function () {
							if (s.last_signal === signal) {
								s.hold_signal = true
								executeSignal(signal, sig_kind, position_id, undefined, undefined, true)
...
//		debug.msg('checkOrder')
		...
		if (now() > s.next_order) {
//			debug.msg('checkOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + ' - now() ' + now() + ' ; s.next_order ' + s.next_order)
			s.next_order = now() + 1000/max_requests_per_second
...
//			debug.msg('checkOrder - ' + signal + ' ' + sig_kind + ' ' + position_id + '. Attendo... (now()=' + now() + ' ; s.next_order ' + s.next_order + ')')
			setTimeout(function() { checkOrder(signal, sig_kind, position_id, cb) }, (s.next_order - now() + 1))
		}
	}
...
//			positionStatus(position, 'Unset', sig_kind)
		...
		if (!is_partial) {
			...
			emitSignal('orderExecuted', signal, position_id)
		}
		else {
	...
	function writeReport (is_progress, blink_off) {
		...
		if (orderExist('buy', 'standard')) {
			process.stdout.write(z(9, 'buying', ' ').green)
		}
		else if (orderExist('sell', 'standard')) {
			process.stdout.write(z(9, 'selling', ' ').red)
		}
		else if (s.action) {
			process.stdout.write(z(9, s.action, ' ')[(s.action === 'bought' || s.action === 'part buy') ? 'green' : 'red'])
		}
		else if (s.signal) {
			...
			process.stdout.write(z(9, buysell + buysell_profit, ' ')[n(buysell_profit) > 0 ? 'green' : 'red'])
		}
		else {
		...
		if (s.max_profit_position.trail.buy != null || s.max_profit_position.trail.sell != null) {
			...
			process.stdout.write(z(9, buysell + buysell_profit, ' ')['yellow'])
		}
		else {
			process.stdout.write(z(9, '', ' '))
		}
...
//		debug.msg('updatePositions')
		...
		s.positions.forEach(function (position, index) {
			...
			//Aggiorno i profit trailing stop
...
// Da controllare se con le modifiche fatte, il problema seguente è risolto.
	...
	function positionStatus(position, mode, status, cb = function() {}) {
		switch (mode) {
		case 'Set': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Set ' + status + ') -> ' + (position.status | statusByte[status]))
			position.status = (position.status | statusByte[status])
			return cb()
			break
		}
		case 'Unset': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Unset ' + status + ') -> ' + (position.status & ~statusByte[status]))
			position.status = (position.status & ~statusByte[status])
			return cb()
			break
		}
		case 'Free': {
			debug.msg('positionStatus - position ' + position.id + ' Status= ' + position.status + ' -> (Free) -> ' + statusByte.free)
			position.status = statusByte.free
			return cb()
			break
		}
		case 'Check': {
//			debug.msg('positionStatus - position ' + position.id + ' Check ' + status + ' (position.status= ' + position.status + ')')
			return (position.status & statusByte[status])
		}
		}
	}
