...
module.exports = function (s, conf) {
	...
	s.tools = {
		...
		// 								console.error('quantum-tools - ' + strategy_name + ' ' + strategy_function + ': error= ' + err)
		// 								resolve(err)
		// 							})
		// 						}
		// 						else {
		...
		functionStrategies: function (strategy_function, opts = {}, callbackStrategy, callbackFinal) {
			...
			if (!_.isFunction(callbackStrategy)) {
				callbackStrategy = function (_result, _strategy_name, _strategy_function, cb) {
					cb()
				}
			}

			if (!_.isFunction(callbackFinal)) {
				callbackFinal = function (err, result) {}
			}

			if (Object.keys(s.options.strategy).length == undefined) {
			...
			Object.keys(s.options.strategy).forEach(function (strategy_name, index, array) {
				if (s.options.strategy[strategy_name].lib[strategy_function]) {
					s.options.strategy[strategy_name].lib[strategy_function](s, opts, function (err, result) {
						...
						callbackStrategy(result, strategy_name, strategy_function, checkLast)
					})
				}
				else {
					callbackStrategy(null, strategy_name, strategy_function, checkLast)
				}

				function checkLast() {
					if (index == (array.length-1)) {
						return callbackFinal(null, strategy_function)
					}
				}
			})
		},
