...
module.exports = function vwap (s, strategy_name, length, max_period, source_key) {
	...
	if (VWAP_LOOKBACK.length >= length) {
		...
		VWAP_DATA.vwap.vwapMultiplier = VWAP_DATA.vwap.vwapMultiplier + parseFloat(VWAP_PERIOD[source_key]) * parseFloat(VWAP_PERIOD['volume'])
		VWAP_DATA.vwap.vwapDivider = VWAP_DATA.vwap.vwapDivider + parseFloat(VWAP_PERIOD['volume'])

		VWAP_PERIOD.vwap = VWAP_DATA.vwap.vwap = (VWAP_DATA.vwap.vwapMultiplier / VWAP_DATA.vwap.vwapDivider)
