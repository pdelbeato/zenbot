...
module.exports = function ta_stoch(s, strategy_name, k_periods = 5, sk_periods = 3, k_ma_type = 'SMA', d_periods = 3, d_ma_type = 'SMA') {
  return new Promise(function (resolve, reject) {
    var min_length = Math.max(k_periods, d_periods, sk_periods)+5

    if (strategy_name) {
      var tmpMarket = s.options.strategy[strategy_name].calc_lookback.slice(0, min_length).reverse()
    }
    else {
      var tmpMarket = s.lookback.slice(0, min_length).reverse()
    ...
    if (tmpMarket.length >= min_length) {
      ...
      talib.execute({
        ...
        if (strategy_name) {
          s.options.strategy[strategy_name].data.stoch = result_stoch
        }

        resolve(result_stoch)
      })
    }
    else {
      reject('ta_stoch - (nice error) - MarketLength not populated enough')
    }
  })
}
