...
//						if (period.close > last_close) {
		...
		if (s.options.strategy[strategy_name].calc_lookback.length >= length) {
			var avg_gain = 0 //s.options.strategy[strategy_name].calc_lookback[0][key + '_avg_gain']
			var avg_loss = 0 //s.options.strategy[strategy_name].calc_lookback[0][key + '_avg_loss']
//			if (typeof avg_gain === 'undefined') {
				var gain_sum = 0
				var loss_sum = 0
				var last_close
