	...
	if(strategy_name == undefined) {
		if (s.lookback.length >= length) {
			...
			if (typeof avg_gain === 'undefined') {
				...
				var loss_sum = 0
				var last_close
				//ATTENZIONE!!! I valori sono richiamati in ordine inverso, quindi anche la formula andava ragionata in modo inverso
...
//							loss_sum += last_close - period.close
		...
		if (s.options.strategy[strategy_name].calc_lookback.length >= length) {
			...
			var rsi_avg_gain = null
			var rsi_avg_loss = null
			var rsi = null
			...
			if (typeof avg_gain === 'undefined') {
				...
				rsi_avg_gain = gain_sum / length
				rsi_avg_loss = loss_sum / length	
			}
			else {
				var current_gain = s.period.close - s.options.strategy[strategy_name].calc_lookback[0].close
				rsi_avg_gain = ((avg_gain * (length - 1)) + (current_gain > 0 ? current_gain : 0)) / length
				var current_loss = s.options.strategy[strategy_name].calc_lookback[0].close - s.period.close
				rsi_avg_loss = ((avg_loss * (length - 1)) + (current_loss > 0 ? current_loss : 0)) / length
			}

			if (rsi_avg_loss == 0) {
				rsi = 100
			} else {
				var rs = rsi_avg_gain / rsi_avg_loss
				rsi = precisionRound(100 - (100 / (1 + rs)), 2)
			}
			const results = {
					rsi: rsi,
