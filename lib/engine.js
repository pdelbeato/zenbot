...
let clock
let nice_errors = new RegExp(/(slippage protection|loss protection|max quantum reached|pumpdump watchdog)/)
...
module.exports = function (s, conf) {
  ...
  function nextBuyForQuote(s, quote) {
    if (s.next_buy_price)
      ...
      msg('nextBuyForQuote: bid= ' + quote.bid + 'return= ' +n(quote.bid).subtract(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).add(so.best_bid ? s.product.increment : 0).format(s.product.increment, Math.floor))
        ...
        if (!s.acted_on_stop) {
          if (position.type === 'buy') {
            if (do_sell_stop && position.sell_stop && s.period.close < position.sell_stop) {
              stop_signal = 'sell'
              console.log(('\nsell stop triggered at ' + formatPercent(s.trade_worth) + ' trade worth for position ' + index + '\n').red)
            }
            else if (so.profit_stop_enable_pct && s.trade_worth >= (so.profit_stop_enable_pct / 100)) {
              s.profit_stop_high = Math.max(s.profit_stop_high || s.period.close, s.period.close)
              s.profit_stop = s.profit_stop_high - (s.profit_stop_high * (so.profit_stop_pct / 100))
            }
            if (s.profit_stop && s.period.close < s.profit_stop && s.trade_worth > 0) {
              stop_signal = 'sell'
              console.log(('\nprofit stop triggered at ' + formatPercent(s.trade_worth) + ' trade worth for position ' + index + '\n').green)
            }
          }
          else {
            if (s.buy_stop && s.period.close > s.buy_stop) {
              stop_signal = 'buy'
              console.log(('\nbuy stop triggered at ' + formatPercent(s.trade_worth) + ' trade worth for position ' + index + '\n').red)
            }
          }
        }
      })
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    if (s.is_dump_watchdog || s.is_pump_watchdog) {
      let err = new Error('\npumpdump watchdog')
      err.desc = 'refusing to buy/sell. PumpDump Watchdog is active! Positions opened: ' + s.my_positions.length
      return cb(err)
    }
    ...
    if (!is_reorder && s[signal + '_order']) {
      msg('Not is_reorder && esiste s[signal + "_order"]')
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (!so.quantum_size) {
        ...
        if (is_reorder && s[signal + '_order']) {
          ...
          msg('price changed, resizing order, ' + formatCurrency(s[signal + '_order'].remaining_size,s.asset) + ' remain')
          size = null
        }
      }
      ...
      if (signal === 'buy') {
        ...
        if (!so.quantum_size) {
          ...
          msg('Quantum buy order')
          if (is_reorder) {
            buy_pct = n(s[signal + '_order'].remaining_size).multiply(price).multiply(100).divide(s.balance.deposit)
            msg('Calcolo buy_pct in quantum: ' + buy_pct + '% (' + s[signal + '_order'].remaining_size + '*' + price + '*100/' + s.balance.deposit + ')')
          } else {
            //Faccio in modo che la buy_pct sia calcolata per farmi avere quantum_size come size per il trade
            buy_pct = n(so.quantum_size).multiply(100).divide(s.balance.deposit)
            msg('Calcolo buy_pct in quantum: ' + buy_pct + '% (' + so.quantum_size + '*100/' + s.balance.deposit + ')')
          }
        }
        ...
        if (!so.quantum_size) {
          ...
          if (is_reorder) {
            ...
            msg('Not is_reorder: position_max_profit_index ' + position_max_profit_index)
            ...
            if (so.max_sell_loss_pct != null && sell_loss > so.max_sell_loss_pct) {
              let err = new Error('\nposition ' + working_position_index + ' loss protection')
              err.desc = 'refusing to sell at ' + formatCurrency(price, s.currency) + ', sell loss of ' + formatPercent(sell_loss / 100)
              return cb(err)
            }
          }
          ...
          if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
            ...
            if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
              msg('sell delayed: ' + formatPercent(n(s.balance.asset_hold || 0).divide(s.balance.asset).value()) + ' of funds (' + formatAsset(s.balance.asset_hold, s.asset) + ') on hold')
              msg('s.balance.asset ' + s.balance.asset + ' s.balance.asset_hold ' + s.balance.asset_hold + ' size ' + size)
              return setTimeout(function () {
                if (s.last_signal === signal) {
                  executeSignal(signal, cb, size, true)
                }
      ...
      placeOrder(signal, {
        ...
        if (!order) {
          ...
          if (remaining_size !== size) {
            msg('remaining size: ' + remaining_size + ' of ' + s[signal + '_order'].size)
          }
  ...
  function executeOrder (trade, type) {
    ...
    if (s.buy_order) {
      ...
      delete s.buy_stop
      delete s.sell_stop
      if (!so.quantmu_size && so.sell_stop_pct) {
      ...
      if (so.quantum_size) {
        ...
        msg('delete position ' + working_position_index + '(lenght attuale ' + s.my_positions.length +')')
        s.my_positions.splice(working_position_index, 1)

        //debug
        msg('Lista posizioni rimaste')
        let temp = JSON.parse(JSON.stringify(s.my_positions))
        console.error(temp)
      }
          ...
          if (api_order) {
            ...
            if (api_order.filled_size) {
              ...
              if (!((s.product.min_size && Number(order.remaining_size) >= Number(s.product.min_size)) || (s.product.min_total && n(order.remaining_size).multiply(order.price).value() >= Number(s.product.min_total)))) {
                msg('cancelOrder: order.remaining_size < minimo ordine possibile (o errore equivalente')
